(() => {
  ...
  const BH_VERSION = "2026-07-24-performance-pause-v6";
  ...
  async function firebaseRuntime() {
    ...
    rt.firebasePromise = (async () => {
      if (typeof loadFirebasePhoneRuntime === "function") {
      ...
      let db;
      try { db = dbMod.initializeFirestore(app, { experimentalForceLongPolling: true, experimentalLongPollingOptions: { timeoutSeconds: 30 }, useFetchStreams: false }, BH_DATABASE_ID); }
      catch { db = dbMod.getFirestore(app, BH_DATABASE_ID); }
  ...
  function requireUser(fb = rt.fb) {
    ...
    if (!user) throw new Error("Bitte zuerst mit deinem JK.Games-Account anmelden.");
    return user;
  }
  ...
  function startTicker() {
    ...
    rt.ticker = setInterval(() => {
      ...
      mutate((next) => tickGame(next)).catch((error) => toast(onlineErrorText(error)));
    }, interval);
  }
  ...
  function launcherHtml() {
    const meta = gameMeta();
    return `<section class="bh-shell bh-launcher" style="--accent:${meta.accent}"><header class="bh-header"><button data-bh-close>×</button><div><p>JK.GAMES · BATTLE APPS</p><h2>Drei eigenständige Spiele</h2></div><div class="bh-live-pill">2–4 Spieler</div></header><div class="bh-launch-hero"><div><span>KL</span><p>BOT · ONLINE · PRIVATCODE</p><h1>JK.Games Battle</h1><small>Jedes Spiel kann im Life App Store einzeln installiert werden.</small></div></div><div class="bh-launch-grid">${gameCard("territory")}${gameCard("packages")}${gameCard("reaction")}</div><button class="bh-return-am" data-bh-close>Battle Hub schließen</button></section>`;
  }

  function singleLauncherHtml() {
    const meta = gameMeta(rt.gameType);
    const pausedLocal = rt.local && !rt.local.online && rt.local.type === rt.gameType && ["playing", "roundOver", "seriesOver"].includes(rt.local.status);
    return `<section class="bh-shell bh-launcher bh-single-launcher" style="--accent:${meta.accent}">
      ...
      ${pausedLocal ? `<button class="bh-resume-local" data-bh-resume-local><span>▶</span><div><b>Gestoppte Partie fortsetzen</b><small>Das Spiel lief im Hintergrund nicht weiter.</small></div><i>›</i></button>` : ""}
  ...
  function openSingle(type = "territory", target = "home") {
    ...
    overlay.classList.add("show");
    document.body.classList.add("bh-open");
    if (rt.view === "online") listenPublicRooms().catch((error) => toast(onlineErrorText(error)));
    render();
  }
  ...
  function pauseBackgroundWork() {
    ...
    rt.holdTimer = null;
    const game = currentGame();
    // Online-Partien nutzen absolute Zeitstempel und müssen beim Host weiterlaufen.
    // Lokale Bot-Partien werden vollständig gestoppt, bis die App erneut geöffnet wird.
    if (!game?.online) {
      clearInterval(rt.ticker);
      rt.ticker = null;
    }
  ...
  function close() {
    ...
    rt.pendingPower = "";
    rt.pendingCard = "";
    rt.packageCardsOpen = false;
    rt.territoryPowerOpen = false;
  }
  ...
  function onlineErrorText(error) {
    ...
    if (code.includes("permission-denied") || /missing or insufficient permissions/i.test(text)) {
      return "Online-Spiele sind derzeit nicht verfügbar. Bitte später erneut versuchen.";
    }
    return text;
  }
  ...
  async function listenPublicRooms() {
    ...
    }, (error) => toast(`Raumliste: ${onlineErrorText(error)}`));
  }
  ...
  async function watchRoom(code) {
    ...
    }, (error) => toast(`Raumverbindung: ${onlineErrorText(error)}`));
  }
  ...
  async function leaveRoom() {
    ...
    } catch (error) { toast(onlineErrorText(error)); }
    disconnectRoom();
  }
  ...
  function bindOverlay(shell) {
    ...
    shell.querySelectorAll("[data-bh-online-menu]").forEach((button) => button.addEventListener("click", () => { rt.gameType = button.dataset.bhOnlineMenu; rt.view = "online"; listenPublicRooms().catch((error) => toast(onlineErrorText(error))); render(); }));
    shell.querySelector("[data-bh-resume-local]")?.addEventListener("click", () => {
      if (!rt.local || rt.local.type !== rt.gameType) return;
      rt.view = "game";
      render();
      if (rt.local.status === "playing") startTicker();
    });
    shell.querySelector("[data-bh-start-local]")?.addEventListener("click", () => startLocal(shell));
    shell.querySelector("[data-bh-create-room]")?.addEventListener("click", () => createRoom(shell).catch((error) => toast(onlineErrorText(error))));
    shell.querySelector("[data-bh-join-code]")?.addEventListener("click", () => joinRoom(shell.querySelector("[data-bh-room-code]")?.value).catch((error) => toast(onlineErrorText(error))));
    shell.querySelectorAll("[data-bh-join-room]").forEach((button) => button.addEventListener("click", () => joinRoom(button.dataset.bhJoinRoom).catch((error) => toast(onlineErrorText(error)))));
    shell.querySelector("[data-bh-refresh-rooms]")?.addEventListener("click", () => listenPublicRooms().catch((error) => toast(onlineErrorText(error))));
    shell.querySelector("[data-bh-start-online]")?.addEventListener("click", () => startOnline(shell).catch((error) => toast(onlineErrorText(error))));
    ...
    shell.querySelector("[data-bh-next-round]")?.addEventListener("click", () => nextRound().catch((error) => toast(onlineErrorText(error))));
    shell.querySelector("[data-bh-rematch]")?.addEventListener("click", () => rematch().catch((error) => toast(onlineErrorText(error))));
    ...
    shell.querySelectorAll("[data-bh-cell]").forEach((button) => button.addEventListener("click", () => {
      ...
      if (rt.pendingPower === "bomb") {
        mutate((next) => useTerritoryPower(next, ownIndex, "bomb", { cell })).then((ok) => { if (ok !== false) { rt.pendingPower = ""; render(); } }).catch((error) => toast(onlineErrorText(error)));
      } else mutate((next) => territoryMove(next, ownIndex, cell)).catch((error) => toast(onlineErrorText(error)));
    }));
    ...
    shell.querySelectorAll("[data-bh-territory-power]").forEach((button) => button.addEventListener("click", () => {
      ...
      const game = currentGame();
      mutate((next) => useTerritoryPower(next, ownPlayerIndex(game), type)).then(() => { rt.territoryPowerOpen = false; render(); }).catch((error) => toast(onlineErrorText(error)));
    }));
    ...
    shell.querySelectorAll("[data-bh-sort-city]").forEach((button) => button.addEventListener("click", () => {
      ...
      rt.careful = false;
      mutate((next) => packageProcess(next, ownIndex, button.dataset.bhSortCity, careful)).catch((error) => toast(onlineErrorText(error)));
    }));
    ...
    shell.querySelectorAll("[data-bh-card-target]").forEach((button) => button.addEventListener("click", () => {
      const game = currentGame();
      const ownIndex = ownPlayerIndex(game);
      mutate((next) => usePackageCard(next, ownIndex, rt.pendingCard, Number(button.dataset.bhCardTarget))).then(() => { rt.pendingCard = ""; rt.packageCardsOpen = false; render(); }).catch((error) => toast(onlineErrorText(error)));
    }));
    ...
    shell.querySelectorAll("[data-bh-reaction-answer]").forEach((button) => button.addEventListener("click", () => {
      const game = currentGame();
      mutate((next) => submitReaction(next, ownPlayerIndex(game), button.dataset.bhReactionAnswer)).catch((error) => toast(onlineErrorText(error)));
    }));
    ...
    if (hold) {
      const begin = (event) => {
        ...
        rt.holdTimer = setTimeout(() => {
          const game = currentGame();
          mutate((next) => submitReaction(next, ownPlayerIndex(game), "hold")).catch((error) => toast(onlineErrorText(error)));
    ...
    if (swipe) {
      ...
      swipe.addEventListener("pointerup", (event) => {
        ...
        if (dx < -55 && dy < 70) {
          const game = currentGame();
          mutate((next) => submitReaction(next, ownPlayerIndex(game), "left")).catch((error) => toast(onlineErrorText(error)));
        }
      });
    }
  }
  ...
  window.LifeBuilderBattleHub = { open, openSingle, close, suspend: close, version: BH_VERSION };
  window.addEventListener("resize", () => { const game = currentGame(); if (game?.type === "territory" && rt.overlay?.classList.contains("show")) requestAnimationFrame(() => drawTerritoryCanvas(rt.overlay?.querySelector("[data-bh-territory-canvas]"), game)); });
  document.addEventListener("visibilitychange", () => { if (document.hidden && rt.overlay?.classList.contains("show")) close(); });
