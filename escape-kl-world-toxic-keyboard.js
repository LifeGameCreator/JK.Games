...
export function buildToxicKeyboardWorld(api){
  ...
  title(1,'ACID WARMUP');for(let i=0;i<8;i++)key(4.4+i*.08,i%2?2.1:-2.1,.65,3.45,2.75,i%2?'A':'D');win(1,rewards[0]);
  ...
  z-=3.0;title(5,'GLITCH DROP');addSign('GL = NUR OPTISCH · PLATTFORM BLEIBT FEST',{x:0,y:2.75,z:z+.55},0xffd36a,.31);
  ...
  z-=3.1;title(7,'CHASE WALL · LAUF!');addSign('⚠ TOXIC WALL STARTET HINTER DIR · NICHT STEHEN BLEIBEN',{x:0,y:2.72,z:z+.55},0xffd36a,.31);
  const chaseTrigger=z-1.5,chaseStart=z+6.5;
  for(let i=0;i<10;i++)key(8.0+i*.12,i%2?2.5:-2.5,5.2+(i%3)*.12,3.05,2.35,i===8?'ESC':'RUN',0x347c40,{motion:i===3||i===6?{axis:'x',amp:1.7,speed:.8,phase:i}:null});
