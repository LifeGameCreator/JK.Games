...
export function buildToxicKeyboardWorld(api){
  ...
  z-=2.8;title(2,'AIR CONTROL SHAFT',4.6);
  // Jump into the open shaft and steer laterally while airborne to catch the offset keys.
  key(5.5,0,1.0,5.4,2.7,'JUMP',0x337a3d);for(let i=0;i<8;i++)key(6.0+i*.18,[-2.8,2.6,-1.5,3.0,-2.7,1.6,0][i],1.0+(i%2)*.16,2.75,2.35,'AIR',0x2d7439);win(2,rewards[1],1.1);

  z-=2.8;title(3,'VERTICAL TOXIC CLIMB',4.8);
  for(let i=0;i<10;i++)key(4.8+(i%2)*.3,Math.sin(i*.9)*2.3,1.0+i*.92,3.1,2.5,i%2?'UP':'W',0x317c3e);win(3,rewards[2],8.45);

  z-=3.0;title(4,'MOVING ACID RAILS',11.15);
  for(let i=0;i<9;i++)key(7.1+i*.15,i%2?1.4:-1.4,8.1+(i%3)*.12,3.0,2.35,'MOVE',0x2d843e,{motion:{axis:'x',amp:3.2,speed:.78+i*.045,phase:i*.83}});win(4,rewards[3],8.35);

  z-=3.0;title(5,'GLITCH DROP',10.55);addSign('GL = NUR OPTISCH · PLATTFORM BLEIBT FEST',{x:0,y:2.75,z:z+.55},0xffd36a,.31);
  for(let i=0;i<10;i++)key(7.8+i*.16,(i%3-1)*2.45,8.0-i*.38,2.85,2.3,'GL',0x4b7e35,{blink:i!==0});win(5,rewards[4],5.4);

  z-=3.0;title(6,'TOXIC LONG JUMPS',8.35);
  for(let i=0;i<8;i++)key(9.3+i*.23,Math.sin(i*1.0)*2.8,5.0+i*.13,2.8,2.25,i%2?'SHIFT':'SPACE',0x2a6e34);win(6,rewards[5],5.65);

  z-=3.1;title(7,'TOXIC SPRINT',8.45);addSign('SPRINT-LINIE · KEINE BLOCKIERENDE TOXIC WALL',{x:0,y:7.55,z:z+.55},0xffd36a,.31);
  // V485: Die alte Chase-Wand stand direkt in der Sprunglinie und war an der letzten Plattform zu knapp.
  // Der Parcours bleibt hier frei und etwas breiter. Die Toxic-Wand steht nur noch seitlich als sichtbares
  // Umgebungsobjekt und kann den Spieler nicht mehr aus einem eigentlich korrekten Sprung werfen.
  const wallDisplayZ=z-31;boxDeco(27,5.2,wallDisplayZ,2.4,10.4,30,0x2f7c38);addGlowLight(24.7,6.2,wallDisplayZ,0x78ff50,.55,11);
  for(let i=0;i<10;i++)key(6.8+i*.07,i%2?2.2:-2.2,5.2+(i%3)*.12,3.55,2.8,i===8?'ESC':'RUN',0x347c40,{motion:i===3||i===6?{axis:'x',amp:1.25,speed:.72,phase:i}:null});
  win(7,rewards[6],5.45);

  z-=3.1;title(8,'HIGH RISE KEYS',8.65);
  for(let i=0;i<11;i++)key(7.2+(i%3)*.25,Math.cos(i*.8)*2.7,5.1+i*.88,2.75,2.25,i%2?'W':'SPACE',0x3d8248);win(8,rewards[7],14.0);

  z-=3.2;title(9,'CHAOS AIR KEYS',17.2);
  for(let i=0;i<10;i++)key(11.3+i*.18,Math.sin(i*1.25)*3.0,13.6+(i%2)*.16,2.65,2.15,'X',0x3a7f42,{motion:i%3===1?{axis:'x',amp:2.1,speed:.96,phase:i}:null,blink:i===3||i===6});win(9,rewards[8],13.85);

  z-=3.4;title(10,'SPEED 300 TOXIC FINALE',17.35);
