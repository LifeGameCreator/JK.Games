/* Escape.kl World 3 – Toxic Keyboard V437. Hard difficulty. */
export function buildToxicKeyboardWorld(api){
  ...
  const rewards=[120,170,230,310,420,560,750,1000,1350,1800];
  const startZ=-70;
  ...
  addPlatform({x:0,y:.3,z:startZ,w:13,h:.6,d:10,color:0x245d2c,label:'TOXIC',stage:1,kind:'start'});addSign('WORLD 3 · TOXIC KEYBOARD',{x:0,y:5.2,z:startZ+4.4},0x7bff79,1.0);addSign('SCHWER · 10 STAGES',{x:0,y:4.0,z:startZ+4.42},0xffd16b,.48);
  let z=startZ-10;
  ...
  const win=(stage,reward,y=.8,finish=false)=>{z-=4.4;addPlatform({x:0,y:y-.16,z,w:8.3,h:.38,d:5.2,color:0x16361d,label:'',kind:'safe-zone'});addPlatform({x:0,y:y+.15,z,w:4.4,h:.2,d:2.25,color:0xdfaa2d,label:`WIN +${reward}`,kind:'win-pad',winReward:reward,winStage:stage,stage:Math.min(10,stage+1),finish});addSign(`+${reward} WINS`,{x:0,y:y+2.0,z:z+.1},0xffd35b,.5);};
  ...
  z-=5.5;addPlatform({x:0,y:4.05,z,w:10.5,h:.6,d:6.5,color:0x245e2c,label:'FINISH',kind:'safe-zone'});addPlatform({x:0,y:4.43,z,w:5.2,h:.2,d:2.55,color:0xe1ac2f,label:`WIN +${rewards[9]}`,kind:'win-pad',winReward:rewards[9],winStage:10,stage:10,finish:true});addRingDeco(0,6.0,z-2.5,1.9,.08,0x75ff72,0);addRingDeco(0,6.0,z-2.42,1.35,.05,0xffd45e,0);addGlowLight(0,6.1,z-2.2,0x69ff70,1.4,12);addSign('TOXIC KEYBOARD COMPLETE',{x:0,y:6.65,z:z+3.2},0x7cff79,.76);addSign(`+${rewards[9]} WINS`,{x:0,y:5.58,z:z+3.22},0xffd35b,.55);addInteractable('toxic-hub-return','Zum Hub zurück',0,4.8,z-1.8,5,returnHub);
}
