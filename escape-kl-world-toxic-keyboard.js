/* Escape.kl World 3 – Toxic Keyboard V461. Full endgame redesign with air-control, vertical jumps, moving keys and a lethal chase wall. */
export function buildToxicKeyboardWorld(api){
  const {addPlatform,addSign,boxDeco,addCylinderDeco=()=>{},addRingDeco=()=>{},addGlowLight=()=>{},addInteractable,addChaseWall=()=>{},finishAndReturnHub=()=>{}}=api;
  ...
  const startZ=-70;
  boxDeco(0,-9.0,-340,92,1.2,650,0x061109);boxDeco(-38,-2.0,-340,2.7,12,635,0x14321a);boxDeco(38,-2.0,-340,2.7,12,635,0x14321a);
  for(let zz=-100,i=0;zz>-620;zz-=48,i++){addCylinderDeco(-31,-1.5,zz,2.0,2.4,2.0,0x57d45d,0x173c1b,12);addCylinderDeco(31,-1.5,zz,2.0,2.4,2.0,0x57d45d,0x173c1b,12);addGlowLight(i%2?-30:30,2.3,zz,0x64ff6d,.42,9);}
  ...
  z-=5.8;addPlatform({x:0,y:14.45,z,w:11,h:.6,d:6.5,color:0x245e2c,label:'FINISH',kind:'safe-zone',stage:10});addPlatform({x:3.15,y:14.83,z,w:3.5,h:.2,d:2.4,color:0xe1ac2f,label:`WIN +${compact(rewards[9])}`,kind:'win-pad',winReward:rewards[9],winStage:10});addPlatform({x:0,y:14.82,z:z-2.45,w:4.0,h:.18,d:1.2,color:0x55dc5e,label:'FINISH',kind:'finish-strip',finish:true,stage:10});addRingDeco(0,16.4,z-2.5,1.9,.08,0x75ff72,0);addRingDeco(0,16.4,z-2.42,1.35,.05,0xffd45e,0);addGlowLight(0,16.5,z-2.2,0x69ff70,1.4,12);addSign('TOXIC KEYBOARD COMPLETE',{x:0,y:17.1,z:z+3.2},0x7cff79,.76);addSign('ENDGAME · SPEED 300',{x:0,y:16.0,z:z+3.22},0xffd35b,.55);addInteractable('toxic-hub-return','Finish + Wins · Zum Hub',0,15.2,z-1.8,5,finishAndReturnHub);
}
