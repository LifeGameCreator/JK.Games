(() => {
  ...
  const VERSION = "20260804-dungeon-kl-v175-coop-party-damage-sync";
  const MAX_LEVEL = 100;
  ...
  function activeBuffTotals(data){ensureProfessionData(data);data.activeBuffs=data.activeBuffs.filter(x=>Number(x.expiresAt||0)>Date.now());const total={healthPct:0,damagePct:0,healingPct:0,armorPct:0,powerFlat:0};for(const x of data.activeBuffs)for(const key of Object.keys(total))total[key]+=Number(x.buff?.[key]||0);return total;}
  ...
  function makeSupportPetRuntime(profile,{x,y,ownerId,ownerLevel=1,ownerPower=100}){if(!profile)return null;const rarity=RARITIES[profile.rarity]||RARITIES.common,mult=rarity.mult,role=profile.role||"dps",level=clamp(Math.floor(ownerLevel||1),1,MAX_LEVEL),maxHp=Math.round((90+level*24+ownerPower*.12)*(role==="tank"?1.35:role==="healer"?.78:.9)*mult);return {id:String(profile.id||uid()),ownerId:String(ownerId||""),species:String(profile.species||"wolf"),name:String(profile.name||"Begleiter"),rarity:profile.rarity||"common",role,level,x:x-55,y:y+34,netX:x-55,netY:y+34,maxHp,hp:maxHp,armor:role==="tank"?.32:role==="healer"?.08:.14,damage:Math.round((10+ownerPower*(role==="dps"?.26:.11))*mult),healing:Math.round((8+ownerPower*.09)*mult),shield:0,attackCd:0,angle:0,moving:false,attackAnim:0,radius:15,buffs:{},lastDamageAt:performance.now()};}
  ...
  function playerStats(data=ensureState()) {
    ...
    const health = Math.round((levelScale * role.hp + sum("health")) * (1 + talents.health + buffs.healthPct));
    const damage = Math.round((18 + d.level * 5.1 + sum("damage")) * role.damage * (1 + talents.damage + buffs.damagePct));
    const healing = Math.round((8 + d.level * 3.2 + sum("healing")) * (role.heal || .18) * (1 + talents.healing + buffs.healingPct));
    const armor = clamp(role.armor + sum("armor") / Math.max(1200, levelScale * 8) + talents.armor + buffs.armorPct, 0, .72);
    const crit = clamp(.05 + sum("crit") / 100 + talents.crit, .05, .55);
    const haste = clamp(sum("haste") / 100 + talents.haste, 0, .62);
    const rawPower = d.level * 80 + items.reduce((total, item) => total + item.power, 0);
    const power = Math.round(rawPower * (1 + talents.power) + health*.08 + damage*1.3 + healing*.8 + buffs.powerFlat);
  ...
  function itemEquipScore(item,data=ensureState()){if(!item)return -Infinity;const role=data.role;let score=Number(item.power||0)*3+Number(item.health||0)*.35+Number(item.damage||0)*(role==="dps"?2.1:role==="tank"?1.25:1.05)+Number(item.healing||0)*(role==="healer"?2.35:.35)+Number(item.armor||0)*(role==="tank"?8:4)+Number(item.crit||0)*10+Number(item.haste||0)*9+rarityIndex(item.rarity)*18;return score;}
  ...
  const DUNGEON_MOD_RELEVANT_PERMISSIONS=["character.write","player.write","money.write","items.write","games.write","shop.write"];
  function normalizedDungeonStaffContext(raw={}){const role=String(raw?.role||"").toLowerCase(),fallback=DUNGEON_STAFF_ROLE_PERMISSIONS[role]||[],permissions=Array.isArray(raw?.permissions)?raw.permissions.map(String):fallback;return{role,roleLabel:String(raw?.roleLabel||DUNGEON_STAFF_ROLE_LABELS[role]||role||"Teamrolle"),permissions,active:raw?.active!==false};}
  ...
  function dungeonStaffHas(permission,staff=currentStaffContext()){return !!staff?.active&&(staff.permissions?.includes("*")||staff.permissions?.includes(permission));}
  function dungeonStaffHasAny(list,staff=currentStaffContext()){return list.some(permission=>dungeonStaffHas(permission,staff));}
  function canUseDungeonModMenu(staff=currentStaffContext()){return !!staff?.active&&dungeonStaffHasAny(DUNGEON_MOD_RELEVANT_PERMISSIONS,staff);}
  function dungeonModCapabilities(staff=currentStaffContext()){return{profile:dungeonStaffHas("character.write",staff)||dungeonStaffHas("player.write",staff),money:dungeonStaffHas("money.write",staff),items:dungeonStaffHas("items.write",staff),games:dungeonStaffHas("games.write",staff),shop:dungeonStaffHas("shop.write",staff),owner:dungeonStaffHas("*",staff)};}
  function requireDungeonModCapability(capability,message="Für diese Funktion fehlt deiner Teamrolle das erforderliche Recht."){const staff=UI.dungeonVerifiedStaff||currentStaffContext(),caps=dungeonModCapabilities(staff);if(caps.owner||caps[capability])return true;toast("Keine Berechtigung",message);return false;}
  async function verifyDungeonStaffRole(){if(!canUseDungeonModMenu())return null;try{const {fb,user}=await firebase(),snap=await fb.getDoc(fb.doc(fb.db,"staffRoles",user.uid));if(!snap.exists())return null;const data=snap.data()||{},staff=normalizedDungeonStaffContext({...data,active:data.active!==false});return canUseDungeonModMenu(staff)?staff:null;}catch(error){console.warn("Dungeon.KL Rollenprüfung",error);return null;}}
  async function openDungeonModMenu(){if(!canUseDungeonModMenu())return toast("Kein Zugriff","Für deine aktive Teamrolle sind keine Dungeon.KL-Mod-Rechte freigeschaltet.");toast("Rolle wird geprüft","Firebase bestätigt deine Teamrechte.");const verified=await verifyDungeonStaffRole();if(!verified)return toast("Zugriff nicht bestätigt","Die aktive Teamrolle oder ihre Dungeon.KL-Rechte konnten nicht bestätigt werden.");UI.dungeonVerifiedStaff=verified;renderDungeonModMenu();}
  function upgradeAllInventoryItems(){if(!requireDungeonModCapability("items","Zum Verbessern von Items wird das Recht items.write benötigt."))return;const d=ensureState();d.inventory=d.inventory.map(item=>{const idx=rarityIndex(item.rarity);if(idx>=RARITY_ORDER.length-1)return item;const next=RARITY_ORDER[idx+1],fresh=createItem(Math.max(1,Number(item.level||d.level)),next,item.slot,item.classId);return{...fresh,uid:item.uid,acquiredAt:item.acquiredAt||Date.now()};});refreshSessionLoadout();safeSave();toast("Inventar verbessert","Alle Items wurden um eine Seltenheitsstufe verbessert.");renderDungeonModMenu();}
  function renderDungeonModMenu(){
    ...
    const itemPanel=(caps.items||caps.owner)?`<article class="dkl-panel"><h3>Item-Geber</h3><label>Slot<select data-dkl-mod-slot>${SLOT_KEYS.map(s=>`<option value="${s}">${SLOT_DEFS[s].name}</option>`).join("")}</select></label><label>Seltenheit<select data-dkl-mod-rarity>${RARITY_ORDER.map(r=>`<option value="${r}">${RARITIES[r].name}</option>`).join("")}</select></label><label>Item-Level<input type="number" min="1" max="100" value="${d.level}" data-dkl-mod-item-level></label><div class="dkl-mod-actions"><button class="dkl-btn gold" data-dkl-mod-grant>Item geben</button><button class="dkl-btn" data-dkl-mod-fullset>Komplettes Set geben</button></div></article><article class="dkl-panel"><h3>Inventar-Werkzeuge</h3><p>${d.inventory.length} Items im aktuellen Charakter.</p><div class="dkl-mod-actions"><button class="dkl-btn" data-dkl-mod-upgrade>Alle Items +1 Seltenheit</button><button class="dkl-btn" data-dkl-mod-best>Bestes anziehen</button><button class="dkl-btn danger" data-dkl-mod-repair>Alle auf Charakterlevel setzen</button></div></article>`:"";
    ...
    UI.main.innerHTML=`<div class="dkl-page dkl-mod-page"><header class="dkl-page-head"><button class="dkl-btn" data-dkl-mod-back>← Charakterfenster</button><div><small>TEAM-MODERATION</small><h2>Dungeon.KL Mod-Menü</h2><p>${esc(roleLabel)} · aktueller Charakter: ${esc(d.name)}</p></div></header><section class="dkl-mod-grid">${profilePanel}${itemPanel}${gamePanel}</section></div>`;
    ...
    UI.main.querySelector("[data-dkl-mod-grant]")?.addEventListener("click",()=>{if(!requireDungeonModCapability("items","Zum Vergeben von Items wird das Recht items.write benötigt."))return;const slot=UI.main.querySelector("[data-dkl-mod-slot]").value,rarity=UI.main.querySelector("[data-dkl-mod-rarity]").value,level=clamp(Math.floor(Number(UI.main.querySelector("[data-dkl-mod-item-level]").value)||d.level),1,100);addItem(createItem(level,rarity,slot,d.classId),false);renderDungeonModMenu();});
    UI.main.querySelector("[data-dkl-mod-fullset]")?.addEventListener("click",()=>{if(!requireDungeonModCapability("items","Zum Vergeben von Ausrüstung wird das Recht items.write benötigt."))return;const rarity=UI.main.querySelector("[data-dkl-mod-rarity]").value,level=clamp(Math.floor(Number(UI.main.querySelector("[data-dkl-mod-item-level]").value)||d.level),1,100);for(const slot of SLOT_KEYS)addItem(createItem(level,rarity,slot,d.classId),false);safeSave();renderDungeonModMenu();});
    UI.main.querySelector("[data-dkl-mod-upgrade]")?.addEventListener("click",upgradeAllInventoryItems);
    UI.main.querySelector("[data-dkl-mod-best]")?.addEventListener("click",()=>{if(!requireDungeonModCapability("items","Zum Ändern der Ausrüstung wird das Recht items.write benötigt."))return;equipBestAvailable();});
    UI.main.querySelector("[data-dkl-mod-repair]")?.addEventListener("click",()=>{if(!requireDungeonModCapability("items","Zum Bearbeiten des Inventars wird das Recht items.write benötigt."))return;d.inventory=d.inventory.map(item=>({...createItem(d.level,item.rarity,item.slot,item.classId),uid:item.uid,acquiredAt:item.acquiredAt||Date.now()}));refreshSessionLoadout();safeSave();renderDungeonModMenu();});
  ...
  function setupDungeonNetwork(options) {
    ...
    for (const id of options.playerUids || []) { if (id === p.user.uid) continue; const pr = profileMap[id] || {}; const role = ROLES[pr.role] ? pr.role : "dps", classId = CLASSES[pr.classId] ? pr.classId : "berserker", stats = { health: Number(pr.maxHp || 500), damage: Number(pr.damage || 50), healing: Number(pr.healing || 0), armor: Number(pr.armor || 0), crit: .08, haste: .05, power: Number(pr.power || 0) }; const remote = makePlayer({ uid: id, name: pr.name || "Mitspieler", role, classId, stats, local: false, x: WORLD_W / 2 + rand(-100, 100), y: WORLD_H - 150 + rand(-30, 30), appearance:pr.appearance||null, gearVisual:pr.gearVisual||null, hunterPet:pr.hunterPet||null, actionBar:pr.actionBar||defaultActionBar(classId) }); remote.supportPet=makeSupportPetRuntime(pr.supportPet||null,{x:remote.x,y:remote.y,ownerId:remote.uid,ownerLevel:pr.level||1,ownerPower:stats.power});s.players.push(remote); s.remotePlayers.set(id, remote); }
  ...
  function updateSession(s, dt, now) {
    ...
    else { updateHunterPets(s,dt,false);updateSupportPets(s,dt,false); updateGuestVisuals(s, dt); }
    updateOutOfCombatRegen(s,dt,now);updateGroundLoot(s);updateEffects(s, dt); s.reviveCooldown=Math.max(0,finite(s.reviveCooldown,0)-dt); s.camera.x += (s.player.x - s.camera.x) * Math.min(1, dt * 7); s.camera.y += (s.player.y - s.camera.y) * Math.min(1, dt * 7);
  }
  ...
  function updateSupportPets(s,dt,combat){for(const owner of s.players){const pet=owner.supportPet;if(!pet||pet.dead)continue;pet.level=owner.local?ensureState().level:pet.level;pet.attackCd=Math.max(0,(pet.attackCd||0)-dt);pet.attackAnim=Math.max(0,(pet.attackAnim||0)-dt);const target=s.enemies.find(e=>e.id===(owner.local?s.selectedEnemyId:owner.targetEnemyId)&&!e.dead),follow={x:owner.x+42*Math.cos((owner.angle||0)+Math.PI/2),y:owner.y+42*Math.sin((owner.angle||0)+Math.PI/2)+20};if(pet.role==="healer"&&combat&&pet.attackCd<=0){const ally=s.players.filter(p=>!p.dead&&p.hp<p.maxHp).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(ally){pet.attackCd=4.2;healPlayer(s,ally,Math.max(1,pet.healing));s.effects.push({type:"natureburst",x:ally.x,y:ally.y,radius:52,life:.8,color:"#72f3a5"});}}if(target&&combat&&lineWalkable(s,pet,target,5)){const dist=distance(pet,target),desired=pet.role==="healer"?150:50,angle=Math.atan2(target.y-pet.y,target.x-pet.x);pet.angle=angle;if(dist>desired){pet.moving=tryMoveEntity(s,pet,Math.cos(angle)*150*dt,Math.sin(angle)*150*dt);if(pet.moving)pet.walkPhase=(pet.walkPhase||0)+dt*13;}else if(pet.attackCd<=0&&pet.role!=="healer"){pet.attackCd=pet.role==="dps"?.85:1.35;pet.attackAnim=.28;damageEnemy(s,target,pet.damage,owner);}}else{const angle=Math.atan2(follow.y-pet.y,follow.x-pet.x),dist=Math.hypot(follow.x-pet.x,follow.y-pet.y);pet.moving=dist>18&&tryMoveEntity(s,pet,Math.cos(angle)*145*dt,Math.sin(angle)*145*dt);if(pet.moving){pet.angle=angle;pet.walkPhase=(pet.walkPhase||0)+dt*12;}}}}
  ...
  function chooseEnemyTarget(s, enemy) {
    const visible=s.players.filter(p=>canEnemySeePlayer(s,enemy,p));
    const tankPets=s.players.map(p=>p.supportPet).filter(p=>p&&p.role==="tank"&&!p.dead&&canEnemySeePlayer(s,enemy,p));
    if(tankPets.length&&Math.random()<.38)return tankPets.sort((a,b)=>distance(enemy,a)-distance(enemy,b))[0];
    if(!visible.length)return null;
    const nearby=visible.filter(p=>distance(enemy,p)<=enemySightRange(enemy)+28),pool=nearby.length?nearby:visible,tanks=pool.filter(p=>p.role==="tank");
    if(tanks.length&&Math.random()<.84)return tanks.sort((a,b)=>distance(enemy,a)-distance(enemy,b))[0];
    return pool.sort((a,b)=>distance(enemy,a)/(ROLES[a.role].aggro||1)-distance(enemy,b)/(ROLES[b.role].aggro||1))[0];
  }
  function damagePlayer(s, p, amount, source) {
    if (!s || !p || p.dead) return 0;
    p.lastDamageAt=performance.now();p.regenAccumulator=0;
    const armor=clamp(Number(p.armor)||0,0,.82);
    let dealt=Math.max(1,Math.round((Number(amount)||0)*(1-armor)));
    ...
    if(p.hp<=0){p.hp=0;p.dead=true;p.downedAt=Date.now();if(p.local)ensureState().stats.deaths++;showMessage(`${p.name} ist gefallen`,2200);}
  ...
  function healPlayer(s,p,amount){if(!p||p.dead)return 0;const before=Math.max(0,Number(p.hp)||0),value=Math.max(1,Math.round(amount)),after=Math.min(Math.max(1,Number(p.maxHp)||1),before+value),actual=Math.max(0,after-before);p.hp=after;if(actual>0){p.healthSeq=(Number(p.healthSeq)||0)+1;s.texts.push({x:p.x,y:p.y-35,text:`+${actual}`,color:"#64f3a4",life:.9});if(s?.online&&s.host)s.networkLastWorld=0;}return actual;}
  ...
  function updateNetwork(s, now) {
    ...
    if (now - s.networkLastWrite > 55) {
      ...
      safeSetDoc(fb, s.selfRef, {
        ...
        classId: String(s.player.classId || "berserker"), power: Math.max(0, Math.round(finite(s.player.power, 0))), tradeGift:s.outgoingTradeGift?firestoreSafe(s.outgoingTradeGift):null, hunterPet:s.player.hunterPet?{id:String(s.player.hunterPet.id||""),species:String(s.player.hunterPet.species||"wolf"),name:String(s.player.hunterPet.name||"Begleiter"),level:Math.max(1,Math.floor(finite(s.player.hunterPet.level,1))),x:finite(s.player.hunterPet.x,s.player.x),y:finite(s.player.hunterPet.y,s.player.y),hp:Math.max(0,finite(s.player.hunterPet.hp,1)),maxHp:Math.max(1,finite(s.player.hunterPet.maxHp,1)),angle:finite(s.player.hunterPet.angle,0),moving:!!s.player.hunterPet.moving,attackAnim:Math.max(0,finite(s.player.hunterPet.attackAnim,0))}:null,supportPet:s.player.supportPet?firestoreSafe(s.player.supportPet):null,
  ...
  function applyRemoteInput(id,data){const s=UI.session,p=s?.remotePlayers.get(id);if(!p)return;const tx=Number(data.x??p.x),ty=Number(data.y??p.y);p.netX=tx;p.netY=ty;const dx=tx-p.x,dy=ty-p.y;tryMoveEntity(s,p,dx*.72,0);tryMoveEntity(s,p,0,dy*.72);p.moving=!!data.moving||Math.hypot(dx,dy)>.8;if(p.moving)p.walkPhase=(p.walkPhase||0)+.22;p.angle=Number(data.angle??p.angle);p.wantAttack=!!data.wantAttack;p.targetUid=String(data.targetUid||"");p.targetEnemyId=String(data.targetEnemyId||"");p.attackAnim=Math.max(p.attackAnim||0,Number(data.attackAnim||0));p.castAnim=Math.max(p.castAnim||0,Number(data.castAnim||0));p.skillAnim=Number(data.skillAnim||p.skillAnim||0);/* HP/Schild/K.O. kommen nur aus world/current vom Host. */if(data.hunterPet){if(!p.hunterPet||p.hunterPet.id!==data.hunterPet.id)p.hunterPet=makeHunterPetRuntime(data.hunterPet,{x:data.hunterPet.x??p.x,y:data.hunterPet.y??p.y,ownerId:p.uid,ownerLevel:data.hunterPet.level});p.hunterPet.netX=Number(data.hunterPet.x??p.hunterPet.x);p.hunterPet.netY=Number(data.hunterPet.y??p.hunterPet.y);p.hunterPet.x+=(p.hunterPet.netX-p.hunterPet.x)*.5;p.hunterPet.y+=(p.hunterPet.netY-p.hunterPet.y)*.5;p.hunterPet.level=Number(data.hunterPet.level||p.hunterPet.level);p.hunterPet.species=String(data.hunterPet.species||p.hunterPet.species);p.hunterPet.name=String(data.hunterPet.name||p.hunterPet.name);}if(data.supportPet){if(!p.supportPet||p.supportPet.id!==data.supportPet.id)p.supportPet=makeSupportPetRuntime(data.supportPet,{x:data.supportPet.x??p.x,y:data.supportPet.y??p.y,ownerId:p.uid,ownerLevel:data.supportPet.level,ownerPower:p.power});p.supportPet.netX=Number(data.supportPet.x??p.supportPet.x);p.supportPet.netY=Number(data.supportPet.y??p.supportPet.y);p.supportPet.x+=(p.supportPet.netX-p.supportPet.x)*.5;p.supportPet.y+=(p.supportPet.netY-p.supportPet.y)*.5;p.supportPet.hp=Number(data.supportPet.hp||p.supportPet.hp);p.supportPet.maxHp=Number(data.supportPet.maxHp||p.supportPet.maxHp);p.supportPet.angle=Number(data.supportPet.angle||0);p.supportPet.moving=!!data.supportPet.moving;p.supportPet.attackAnim=Math.max(p.supportPet.attackAnim||0,Number(data.supportPet.attackAnim||0));}if(data.tradeGift&&data.tradeGift.id!==p.lastTradeGiftId){p.lastTradeGiftId=data.tradeGift.id;s.tradeGifts ||= [];s.tradeGifts.push(data.tradeGift);applyTradeGift(s,data.tradeGift);}if(Number(data.actionSeq||0)>Number(p.lastActionSeq||0)){p.lastActionSeq=Number(data.actionSeq);p.actionSlotRequested=Number(data.actionSlotRequested||0);}if(Number(data.skillSeq||0)>Number(p.lastSkillSeq||0)){p.lastSkillSeq=Number(data.skillSeq);p.skillRequested=Number(data.skillRequested||0);}if(Number(data.reviveSeq||0)>Number(p.lastReviveSeq||0)){p.lastReviveSeq=Number(data.reviveSeq);p.reviveRequested=true;}}
  function worldSnapshot(s) {
    return firestoreSafe({
      ...
      players: s.players.filter(Boolean).map(player => ({
        ...
        power: Math.max(0, Math.round(finite(player.power, 0))), supportPet:player.supportPet?firestoreSafe(player.supportPet):null,hunterPet:player.hunterPet?{id:String(player.hunterPet.id||""),species:String(player.hunterPet.species||"wolf"),name:String(player.hunterPet.name||"Begleiter"),level:Math.max(1,Math.floor(finite(player.hunterPet.level,1))),x:finite(player.hunterPet.x,player.x),y:finite(player.hunterPet.y,player.y),hp:Math.max(0,finite(player.hunterPet.hp,1)),maxHp:Math.max(1,finite(player.hunterPet.maxHp,1)),angle:finite(player.hunterPet.angle,0),moving:!!player.hunterPet.moving,attackAnim:Math.max(0,finite(player.hunterPet.attackAnim,0))}:null
      })),
  ...
  function applyWorldSnapshot(data){const s=UI.session;if(!s)return;const incomingRun=String(data.runId||"");if(incomingRun&&incomingRun!==String(s.lastWorldRunId||"")){s.lastWorldRunId=incomingRun;s.lastWorldSeq=0;}if(Number(data.seq||0)<=Number(s.lastWorldSeq||0))return;s.lastWorldSeq=Number(data.seq);if(Number(data.room||1)!==s.room){s.room=Number(data.room);s.roomState=data.roomState;s.layout=createRoomLayout(s,s.room);s.chests=createRoomChests(s,s.room);s.pickedGroundLootIds=new Set();s.roomBannerUntil=performance.now()+2600;s.roomExitOpen=s.roomState==="door-open";s.player.x=s.layout.entry.x;s.player.y=s.layout.entry.y;s.player.path=[];}else{s.roomState=data.roomState||s.roomState;s.roomExitOpen=s.roomState==="door-open";}s.completed=!!data.completed;s.exitOpen=!!data.exitOpen;const previous=new Map((s.enemies||[]).map(e=>[e.id,e]));s.enemies=(data.enemies||[]).map(raw=>{const e=previous.get(raw.id)||{...raw,x:raw.x,y:raw.y,walkPhase:0};e.netX=Number(raw.x??e.x);e.netY=Number(raw.y??e.y);e.hp=raw.hp;e.maxHp=raw.maxHp;e.radius=raw.radius;e.boss=!!raw.boss;e.elite=!!raw.elite;e.finalBoss=!!raw.finalBoss;e.color=raw.color;e.name=raw.name;e.angle=Number(raw.angle||0);e.moving=!!raw.moving;e.attackAnim=Math.max(e.attackAnim||0,Number(raw.attackAnim||0));e.dead=false;return e;});s.projectiles=(data.projectiles||[]).map(b=>({...b,life:.42}));s.zones=(data.zones||[]).map(z=>({...z}));s.groundLoot=(data.groundLoot||[]).map(loot=>({...loot}));for(const gift of data.tradeGifts||[])applyTradeGift(s,gift);for(const raw of data.players||[]){const p=raw.uid===s.player.uid?s.player:s.remotePlayers.get(raw.uid);if(!p)continue;const previousHp=Math.max(0,Number(p.hp)||0),incomingHp=Math.max(0,Number(raw.hp)||0),incomingSeq=Math.max(0,Number(raw.healthSeq)||0),incomingHitSeq=Math.max(0,Number(raw.hitSeq)||0);p.hp=incomingHp;p.maxHp=Math.max(1,Number(raw.maxHp)||p.maxHp||1);p.dead=!!raw.dead;p.shield=Math.max(0,Number(raw.shield||0));p.healthSeq=incomingSeq;if(incomingHitSeq>Number(p.lastAppliedHitSeq||0)){p.lastAppliedHitSeq=incomingHitSeq;const hit=Math.max(0,Number(raw.lastHitAmount)||Math.max(0,previousHp-incomingHp));if(hit>0&&p.local){s.texts.push({x:p.x,y:p.y-42,text:`-${Math.round(hit)}`,color:"#ff647a",life:.85});}}p.lastAppliedHealthSeq=Math.max(Number(p.lastAppliedHealthSeq||-1),incomingSeq);if(!p.local){p.netX=Number(raw.x??p.x);p.netY=Number(raw.y??p.y);p.angle=Number(raw.angle??p.angle);p.moving=!!raw.moving;p.attackAnim=Math.max(p.attackAnim||0,Number(raw.attackAnim||0));p.castAnim=Math.max(p.castAnim||0,Number(raw.castAnim||0));p.skillAnim=Number(raw.skillAnim||p.skillAnim||0);}if(raw.hunterPet){if(!p.hunterPet)p.hunterPet=makeHunterPetRuntime(raw.hunterPet,{x:raw.hunterPet.x,y:raw.hunterPet.y,ownerId:p.uid,ownerLevel:raw.hunterPet.level});p.hunterPet.netX=Number(raw.hunterPet.x??p.hunterPet.x);p.hunterPet.netY=Number(raw.hunterPet.y??p.hunterPet.y);p.hunterPet.x+=(p.hunterPet.netX-p.hunterPet.x)*.45;p.hunterPet.y+=(p.hunterPet.netY-p.hunterPet.y)*.45;p.hunterPet.hp=Number(raw.hunterPet.hp||p.hunterPet.hp);p.hunterPet.maxHp=Number(raw.hunterPet.maxHp||p.hunterPet.maxHp);p.hunterPet.angle=Number(raw.hunterPet.angle||0);p.hunterPet.moving=!!raw.hunterPet.moving;p.hunterPet.attackAnim=Math.max(p.hunterPet.attackAnim||0,Number(raw.hunterPet.attackAnim||0));}if(raw.supportPet){if(!p.supportPet)p.supportPet=makeSupportPetRuntime(raw.supportPet,{x:raw.supportPet.x,y:raw.supportPet.y,ownerId:p.uid,ownerLevel:raw.supportPet.level,ownerPower:p.power});p.supportPet.netX=Number(raw.supportPet.x??p.supportPet.x);p.supportPet.netY=Number(raw.supportPet.y??p.supportPet.y);p.supportPet.x+=(p.supportPet.netX-p.supportPet.x)*.45;p.supportPet.y+=(p.supportPet.netY-p.supportPet.y)*.45;p.supportPet.hp=Number(raw.supportPet.hp||p.supportPet.hp);p.supportPet.maxHp=Number(raw.supportPet.maxHp||p.supportPet.maxHp);p.supportPet.angle=Number(raw.supportPet.angle||0);p.supportPet.moving=!!raw.supportPet.moving;p.supportPet.attackAnim=Math.max(p.supportPet.attackAnim||0,Number(raw.supportPet.attackAnim||0));}}if(Number(data.lootSeq||0)>s.lastLootSeq){const count=Number(data.lootSeq)-s.lastLootSeq;s.lastLootSeq=Number(data.lootSeq);for(let i=0;i<count;i++)awardGuestLoot(s);}if(s.exitOpen)UI.main?.querySelector("[data-dkl-exit]")?.removeAttribute("hidden");}
