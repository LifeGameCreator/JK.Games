(() => {
  ...
  const VERSION = "20260804-dungeon-kl-v171-smart-inventory-mod-professions-companions";
  const MAX_LEVEL = 100;
  ...
  function currentStaffContext(){try{const api=window.JKGamesSettingsMenu||window.LifeBuilderSettingsMenu||window.LifeBuilderOnlineMod,live=api?.getRole?.()||null;if(live)return{role:String(live.role||"").toLowerCase(),active:live.active!==false};const parsed=JSON.parse(sessionStorage.getItem("lifebuilder-2026-online-mod-session")||"null");if(!parsed?.authorized||Number(parsed.expiresAt||0)<=Date.now())return{role:"",active:false};return{role:String(parsed.role||"").toLowerCase(),active:true};}catch{return{role:"",active:false};}}
  function canUseDungeonModMenu(){const staff=currentStaffContext();return staff.active&&["admin","owner"].includes(staff.role);}
  async function verifyDungeonAdminRole(){if(!canUseDungeonModMenu())return false;try{const {fb,user}=await firebase(),snap=await fb.getDoc(fb.doc(fb.db,"staffRoles",user.uid));if(!snap.exists())return false;const data=snap.data()||{},role=String(data.role||"").toLowerCase();return data.active!==false&&["admin","owner"].includes(role);}catch(error){console.warn("Dungeon.KL Rollenprüfung",error);return false;}}
  async function openDungeonModMenu(){if(!canUseDungeonModMenu())return toast("Kein Zugriff","Nur aktive Admin- und Owner-Konten dürfen das Dungeon-Mod-Menü öffnen.");toast("Rolle wird geprüft","Firebase bestätigt den Zugriff.");if(!await verifyDungeonAdminRole())return toast("Zugriff nicht bestätigt","Aktive Admin- oder Owner-Rolle erforderlich.");renderDungeonModMenu();}
  function upgradeAllInventoryItems(){const d=ensureState();d.inventory=d.inventory.map(item=>{const idx=rarityIndex(item.rarity);if(idx>=RARITY_ORDER.length-1)return item;const next=RARITY_ORDER[idx+1],fresh=createItem(Math.max(1,Number(item.level||d.level)),next,item.slot,item.classId);return{...fresh,uid:item.uid,acquiredAt:item.acquiredAt||Date.now()};});refreshSessionLoadout();safeSave();toast("Inventar verbessert","Alle Items wurden um eine Seltenheitsstufe verbessert.");renderDungeonModMenu();}
  function renderDungeonModMenu(){if(!canUseDungeonModMenu())return renderInventory();UI.view="mod";const d=ensureState(),role=currentStaffContext().role;UI.main.innerHTML=`<div class="dkl-page dkl-mod-page"><header class="dkl-page-head"><button class="dkl-btn" data-dkl-mod-back>← Charakterfenster</button><div><small>ADMIN / OWNER</small><h2>Dungeon.KL Mod-Menü</h2><p>${role==="owner"?"Owner":"Admin"} · aktueller Charakter: ${esc(d.name)}</p></div></header><section class="dkl-mod-grid"><article class="dkl-panel"><h3>Level, XP & Gold</h3><label>Level<input type="number" min="1" max="100" value="${d.level}" data-dkl-mod-level></label><label>XP hinzufügen<input type="number" min="0" value="1000" data-dkl-mod-xp></label><label>Dungeon-Gold hinzufügen<input type="number" min="0" value="10000" data-dkl-mod-gold></label><div class="dkl-mod-actions"><button class="dkl-btn primary" data-dkl-mod-apply>Übernehmen</button></div></article><article class="dkl-panel"><h3>Item-Geber</h3><label>Slot<select data-dkl-mod-slot>${SLOT_KEYS.map(s=>`<option value="${s}">${SLOT_DEFS[s].name}</option>`).join("")}</select></label><label>Seltenheit<select data-dkl-mod-rarity>${RARITY_ORDER.map(r=>`<option value="${r}">${RARITIES[r].name}</option>`).join("")}</select></label><label>Item-Level<input type="number" min="1" max="100" value="${d.level}" data-dkl-mod-item-level></label><div class="dkl-mod-actions"><button class="dkl-btn gold" data-dkl-mod-grant>Item geben</button><button class="dkl-btn" data-dkl-mod-fullset>Komplettes Set geben</button></div></article><article class="dkl-panel"><h3>Inventar-Werkzeuge</h3><p>${d.inventory.length} Items im aktuellen Charakter.</p><div class="dkl-mod-actions"><button class="dkl-btn" data-dkl-mod-upgrade>Alle Items +1 Seltenheit</button><button class="dkl-btn" data-dkl-mod-best>Bestes anziehen</button><button class="dkl-btn danger" data-dkl-mod-repair>Alle auf Charakterlevel setzen</button></div></article><article class="dkl-panel"><h3>Materialien & Beruf</h3><div class="dkl-material-grid">${Object.entries(MATERIALS).map(([id,m])=>`<button data-dkl-mod-material="${id}">${m.icon} +100 ${m.name}</button>`).join("")}</div><div class="dkl-mod-actions"><button class="dkl-btn" data-dkl-mod-profession>Beruf Stufe 100</button></div></article></section></div>`;UI.main.querySelector("[data-dkl-mod-back]").onclick=renderInventory;UI.main.querySelector("[data-dkl-mod-apply]").onclick=()=>{d.level=clamp(Math.floor(Number(UI.main.querySelector("[data-dkl-mod-level]").value)||1),1,100);d.xp+=Math.max(0,Math.floor(Number(UI.main.querySelector("[data-dkl-mod-xp]").value)||0));d.gold+=Math.max(0,Math.floor(Number(UI.main.querySelector("[data-dkl-mod-gold]").value)||0));ensureHunterData(d);refreshSessionLoadout();safeSave();updateHead();renderDungeonModMenu();};UI.main.querySelector("[data-dkl-mod-grant]").onclick=()=>{const slot=UI.main.querySelector("[data-dkl-mod-slot]").value,rarity=UI.main.querySelector("[data-dkl-mod-rarity]").value,level=clamp(Math.floor(Number(UI.main.querySelector("[data-dkl-mod-item-level]").value)||d.level),1,100);addItem(createItem(level,rarity,slot,d.classId),false);renderDungeonModMenu();};UI.main.querySelector("[data-dkl-mod-fullset]").onclick=()=>{const rarity=UI.main.querySelector("[data-dkl-mod-rarity]").value,level=clamp(Math.floor(Number(UI.main.querySelector("[data-dkl-mod-item-level]").value)||d.level),1,100);for(const slot of SLOT_KEYS)addItem(createItem(level,rarity,slot,d.classId),false);safeSave();renderDungeonModMenu();};UI.main.querySelector("[data-dkl-mod-upgrade]").onclick=upgradeAllInventoryItems;UI.main.querySelector("[data-dkl-mod-best]").onclick=equipBestAvailable;UI.main.querySelector("[data-dkl-mod-repair]").onclick=()=>{d.inventory=d.inventory.map(item=>({...createItem(d.level,item.rarity,item.slot,item.classId),uid:item.uid,acquiredAt:item.acquiredAt||Date.now()}));refreshSessionLoadout();safeSave();renderDungeonModMenu();};UI.main.querySelectorAll("[data-dkl-mod-material]").forEach(btn=>btn.onclick=()=>{addMaterial(d,btn.dataset.dklModMaterial,100);safeSave();renderDungeonModMenu();});UI.main.querySelector("[data-dkl-mod-profession]").onclick=()=>{ensureProfessionData(d);if(!d.profession.id)d.profession.id="blacksmith";d.profession.level=100;d.profession.xp=0;safeSave();renderDungeonModMenu();};}
  ...
  function chooseEnemyTarget(s, enemy) {
    ...
    if(tankPets.length&&Math.random()<.72)return tankPets.sort((a,b)=>distance(enemy,a)-distance(enemy,b))[0];
