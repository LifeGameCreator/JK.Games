(() => {
  ...
  const VERSION = "20260804-dungeon-kl-v178-koop-damage-pet-host-leave-fix";
  const MAX_LEVEL = 100;
  ...
  function isDungeonAdminOrOwner(staff=currentStaffContext()){return !!staff?.active&&(staff.role==="admin"||staff.role==="owner");}
  function canUseDungeonModMenu(staff=currentStaffContext()){return isDungeonAdminOrOwner(staff);}
  function dungeonModCapabilities(staff=currentStaffContext()){const allowed=isDungeonAdminOrOwner(staff);return{profile:allowed,money:allowed,items:allowed,games:allowed,shop:allowed,owner:staff?.role==="owner"&&allowed};}
  function requireDungeonModCapability(_capability,message="Dieses Dungeon.KL-Mod-Menü ist nur für Admin und Owner freigeschaltet."){const staff=UI.dungeonVerifiedStaff||currentStaffContext();if(isDungeonAdminOrOwner(staff))return true;toast("Keine Berechtigung",message);return false;}
  async function verifyDungeonStaffRole(){if(!isDungeonAdminOrOwner(currentStaffContext()))return null;try{const {fb,user}=await firebase(),snap=await fb.getDoc(fb.doc(fb.db,"staffRoles",user.uid));if(!snap.exists())return null;const data=snap.data()||{},staff=normalizedDungeonStaffContext({...data,active:data.active!==false});return isDungeonAdminOrOwner(staff)?staff:null;}catch(error){console.warn("Dungeon.KL Rollenprüfung",error);return null;}}
  async function openDungeonModMenu(){if(!isDungeonAdminOrOwner(currentStaffContext()))return toast("Kein Zugriff","Das Dungeon.KL-Mod-Menü ist ausschließlich für Admin und Owner sichtbar.");toast("Rolle wird geprüft","Firebase bestätigt Admin- oder Owner-Rechte.");const verified=await verifyDungeonStaffRole();if(!verified)return toast("Zugriff nicht bestätigt","Firebase konnte keine aktive Admin- oder Owner-Rolle bestätigen.");UI.dungeonVerifiedStaff=verified;renderDungeonModMenu();}
  ...
  function renderDungeonModMenu(){
    ...
    UI.main.innerHTML=`<div class="dkl-page dkl-mod-page"><header class="dkl-page-head"><button class="dkl-btn" data-dkl-mod-back>← Charakterfenster</button><div><small>ADMIN-/OWNER-WERKZEUGE</small><h2>Dungeon.KL Mod-Menü</h2><p>${esc(roleLabel)} · aktueller Charakter: ${esc(d.name)}</p></div></header><section class="dkl-mod-grid">${profilePanel}${itemPanel}${gamePanel}</section></div>`;
