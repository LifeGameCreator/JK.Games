(() => {
  ...
  function createRoomLayout(s, room) {
    ...
    if (variant===0 || kind==="catacomb") {
      ...
      ladder(700,560,100,90); entry={x:750,y:755}; exit={x:750,y:140}; bossPoint={x:750,y:245}; chestPoint={x:300,y:450};
      block(540,350,120,190,kind==="flooded"?"water":"pit"); block(840,350,120,190,kind==="flooded"?"water":"pit");
  ...
  function isPointInWalkableRaw(walkable,blocked,x,y,r=0){return walkable.some(z=>containsRect(z,x,y,r))&&!blocked.some(z=>intersectsRect(z,x,y,r));}
  function collectSpawnPoints(layout,count,rnd,entry,exit){const out=[];let guard=0;while(out.length<count&&guard++<500){const z=layout.walkable[Math.floor(rnd()*layout.walkable.length)],x=z.x+45+rnd()*Math.max(20,z.w-90),y=z.y+45+rnd()*Math.max(20,z.h-90);if(!isPointInWalkableRaw(layout.walkable,layout.blocked,x,y,28))continue;if(Math.hypot(x-entry.x,y-entry.y)<260||Math.hypot(x-exit.x,y-exit.y)<100)continue;if(layout.props.some(p=>p.r&&Math.hypot(x-p.x,y-p.y)<p.r+40))continue;out.push({x,y});}return out;}
  ...
  function buildRoom(s, room) {
    s.room = room; s.enemies = []; s.projectiles = []; s.effects = []; s.traps = []; s.chests = []; s.boss = null; s.bossTelegraph = null; s.roomState = "combat"; s.clearAt = 0; s.exitOpen = false; s.roomExitOpen = false; s.roomTransitionLock = performance.now() + 1000;
  ...
  function updateEffects(s, dt) { for (const e of s.effects) e.life -= dt; for (const t of s.texts) { t.life -= dt; t.y -= 35 * dt; } s.effects = s.effects.filter(e => e.life > 0); s.texts = s.texts.filter(t => t.life > 0); for (const p of s.players) { p.attackAnim=Math.max(0,(p.attackAnim||0)-dt); for (const key of Object.keys(p.buffs)) p.buffs[key] = Math.max(0, p.buffs[key] - dt); } }
  ...
  function updateNetwork(s, now) {
    ...
    if (now - s.networkLastWrite > 90) {
      ...
      safeSetDoc(fb, s.selfRef, {
        ...
        wantAttack: !!s.autoAttack, skillSeq: Math.max(0, Math.floor(finite(s.player.skillSeq, 0))),
        skillRequested: Math.max(0, Math.floor(finite(s.player.skillRequested, 0))), role: String(s.player.role || "dps"),
    ...
    if (s.host && now - s.networkLastWorld > 115) {
      s.networkLastWorld = now;
      safeSetDoc(fb, s.worldRef, worldSnapshot(s), { merge: false }, "Weltstatus konnte nicht geschrieben werden");
    }
  }
  function applyRemoteInput(id, data) { const s = UI.session, p = s?.remotePlayers.get(id); if (!p) return; const ox=p.x,oy=p.y,tx=Number(data.x||p.x),ty=Number(data.y||p.y);tryMoveEntity(s,p,(tx-p.x)*.55,0);tryMoveEntity(s,p,0,(ty-p.y)*.55);p.moving=Math.hypot(p.x-ox,p.y-oy)>.5;if(p.moving)p.walkPhase=(p.walkPhase||0)+.18; p.angle = Number(data.angle || p.angle); p.wantAttack = !!data.wantAttack; if (Number(data.skillSeq || 0) > Number(p.lastSkillSeq || 0)) { p.lastSkillSeq = Number(data.skillSeq); p.skillRequested = Number(data.skillRequested || 0); } }
  function worldSnapshot(s) {
    return firestoreSafe({
      ...
      players: s.players.filter(Boolean).map(player => ({
        ...
        angle: finite(player.angle, 0), role: String(player.role || "dps"),
  ...
  function drawSession(s, now) {
    const ctx = s.ctx; if (!ctx) return; ctx.clearRect(0, 0, CANVAS_W, CANVAS_H); drawDungeonGround(ctx, s, now); drawDungeonProps(ctx,s,now,"back"); drawTraps(ctx, s, now); drawChests(ctx, s); drawDungeonDoor(ctx,s,now); if (s.bossTelegraph) drawTelegraph(ctx, s, s.bossTelegraph); for (const e of s.effects) drawEffect(ctx, s, e); for (const b of s.projectiles) drawProjectile(ctx, s, b);
  ...
  function drawPlayer(ctx,s,p,now){const x=sx(s,p.x),y=sy(s,p.y),c=CLASSES[p.classId]||CLASSES.berserker,r=ROLES[p.role]||ROLES.dps,phase=p.moving?Math.sin((p.walkPhase||now*.012))*5:0,bob=p.moving?Math.abs(Math.sin((p.walkPhase||now*.012)))*2:0,side=Math.cos(p.angle)>=0?1:-1,front=Math.sin(p.angle)>.35,back=Math.sin(p.angle)<-.35,attack=(p.attackAnim||0)>.01;ctx.save();ctx.translate(x,y);ctx.globalAlpha=p.dead?.35:1;
    ctx.fillStyle="#0008";ctx.beginPath();ctx.ellipse(0,5,25,9,0,0,Math.PI*2);ctx.fill();if(p.climbing){ctx.strokeStyle="#d6a267";ctx.lineWidth=7;ctx.lineCap="round";ctx.beginPath();ctx.moveTo(-8,-30);ctx.lineTo(-13+phase,-4);ctx.moveTo(8,-30);ctx.lineTo(13-phase,-4);ctx.moveTo(-7,-2);ctx.lineTo(-10-phase,15);ctx.moveTo(7,-2);ctx.lineTo(10+phase,15);ctx.stroke();}
  ...
  function drawPlayerWeapon(ctx,p,c,torsoY,side,attack){ctx.save();ctx.translate(side*19,torsoY+4);const swing=attack?(side>0?-.7:.7):0;if(p.role==="tank"){ctx.fillStyle="#496a7c";ctx.strokeStyle="#8fe8ff";ctx.lineWidth=3;ctx.beginPath();ctx.ellipse(-side*34,3,14,20,0,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.rotate(swing);ctx.strokeStyle="#edf5fb";ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(0,-24);ctx.lineTo(side*11,24);ctx.stroke();ctx.strokeStyle="#9a6b3f";ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(-side*3,9);ctx.lineTo(side*7,13);ctx.stroke();}
    ...
    else if(c.projectile){ctx.rotate(swing*.5);ctx.strokeStyle=c.color;ctx.shadowColor=c.color;ctx.shadowBlur=12;ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(0,-22);ctx.lineTo(0,25);ctx.stroke();ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(0,-25,6,0,Math.PI*2);ctx.fill();}
