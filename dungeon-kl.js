(() => {
  ...
  function open(phoneItem = "") {
    ...
    UI.overlay.innerHTML = `<section class="dkl-shell"><header class="dkl-head"><div class="dkl-brand"><span>◈</span><div><small>PARTY DUNGEON RPG</small><h2>DUNGEON<span>.KL</span></h2></div></div><div class="dkl-head-stats" data-dkl-head></div><div class="dkl-head-actions"><button class="jkc-game-inline-button" data-dkl-jkcoin title="JK/Coin öffnen">JK</button><button data-dkl-home title="Hauptlobby">⌂</button><button data-dkl-close title="Zurück">×</button></div></header><main class="dkl-main" data-dkl-main></main><div class="dkl-toast" data-dkl-toast></div></section>`;
    ...
    UI.overlay.querySelector("[data-dkl-jkcoin]")?.addEventListener("click", () => window.JKCoinApp?.openForGame?.("dungeon"));
  ...
  function interactNearby(){
    ...
    addItem(item,true);d.gold+=gold;d.stats.chests++;const jkWon=window.JKCoinApp?.rollTopGameDrop?.("dungeon","chest",`dungeon-chest:${s.runId}:${chest.id}`)||0;if(jkWon)showMessage(`JK/COIN DROP · +${jkWon}`,2200);const materialKey=pick(Object.keys(MATERIALS));addMaterial(d,materialKey,Math.max(1,Math.floor(rand(1,4))));if(d.profession.id==="companion"&&Math.random()<.22){const species=pick(Object.keys(COMPANION_SPECIES));addCompanionCopy(d,{species,role:pick(["tank","dps","healer"]),rarity:rarityForLevel(level,8),name:COMPANION_SPECIES[species].name});}safeSave();
  ...
  function killEnemy(s, enemy) { enemy.dead = true;spawnGroundLoot(s,enemy); if(s.selectedEnemyId===enemy.id){s.selectedEnemyId="";s.autoAttack=false;} ensureState().stats.kills++; pushCombatLog(s,`${enemy.name} besiegt${enemy.boss?" · Bossbeute!":""}`); if (enemy.boss) { ensureState().stats.bosses++; rewardBoss(s, enemy); window.JKCoinApp?.addFragments?.(10,`Dungeon.KL Boss · ${enemy.name}`,`dungeon-boss:${s.runId}:${enemy.id}`); } }
