(() => {
  ...
  const VERSION = "20260802-dungeon-kl-v158-enemy-vision-chests-map-fix";
  const MAX_LEVEL = 100;
  ...
  function createRoomLayout(s, room) {
    ...
    const layout={kind,walkable,blocked,ladders,props,raised,entry,exit,bossPoint,chestPoint,chestSpawns,palette:dungeonPalette(s.dungeon.theme),tile:48,seed:seedValue};
  ...
  function enemySightRange(enemy){return enemy.boss?720:enemy.elite?560:enemy.ranged?520:ENEMY_SIGHT_BASE;}
  ...
  function chooseEnemyTarget(s, enemy) {
    ...
    const tanks=visible.filter(p=>p.role==="tank");
    if(tanks.length&&Math.random()<.84)return tanks.sort((a,b)=>distance(enemy,a)-distance(enemy,b))[0];
    return visible.sort((a,b)=>distance(enemy,a)/(ROLES[a.role].aggro||1)-distance(enemy,b)/(ROLES[b.role].aggro||1))[0];
  }
  ...
  function onKeyDown(e){if(!UI.overlay)return;UI.keys[e.code]=true;if(UI.session){if(["Digit1","Digit2","Digit3","Digit4","Digit5"].includes(e.code)){e.preventDefault();useSkill(Number(e.code.slice(-1))-1);}if(e.code==="Space"){e.preventDefault();toggleAutoAttack();}if(e.code==="KeyE"){e.preventDefault();interactNearby();}if(e.code==="Escape"){e.preventDefault();showExitDialog();}}else if(e.code==="Escape"){e.preventDefault();returnToTopGames();}}
