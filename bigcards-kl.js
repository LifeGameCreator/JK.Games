/* BigCards.kl – JK.Games Top Game V406 */
(() => {
  "use strict";

  const VERSION = "2026-08-11-bigcards-v406-hyper-combat-generation";
  ...
  const HYPER_GENERATION_PRODUCTION_MULT = Object.freeze([1,.34,.37,.40,.44]);
  ...
  function hyperPackPointPrice(){
    const r=Math.max(0,Math.floor(Number(S?.totalRebirths)||0)),fixed=[100000,750000,4000000,20000000,100000000];
    if(r<fixed.length)return fixed[r];
    return Math.round(fixed.at(-1)*Math.pow(5,r-4));
  }
  ...
  function hyperProductionFactor(inst){return HYPER_GENERATION_PRODUCTION_MULT[hyperGeneration(inst)]||.34;}
  ...
  async function openHyperPack(currency="points"){
    ...
    if(currency==="jk"){
      if(!await confirmJkPackPurchase("HYPER PACK · 5 Kampfkarten",HYPER_JK_PRICE))return;
    ...
    const results=Array.from({length:5},()=>rollHyperCard());let coreFound=false;
  ...
  function upgradeCost(inst,next){const base=(inst.exclusive||inst.vip)?Math.max(100,inst.basePower):inst.hyper?Math.max(100,cardBaseProduction(inst.rarity,inst.base)*(hyperCardBy(inst.hyperId)?.powerFactor||1)):cardBaseProduction(inst.rarity,inst.base);const rarity=inst.exclusive?12:inst.rarity;return Math.max(100,Math.floor(base*(8+rarity*2)*Math.pow(next,2.25)));}
  ...
  function setPointBonusMultiplier(inst){const ids=setMembershipIds(inst);let bonus=0;for(const id of ids){if(inst?.hyper&&id==="hyper")continue;bonus+=setProgress(id).bonus;}return 1+Math.min(.08,bonus);}
