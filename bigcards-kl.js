/* BigCards.kl – JK.Games Top Game V346 */
(() => {
  "use strict";

  const VERSION = "2026-08-09-bigcards-v346-potions-hit-animations";
  ...
  function cardEffectBadges(inst){
    if(!inst)return"";const aura=auraBy(inst.aura),combat=combatAuraBy(inst.combatAura),potion=preparedPotionMeta(inst);if(!aura&&!combat&&!potion)return"";
    return `<span class="bc-card-effect-badges">${aura?`<span class="bc-mini-effect aura" title="${esc(aura.name)}">${aura.icon}</span>`:""}${combat?`<span class="bc-mini-effect combat" title="${esc(combat.name)}"><span class="bc-cross-weapons"><i class="blade">🗡</i><i class="axe">🪓</i></span><small>⚔</small></span>`:""}${potion?`<span class="bc-mini-effect potion" title="${esc(potion.name)} · ${esc(potion.effectText)}">🧪</span>`:""}</span>`;
  }
