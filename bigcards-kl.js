/* BigCards.kl – JK.Games Top Game V412 · Complete Prestige */
(() => {
  "use strict";

  const VERSION = "2026-08-12-bigcards-v412-complete-prestige";
...
Hyper-Kerne werden für die hohen Hyper-Generationen benötigt.`,confirmText:"Hyper-Kern herstellen",icon:"⚡",tone:"danger"}))return;
  ...
  const TUTORIAL_LABELS = Object.freeze({
    ...
    fusion:["Fusion","🔬"],prestige:["Prestige & Prestige-Shop","◆"],shop:["Shop & Ausrüstung","🛒"],market:["Markt","🏪"],score:["Beste Spieler","🏆"]
  });
  ...
  function refresh(preserve=true){if(!UI.overlay)return;if(preserve)rememberViewScroll();refreshHeader();renderNav();renderMain();applyPrestigeCosmetics();restoreViewScroll(preserve);}
  function renderMain(){const el=UI.main||UI.overlay?.querySelector("[data-bc-main]");if(!el)return;UI.main=el;if(UI.tab==="mod"&&UI.role!=="owner")UI.tab="field";if(UI.tab==="field")el.innerHTML=fieldHtml();else if(UI.tab==="packs")el.innerHTML=packsHtml();else if(UI.tab==="collection")el.innerHTML=collectionHtml();else if(UI.tab==="shop")el.innerHTML=shopHtml();else if(UI.tab==="market")el.innerHTML=marketHtml();else if(UI.tab==="score")el.innerHTML=scoreHtml();else if(UI.tab==="battle")el.innerHTML=battleHtml();else if(UI.tab==="expedition")el.innerHTML=expeditionHtml();else if(UI.tab==="boss")el.innerHTML=bossHtml();else if(UI.tab==="sets")el.innerHTML=setsHtml();else if(UI.tab==="fusion")el.innerHTML=fusionHtml();else if(UI.tab==="prestige")el.innerHTML=prestigeHtml();else if(UI.tab==="card")el.innerHTML=featureCardHtml();else if(UI.tab==="tutorial")el.innerHTML=tutorialHtml();else if(UI.tab==="vipWheel")el.innerHTML=vipWheelHtml();else if(UI.tab==="vipClicker")el.innerHTML=vipClickerHtml();else if(UI.tab==="mod"&&UI.role==="owner")el.innerHTML=modHtml();else el.innerHTML=fieldHtml();}
  ...
  async function syncProfile(force=false){
    ...
    try{
      updateFeaturedEarnings(now());const savedAt=now();S.updatedAt=savedAt;S.version=412;
