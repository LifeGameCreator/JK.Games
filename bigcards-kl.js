...
(() => {
  ...
  const HYPER_POINTS_NERF = .50;
  function hyperEconomyBase(inst){
    const hm=hyperCardBy(inst?.hyperId)||HYPER_CARDS[0];
    return cardBaseProduction(HYPER_ECONOMY_RARITY_INDEX,inst?.base||0)*(Number(hm?.powerFactor)||1)*hyperProductionFactor(inst)*HYPER_POINTS_NERF;
  }
