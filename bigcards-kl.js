/* BigCards.kl – JK.Games Top Game V372 */
(() => {
  "use strict";

  const VERSION = "2026-08-10-bigcards-v372-trail-info";
  const SAVE_KEY = "jk-games-bigcards-kl-v332";
  const CLOUD_SAVE_COLLECTION = "bigCardsSaves";
  const CLOUD_SCHEMA_VERSION = 371;
  ...
  function adoptState(raw,{saveLocal=false,userId=""}={}){
    ...
    S.version=371;const repaired=normalizeFloorUniqueCards();updateFeaturedEarnings(now(),false);if(saveLocal||repaired||potionInventoryMigrated)writeLocalState(userId||cloudUid||currentUidSync());return S;
  }
  function state(){
    if(S)return S;
  ...
  window.BigCardsKL=Object.freeze({version:VERSION,open,close,returnToTopGames,grantJkCoinPurchase,getState:()=>state(),getFeaturedStorageTier:()=>Math.floor(Number(state().featuredStorageTier)||0),hasBulkLevelUnlock:()=>!!state().bulkLevelUnlocked,getJkBoosterState:(kind)=>{state();return jkBoosterState(kind)},featuredStorageTiers:FEATURED_STORAGE_TIERS,featuredRanks:FEATURED_RANKS,rarities:RARITIES,trails:TRAILS,baseNames:BASE_NAMES});
})();
