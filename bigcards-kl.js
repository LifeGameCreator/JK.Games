/* BigCards.kl – JK.Games Top Game V387 */
(() => {
  "use strict";

  const VERSION = "2026-08-10-bigcards-v387-confirm-market-cloud";
  ...
  function collectionTierScore(tier){
    if(tier===14){const max=VIP_CARDS.length*5;if(!max)return 0;let got=0;for(const vm of VIP_CARDS){const row=S.vipCollection?.[`v:${vm.id}`];got+=clamp(Number(row?.highestLevel)||0,0,5);}return clamp(got/max*100,0,100);}
    if(tier===13){
  ...
  function overallCollectionScore(){
    // V381: Hauptscore ist strikt Free-to-Play. Nur die 13 normalen Serien von
    // Gewöhnlich bis Göttlich zählen hier hinein; Exclusive und VIP niemals.
    const totalVariants=RARITIES.length*BASE_NAMES.length;if(!totalVariants)return 0;let earned=0;
  ...
  function payCollectionScore(){
    const totalVariants=EXCLUSIVES.length+VIP_CARDS.length;if(!totalVariants)return 0;
    const earned=collectionTierScore(13)*(EXCLUSIVES.length/100)+collectionTierScore(14)*(VIP_CARDS.length/100);
    return clamp(earned/totalVariants*100,0,100);
  }
  ...
  function collectionHtml(){
    const tier=UI.collectionTier,vip=tier===14,exclusive=tier===13,search=UI.collectionSearch.trim().toLowerCase(),ownedByKey=new Map();
    for(const inst of Object.values(S.instances)){
      if(vip?!inst.vip:exclusive?!inst.exclusive:(inst.vip||inst.exclusive||inst.rarity!==tier))continue;
  ...
  async function syncProfile(force=false){
    ...
    try{
      updateFeaturedEarnings(now());const savedAt=now();S.updatedAt=savedAt;S.version=387;
