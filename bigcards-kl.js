/* BigCards.kl – JK.Games Top Game V347 */
(() => {
  "use strict";
  ...
  function state(){
    ...
    S.version=347;const repaired=normalizeFloorUniqueCards();updateFeaturedEarnings(now());if(repaired){try{localStorage.setItem(SAVE_KEY,JSON.stringify(S));}catch{}}return S;
  }
  ...
  function equipPotion(id,typeId){const inst=instance(id),type=POTION_TYPES.find(x=>x.id===typeId);if(!inst||!type)return;if(inst.listed)return toast("Gelistete Karten sind gesperrt.");const tierId=potionTierId(inst),owned=potionCount(tierId,typeId);if(owned<1)return toast(`Du besitzt keinen passenden ${type.name} für ${tierId==="exclusive"?"Exclusive":RARITIES[potionTierIndex(tierId)].name}.`);if(inst.battlePotion)returnPreparedPotion(inst);S.potionInventory[potionKey(tierId,typeId)]=owned-1;inst.battlePotion={tierId,typeId};persist();showCardDetail(id);refresh();toast(`${type.name} vorbereitet · ${potionEffectText(tierId,typeId)} im nächsten Kampf.`);}
  function removePotion(id){const inst=instance(id);if(!inst?.battlePotion)return;const p=preparedPotionMeta(inst);returnPreparedPotion(inst);persist();showCardDetail(id);refresh();toast(`${p?.name||"Trank"} zurück ins Inventar gelegt.`);}
