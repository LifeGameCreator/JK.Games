/* BigCards.kl – JK.Games Top Game V411 · Progression + Tutorial */
(() => {
  "use strict";

  const VERSION = "2026-08-12-bigcards-v411-progression-tutorial";
  ...
  const TRAILS = Object.freeze([
    {id:"common",name:"Gewöhnliche Spur",icon:"〰️",pointPrice:5000,jk:25,levelReq:1,unlockCost:0,points:.03,xp:.05,damage:.04,hp:.04},
    {id:"uncommon",name:"Ungewöhnliche Spur",icon:"🍃",pointPrice:20000,jk:50,levelReq:5,unlockCost:50000,points:.05,xp:.10,damage:.07,hp:.06},
    {id:"rare",name:"Seltene Spur",icon:"💧",pointPrice:75000,jk:90,levelReq:10,unlockCost:250000,points:.08,xp:.16,damage:.11,hp:.09},
    {id:"epic",name:"Epische Spur",icon:"🔮",pointPrice:200000,jk:150,levelReq:15,unlockCost:1000000,points:.12,xp:.24,damage:.16,hp:.13},
    {id:"legendary",name:"Legendäre Spur",icon:"✨",pointPrice:500000,jk:250,levelReq:20,unlockCost:4000000,points:.16,xp:.35,damage:.22,hp:.18},
    {id:"special",name:"Special Spur",icon:"🔻",pointPrice:1200000,jk:400,levelReq:25,unlockCost:12000000,points:.2,xp:.50,damage:.30,hp:.24},
    {id:"mythic",name:"Mythische Spur",icon:"🌟",pointPrice:3000000,jk:650,levelReq:30,unlockCost:35000000,points:.25,xp:.70,damage:.40,hp:.32},
    {id:"exotic",name:"Exotische Spur",icon:"💠",pointPrice:8000000,jk:1000,levelReq:40,unlockCost:90000000,points:.3,xp:.95,damage:.52,hp:.42},
    {id:"universe",name:"Universe Spur",icon:"🌌",pointPrice:20000000,jk:1500,levelReq:50,unlockCost:250000000,points:.35,xp:1.30,damage:.66,hp:.54},
    {id:"blackhole",name:"Black Hole Spur",icon:"⚫",pointPrice:50000000,jk:2200,levelReq:60,unlockCost:650000000,points:.4,xp:1.75,damage:.82,hp:.68},
    {id:"galaxy",name:"Galaxy Spur",icon:"🌠",pointPrice:120000000,jk:3200,levelReq:75,unlockCost:1500000000,points:.45,xp:2.30,damage:1.00,hp:.85},
    {id:"cosmic",name:"Kosmische Spur",icon:"🔥",pointPrice:280000000,jk:4500,levelReq:90,unlockCost:3500000000,points:.5,xp:3.00,damage:1.22,hp:1.05},
    {id:"godly",name:"Göttliche Spur",icon:"👑",pointPrice:650000000,jk:6500,levelReq:100,unlockCost:8000000000,points:.6,xp:4.00,damage:1.50,hp:1.30}
  ...
  function adoptState(raw,{saveLocal=false,userId=""}={}){invalidateCollectionRenderCache();
    ...
    for(const inst of Object.values(S.instances)){
      ...
      const key=collectionKey(inst),col=cardCollectionMap(inst),scoreLevel=inst.cardRebirth>0?5:clamp(Math.floor(Number(inst.level)||1),1,5);col[key]=col[key]||{firstAt:Number(inst.createdAt)||now(),highestLevel:scoreLevel};col[key].highestLevel=Math.max(Number(col[key].highestLevel)||1,scoreLevel);
    }
  ...
  function hyperPackPointPrice(){
    const r=Math.max(0,Math.floor(Number(S?.totalRebirths)||0)),fixed=[250000,2000000,30000000,120000000,450000000,1200000000];
    if(r<fixed.length)return fixed[r];
    return Math.round(fixed.at(-1)*Math.pow(1.75,r-5));
  }
  ...
  function cardBaseXp(inst){const premium=!!(inst&&(inst.exclusive||inst.vip)),p=inst?.hyper?cardBaseProduction(inst.rarity,inst.base)*(hyperCardBy(inst.hyperId)?.powerFactor||1)*.38:premium?Math.max(1,inst.basePower||1)*PREMIUM_V393_STRENGTH:(inst?.win?cardBaseProduction(inst.rarity,inst.base)*(winCardBy(inst.winId)?.powerFactor||1):cardBaseProduction(inst.rarity,inst.base)),factor=inst?.exclusive?1:(inst?.vip?0.82:(inst?.rarity+1)*.08);return Math.max(.25,Math.sqrt(p)*(.55+factor));}
  ...
  function effectiveXp(inst){if(!inst)return 0;let value=cardBaseXp(inst)*(1+(inst.level-1)*.14)*cardRebirthTotalMultiplier(inst)*xpBoosterMultiplier()*(inst.hyper?(.30+hyperGeneration(inst)*.025):1);if(inst.bind)value*=bindBy(inst.bind)?.mult||1;if(inst.shiny>=1)value*=1.10;value*=Math.min(5,Math.sqrt(rebirthMultiplier()));return value;}
  ...
  function collectionScoreSnapshot(){
    ...
    let gotHyper=0;for(const hm of HYPER_CARDS){const row=S.hyperCollection?.[`h:${hm.id}`];gotHyper+=clamp(Number(row?.highestLevel)||0,0,5);}
    ...
    collectionScoreCache={tiers,overall,pay,wins:tiers[15],hyper:tiers[16]};
  ...
  async function craftHyperCore(){
    if(S.winsCurrency<HYPER_CORE_CRAFT_WINS)return toast(`Du brauchst ${HYPER_CORE_CRAFT_WINS} Wins.`);
    if(S.fusionDust<HYPER_CORE_CRAFT_DUST)return toast(`Du brauchst ${fmt(HYPER_CORE_CRAFT_DUST)} Fusionsstaub.`);
    if(!await gameConfirm({title:"⚡ Hyper-Kern herstellen",message:`${HYPER_CORE_CRAFT_WINS} Wins + ${fmt(HYPER_CORE_CRAFT_DUST)} Fusionsstaub\n\nHyper-Kerne werden für die hohen Hyper-Generationen benötigt.`,confirmText:"Hyper-Kern herstellen",icon:"⚡",tone:"danger"}))return;
    S.winsCurrency-=HYPER_CORE_CRAFT_WINS;S.fusionDust-=HYPER_CORE_CRAFT_DUST;S.hyperCores=(S.hyperCores||0)+1;persist();refresh(true);toast(`⚡ Hyper-Kern hergestellt · Bestand ${S.hyperCores}`);
  }
  ...
  async function upgradeHyperGeneration(id){
    ...
    S.winsCurrency-=cost.wins;S.fusionDust-=cost.dust;S.hyperCores=Math.max(0,(S.hyperCores||0)-cost.cores);inst.hyperGeneration=next;invalidateCardPowerCache();persist();refresh(false);showCardDetail(id);toast(`⚡ ${cardMeta(inst).name} ist jetzt Hyper Generation ${next}.`,5000);
  }
  ...
  async function bulkRebirthCurrentTier(){
    ...
    let applied=0;for(const id of plan.cards){const inst=instance(id);if(!inst||inst.listed||inst.broken||inst.level<5||cardRebirth(inst)>=CARD_REBIRTH_MAX)continue;const key=collectionKey(inst),col=cardCollectionMap(inst);col[key]=col[key]||{firstAt:Number(inst.createdAt)||now(),highestLevel:5};col[key].highestLevel=Math.max(5,Number(col[key].highestLevel)||0);inst.cardRebirth=cardRebirth(inst)+1;inst.level=1;applied++;}
    if(applied)invalidateCardPowerCache();
  ...
  function expeditionRawXpRate(inst){if(!inst)return 0;let v=cardBaseXp(inst)*(1+(inst.level-1)*.14)*cardRebirthTotalMultiplier(inst)*(inst.hyper?(.30+hyperGeneration(inst)*.025):1);if(inst.bind)v*=Math.min(1.15,bindBy(inst.bind)?.mult||1);return Math.max(1,v);}
  ...
  function fusionDustCost(inst,target){const tier=inst.hyper?combatTier(inst):inst.exclusive||inst.vip?premiumReferenceTier():clamp(inst.rarity,0,12),scarcity=Math.max(.8,Math.min(2.5,Math.sqrt(10/Math.max(.1,cardMeta(inst).rarityValue))));return Math.ceil((FUSION_DUST_BASE*(target==="prismatic"?9:1)*(1+tier*.32)*scarcity)/10)*10;}
  ...
  const TUTORIAL_LABELS = Object.freeze({
    ...
    fusion:["Fusion","🔬"],shop:["Shop & Ausrüstung","🛒"],market:["Markt","🏪"],score:["Beste Spieler","🏆"]
  });
  function tutorialSteps(key){
    const defs={
      field:[
        ...
        ["field",".bc-level-panel","Level & XP","Hier läuft dein BigCards-Level. XP wurde durch das neue Point-Balancing nicht verändert."],
      ...
      collection:[
        ["collection",".bc-collection-score-row","Sammlungs-Scores","F2P-Sammlung und Pay-Sammlung werden getrennt angezeigt."],
      ...
      card:[
        ...
        ["card",".bc-feature-top-actions","Karte & Rank","Hier wechselst du die persönliche Karte und öffnest den Rank-Bereich. Bei Hyper beeinflusst Rank die Points nicht, sondern den Kampf."],
        ["card",".bc-card-trail-section","Spuren","Spuren gehören zur persönlichen Haupt- oder Backup-Karte und geben eigene Points-, XP- und Kampfboni."],
      ...
      boss:[
        ...
        ["boss",".bc-boss-attack","Tagesangriffe","Du hast genau 3 Gratis-Angriffe pro Tag und maximal 2 Bonus-Angriffe: einen über 5 Packs und einen über einen Kartenkampf-Sieg."],
  ...
  function showSectionInfo(id){
    const info={
      ...
      collection:["Sammlung","Hier verwaltest du normale, Wins-, Exclusive-, VIP- und Hyper-Karten. Karten-Rebirth-Ränder: RB1 Cyan, RB2 Violett, RB3 Gold, RB4 Pink/Rot, RB5 Regenbogen."],
      ...
      tutorial:["Tutorial","Jeder Spielbereich besitzt ein eigenes Schritt-für-Schritt-Tutorial. Gelbe Umrandungen zeigen immer genau das Element, das gerade erklärt wird."],
  ...
  function navItems(){const items=[["field","Spielfeld","🏗️","core"],["packs","Packs","🎴","core"],["collection","Sammlung","📚","core"],["card","Karte","🃏","core"],["battle","Kartenkampf","⚔️","play"],["expedition","Expedition","🧭","play"],["boss","Wochenboss","👹","play"],["sets","Sets","🧩","collect"],["fusion","Fusion","🔬","collect"],["shop","Shop","🛒","trade"],["market","Markt","🏪","trade"],["score","Beste Spieler","🏆","trade"],["tutorial","Tutorial","🎓","help"]];if(UI.role==="owner")items.push(["mod","Mod","🛠️","staff"]);return items;}
  ...
  function refreshHeader(){const el=UI.overlay?.querySelector("[data-bc-head-stats]");if(!el)return;const need=xpNeed(S.level),prod=productionPerSecond();el.innerHTML=`<div><small>POINTS</small><b>${fmt(S.points)}</b></div><div><small>PRODUKTION</small><b>+${fmt(prod)}/s</b></div><div><small>LEVEL</small><b>${S.level}</b><em>${Math.floor(S.xp)}/${fmt(need)} XP</em></div><div><small>REBIRTH</small><b>${S.totalRebirths} · Points ×${accountPointRebirthMultiplier().toFixed(2).replace(".",",")}</b></div><div><small>SAMMLUNG</small><b>${scorePct(overallCollectionScore())}</b></div>`}
  ...
  function renderMain(){const el=UI.main||UI.overlay?.querySelector("[data-bc-main]");if(!el)return;UI.main=el;if(UI.tab==="mod"&&UI.role!=="owner")UI.tab="field";if(UI.tab==="field")el.innerHTML=fieldHtml();else if(UI.tab==="packs")el.innerHTML=packsHtml();else if(UI.tab==="collection")el.innerHTML=collectionHtml();else if(UI.tab==="shop")el.innerHTML=shopHtml();else if(UI.tab==="market")el.innerHTML=marketHtml();else if(UI.tab==="score")el.innerHTML=scoreHtml();else if(UI.tab==="battle")el.innerHTML=battleHtml();else if(UI.tab==="expedition")el.innerHTML=expeditionHtml();else if(UI.tab==="boss")el.innerHTML=bossHtml();else if(UI.tab==="sets")el.innerHTML=setsHtml();else if(UI.tab==="fusion")el.innerHTML=fusionHtml();else if(UI.tab==="card")el.innerHTML=featureCardHtml();else if(UI.tab==="tutorial")el.innerHTML=tutorialHtml();else if(UI.tab==="vipWheel")el.innerHTML=vipWheelHtml();else if(UI.tab==="vipClicker")el.innerHTML=vipClickerHtml();else if(UI.tab==="mod"&&UI.role==="owner")el.innerHTML=modHtml();else el.innerHTML=fieldHtml();}
  ...
  async function syncProfile(force=false){
    ...
    try{
      updateFeaturedEarnings(now());const savedAt=now();S.updatedAt=savedAt;S.version=411;
