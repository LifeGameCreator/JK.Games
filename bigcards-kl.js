/* BigCards.kl – JK.Games Top Game V383 */
(() => {
  "use strict";

  const VERSION = "2026-08-10-bigcards-v383-backup-break-cloud-throttle";
  ...
  const CLOUD_SAVE_DELAY_MS = 120000;
  const CLOUD_PASSIVE_SAVE_DELAY_MS = 600000;
  const CLOUD_PROFILE_SAVE_INTERVAL_MS = 1200000;
  const CLOUD_RESOURCE_BACKOFF_MS = 900000;
  ...
  function bulkRebirthCurrentTier(){
    ...
    if(!confirm(`Alle Karten rebirthen · ${bulkLevelTierLabel()}\n\n${plan.count} Karten werden jeweils genau 1× rebirtht.\nJede betroffene Karte fällt auf Level 1 zurück.\nDer Sammlungs-Score bleibt auf dem bereits erreichten Level-5-Stand.\nAusrüstung, Shiny, Spur, Rank, persönlicher Slot und Backup-Zuordnung bleiben erhalten.`))return;
