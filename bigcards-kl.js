...
(() => {
  ...
  function combatStats(inst){
    ...
    const exclusiveBalance=inst.exclusive?.95:1,vipCombatBalance=inst.vip?.94:1,vipWheelDamage=vipWheelMultiplier("damage"); // V375: kleines Combat-only Tuning; Produktion/Dropchance bleiben unverändert.
    const coreMin=Math.max(1,Math.floor(range.min*quality*levelMult*shinyMult*auraMult*trailDamage*exclusiveBalance*vipCombatBalance*vipWheelDamage));
    const coreMax=Math.max(coreMin+1,Math.ceil(range.max*quality*levelMult*shinyMult*auraMult*trailDamage*exclusiveBalance*vipCombatBalance*vipWheelDamage));
  ...
  function navItems(){const items=[["field","Spielfeld"],["packs","Packs"],["collection","Sammlung"],["shop","Shop"],["market","Markt"],["score","Beste Spieler"],["battle","Kartenkampf"],["card","Karte"],["vipWheel","VIP Rad"],["vipClicker","VIP Klicker"]];if(UI.role==="owner")items.push(["mod","Mod"]);return items;}
