/* BigCards.kl – JK.Games Top Game V413 · Boss Live Sync + Combat/Trail UX */
(() => {
  "use strict";

  const VERSION = "2026-08-12-bigcards-v413-boss-sync-combat-trails";
...
Hyper-Kerne werden für die hohen Hyper-Generationen benötigt.`,confirmText:"Hyper-Kern herstellen",icon:"⚡",tone:"danger"}))return;
  ...
  function bestBattleCardCandidate(){
    const cards=battlePickerCards().filter(inst=>inst&&!inst.broken&&!inst.listed&&!isCardOnExpedition(inst.id)&&battleCardUnlocked(inst));
  ...
  async function chooseBestBattleCard(){
    if(!S.bestCombatAutoUnlocked){
      const ok=await gameConfirm({title:"⚔ Beste Kampfkarte dauerhaft freischalten",message:"Für 100 JK/Coin wird die automatische Auswahl der aktuell stärksten erlaubten Kampfkarte dauerhaft für deinen BigCards-Spielstand freigeschaltet. Die Funktion berücksichtigt Kampffreischaltungen, kaputte Karten und Expeditionen.",confirmText:"Für 100 JK/Coin freischalten",icon:"⚡"});
      if(!ok)return false;
      if(!window.JKCoinApp?.spend?.(100,"BigCards Beste Kampfkarte dauerhaft"))return toast("Du brauchst 100 JK/Coin.");
      S.bestCombatAutoUnlocked=true;persist();
      try{await syncProfile(true);}catch{}
      toast("⚡ Beste Kampfkarte dauerhaft freigeschaltet.",3600);
    }
  ...
  async function syncProfile(force=false){
    ...
    try{
      updateFeaturedEarnings(now());const savedAt=now();S.updatedAt=savedAt;S.version=413;
