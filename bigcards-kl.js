/* BigCards.kl – JK.Games Top Game V343 */
(() => {
  "use strict";

  const VERSION = "2026-08-09-bigcards-v343-info-jk-confirm-best-setup-mod-tests";
  ...
  function repairCost(inst){if(!inst)return 0;const st=combatStats(inst),base=(st.min+st.max)/2;return Math.max(250,Math.ceil((base*30+st.hp*5)*(1+(Math.max(1,inst.level)-1)*.16)/50)*50);}
  ...
  function performPlayerBattleAction(abilityId){
    ...
    session.turn="enemy";refresh(false);clearTimeout(UI.battleEnemyTimer);const sid=session.id;UI.battleEnemyTimer=setTimeout(()=>enemyBattleTurn(sid),520);
  }
  function enemyBattleTurn(sessionId){
    ...
    session.round++;session.turn="player";refresh(false);
  }
  function finishBattle(won){
    ...
    UI.battleSession=null;persist();refresh(false);toast(won?`Sieg! +${fmt(rewardPoints)} Points · +${rewardShards} Shards${firstWin?" · Basic Kampf Aura erhalten!":""}`:"Niederlage – deine Kampfkarte ist zerbrochen und muss in der Sammlung repariert werden.",4400);
  }
  function repairCard(id){const inst=instance(id);if(!inst||!inst.broken)return;const cost=repairCost(inst);if(S.points<cost)return toast(`Reparatur kostet ${fmt(cost)} Points.`);S.points-=cost;inst.broken=false;inst.brokenAt=0;persist();refresh(false);showCardDetail(id);toast(`${cardMeta(inst).name} repariert · -${fmt(cost)} Points`);}
  ...
  function battleHtml(){
    ...
    const arena=session?`<div class="bc-battle-arena active-fight">${fightPlayer}<div class="bc-battle-vs"><span>VS</span><small>${session.turn==="player"?"DU BIST DRAN":"GEGNER GREIFT AN …"}</small></div>${fightEnemy}</div><div class="bc-battle-command"><div><small>DEINE ATTACKEN</small><h3>${session.playerState.mustNormal?"Jetzt ist ein normaler Schlag Pflicht":"Attacke auswählen"}</h3><p>Special-Fähigkeiten werden mit jeder Kartenstufe stärker.</p></div><div class="bc-battle-ability-grid">${abilities.map(a=>{const locked=a.locked||(!a.normal&&player.level<(a.req||1))||session.turn!=="player";return `<button data-bc-battle-action="${a.id}" class="${a.normal?"normal":"special"}" ${locked?"disabled":""}><span>${a.icon}</span><b>${a.name}</b><small>${!a.normal&&player.level<(a.req||1)?`🔒 ab Stufe ${a.req}`:a.desc}</small></button>`}).join("")}</div><div class="bc-battle-live-log">${session.log.slice(-8).map(x=>`<span>${esc(x)}</span>`).join("")}</div></div>`:(selected?`<div class="bc-battle-arena">${playerCard}<div class="bc-battle-vs"><span>VS</span><small>Gegner wird erst nach „Kampf starten“ aufgedeckt</small></div><article class="bc-battle-fighter enemy hidden-enemy"><small>GEGNER</small><div class="bc-battle-icon">❓</div><h3>Noch unbekannt</h3><b>Zufällige Stärke</b><span>Der Gegner kann schwächer, ähnlich oder auch deutlich stärker sein.</span><em>Bei einer Niederlage zerbricht deine Karte.</em></article></div>`:`<div class="bc-empty-state"><span>⚔</span><h3>Keine Kampfkarte vorhanden</h3><p>Öffne ein Pack oder repariere deine kaputten Karten in der Sammlung.</p><button data-bc-tab="collection">Zur Sammlung</button></div>`);
