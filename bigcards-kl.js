/* BigCards.kl – JK.Games Top Game V388 */
(() => {
  "use strict";

  const VERSION = "2026-08-10-bigcards-v389-mod-fast";
  ...
  function battleCardUnlocked(inst){return !!inst&&(inst.exclusive||inst.vip||combatTier(inst)<=battleUnlockedTier());}
  ...
  async function openExclusivePack(currency="jk"){
    ...
    const results=Array.from({length:5},()=>rollExclusive());S.daily.opened=(S.daily.opened||0)+1;addXp(160);awardMainXp(8,"BigCards.kl Exclusive Pack",`bc-exclusive-${now()}`);showPackReveal(results,{name:"EXCLUSIVE PACK",exclusive:true,paidJk:100});persist();
  }
  ...
  function cardMeta(inst){if(!inst)return null;const combat=combatStats(inst);if(inst.vip){const vm=vipCardBy(inst.vipId)||VIP_CARDS[0],r=RARITIES[clamp(vm.rarity,0,12)]||RARITIES[0];return {name:vm.name,rarity:r,className:`rar-${r.id} rar-vip ${cardRebirthClass(inst)}`,icon:"👑",rarityValue:vm.rarityValue,points:effectivePoints(inst),xp:effectiveXp(inst),value:sellValue(inst),combat,vip:true}}if(inst.exclusive){const ex=EXCLUSIVES.find(x=>x.id===inst.exclusiveId)||EXCLUSIVES[0];return {name:ex.name,rarity:{id:"exclusive",name:"Exclusive"},className:`rar-exclusive ${cardRebirthClass(inst)}`,icon:"🩸",rarityValue:ex.rarityValue,points:effectivePoints(inst),xp:effectiveXp(inst),value:sellValue(inst),combat}}const r=RARITIES[inst.rarity];return {name:BASE_NAMES[inst.base],rarity:r,className:`rar-${r.id} ${cardRebirthClass(inst)}`,icon:motifIcon(BASE_NAMES[inst.base]),rarityValue:rarityValue(inst.base),points:effectivePoints(inst),xp:effectiveXp(inst),value:sellValue(inst),combat}}
  ...
  function cardEffectBadges(inst){
    if(!inst)return"";const aura=auraBy(inst.aura),combat=combatAuraBy(inst.combatAura),potion=preparedPotionMeta(inst),potionCountQueued=potionQueueCount(inst),trail=trailBy(inst.trail);if(!aura&&!combat&&!potion&&!trail)return"";
    return `<span class="bc-card-effect-badges">${aura?`<span class="bc-mini-effect aura" title="${esc(aura.name)}">${aura.icon}</span>`:""}${combat?`<span class="bc-mini-effect combat" title="${esc(combat.name)}"><span class="bc-cross-weapons"><i class="blade">🗡</i><i class="axe">🪓</i></span><small>⚔</small></span>`:""}${potion?`<span class="bc-mini-effect potion" title="${potionCountQueued} Trank-Runden vorbereitet · zuerst ${esc(potion.name)}">🧪<small>${potionCountQueued}</small></span>`:""}${trail?`<span class="bc-mini-effect trail" title="${esc(trail.name)}${S?.featuredCardId===inst.id?" · AKTIV":" · gespeichert"}">☄</span>`:""}${cardRebirthBadge(inst)}</span>`;
  }
  ...
  async function upgradeCard(id){const inst=instance(id);if(!inst||inst.level>=5)return;if(inst.listed)return toast("Gelistete Karten sind bis zum Ende des Angebots gesperrt.");const next=inst.level+1,dupes=duplicateIds(inst),need=DUPE_COST[next],cost=upgradeCost(inst,next);if(dupes.length<need)return toast(`Du brauchst ${need} zusätzliche Exemplare (${dupes.length} vorhanden).`);if(S.points<cost)return toast(`Upgrade kostet ${fmt(cost)} Points.`);if(!await gameConfirm({title:`${cardMeta(inst).name} verbessern`,message:`Auf Stufe ${next} upgraden?\n${need} Duplikate werden verbraucht\nKosten: ${fmt(cost)} Points`,confirmText:`${fmt(cost)} Points einsetzen`,icon:"⬆️",tone:"points"}))return;S.points-=cost;for(const rid of dupes.slice(0,need))delete S.instances[rid];inst.level=next;const key=collectionKey(inst),col=inst.vip?S.vipCollection:(inst.exclusive?S.exclusiveCollection:S.collection);col[key].highestLevel=Math.max(col[key].highestLevel||1,next);S.daily.upgraded=(S.daily.upgraded||0)+1;raiseScore(150*next*(inst.exclusive?3:inst.rarity+1));addXp(50*next);persist();showCardDetail(id);refresh();}
  ...
  async function rebirthCard(id){const inst=instance(id);if(!inst)return;if(inst.listed)return toast("Gelistete Karten können nicht rebirtht werden.");if(inst.broken)return toast("Repariere die Karte vor dem Rebirth.");const current=cardRebirth(inst);if(current>=CARD_REBIRTH_MAX)return toast("Diese Karte ist bereits auf Rebirth 5 MAX.");if(inst.level<5)return toast("Karten-Rebirth ist erst auf Level 5/5 möglich.");const next=current+1,bonus=CARD_REBIRTH_BONUS[next]||0,total=1+bonus;if(!await gameConfirm({title:`Karten-Rebirth ${next}/${CARD_REBIRTH_MAX}`,message:`${cardMeta(inst).name} wird auf Level 1 zurückgesetzt.\nSammlungs-Score bleibt auf Level-5-Stand.\nRebirth-Bonus: ×${String(bonus).replace(".",",")} → Gesamtwerte ×${String(total).replace(".",",")}\n\nFür Level 2–5 brauchst du danach wieder 2 / 5 / 12 / 25 Duplikate.`,confirmText:`Rebirth ${next} starten`,icon:"↻",tone:"danger"}))return;const key=collectionKey(inst),col=inst.vip?S.vipCollection:(inst.exclusive?S.exclusiveCollection:S.collection);col[key]=col[key]||{firstAt:now(),highestLevel:5};col[key].highestLevel=Math.max(5,Number(col[key].highestLevel)||0);inst.cardRebirth=next;inst.level=1;persist();refresh(false);showCardDetail(id);toast(next===CARD_REBIRTH_MAX?`🌈 ${cardMeta(inst).name}: REBIRTH 5 MAX! Rainbow-Umrandung freigeschaltet.`:`↻ ${cardMeta(inst).name}: Rebirth ${next} · ${cardRebirthLabel(inst)}`,5000);}
  ...
  async function sellCard(id){const inst=instance(id);if(!inst)return;if(id===S.featuredCardId)return toast("Wähle die Karte im Tab „Karte“ zuerst ab, bevor du sie verkaufst.");if(featuredCard()?.backupCardId===id)return toast("Entferne diese Karte zuerst im Rank-Menü als Backup-Karte.");if(inst.broken)return toast("Repariere die Karte vor dem Verkauf.");if(inst.favorite||inst.locked)return toast("Favorisierte/gesperrte Karten können nicht verkauft werden.");if(S.floors.flat().includes(id))return toast("Nimm die Karte zuerst vom Stockwerk.");if(inst.listed)return toast("Die Karte ist im Online-Marktplatz gelistet.");const m=cardMeta(inst),warning=(inst.aura||inst.combatAura||inst.bind)?"\nAura/Kampf-Aura/Bindung werden mitverkauft. Entferne sie vorher, wenn du sie behalten willst.":"";if(!await gameConfirm({title:"Karte verkaufen",message:`${m.name}\nVerkaufspreis: ${fmt(m.value)} Points${warning}\n\nDer Verkauf ist endgültig.`,confirmText:`Für ${fmt(m.value)} Points verkaufen`,icon:"💰",tone:"danger"}))return;if(potionQueueCount(inst))returnPreparedPotions(inst);if(inst.trail)returnTrailToInventory(inst);clearBackupReferences(id);delete S.instances[id];S.points+=m.value;persist();refresh();closeModal();toast(`+${fmt(m.value)} Points`);}
  function shredDuplicate(id){const inst=instance(id);if(!inst||id===S.featuredCardId||featuredCard()?.backupCardId===id||inst.favorite||inst.locked||S.floors.flat().includes(id)||inst.listed)return;if(potionQueueCount(inst))returnPreparedPotions(inst);if(inst.trail)returnTrailToInventory(inst);const gain=Math.max(1,Math.round((inst.exclusive?20:inst.rarity+1)*4/Math.max(.3,Math.sqrt(cardMeta(inst).rarityValue))));clearBackupReferences(id);delete S.instances[id];S.shards+=gain;persist();refresh();toast(`+${gain} Card Shards`);}
  ...
  function smartSetup(floor=UI.floor){
    ...
    const sorted=Object.values(S.instances).filter(x=>!x.listed&&x.id!==S.featuredCardId&&((x.exclusive&&floor===0)||(x.vip&&floor<=1)||(!x.exclusive&&!x.vip&&(x.rarity<=max||x.fieldPermit)))&&!used.has(x.id)).sort((a,b)=>effectivePoints(b)-effectivePoints(a));
  ...
  function renderMain(){const el=UI.main||UI.overlay?.querySelector("[data-bc-main]");if(!el)return;UI.main=el;if(UI.tab==="mod"&&UI.role!=="owner")UI.tab="field";if(UI.tab==="field")el.innerHTML=fieldHtml();else if(UI.tab==="packs")el.innerHTML=packsHtml();else if(UI.tab==="collection")el.innerHTML=collectionHtml();else if(UI.tab==="shop")el.innerHTML=shopHtml();else if(UI.tab==="market")el.innerHTML=marketHtml();else if(UI.tab==="score")el.innerHTML=scoreHtml();else if(UI.tab==="battle")el.innerHTML=battleHtml();else if(UI.tab==="card")el.innerHTML=featureCardHtml();else if(UI.tab==="vipWheel")el.innerHTML=vipWheelHtml();else if(UI.tab==="vipClicker")el.innerHTML=vipClickerHtml();else if(UI.tab==="mod"&&UI.role==="owner")el.innerHTML=modHtml();else el.innerHTML=fieldHtml();}
  ...
  function showMarketInfo(){
    ...
    showModal(`<div class="bc-market-info"><small>MARKT · SAMMLERWERT</small><h2>🏪 Warum selbst graue Karten extrem wertvoll sein können</h2><p>Die <b>Farbe</b> und der <b>Raritätswert</b> sind zwei verschiedene Dinge. Auch eine gewöhnliche graue Karte kann innerhalb ihrer 500 Motive extrem selten sein.</p><div class="bc-market-info-grid"><article><b>📉 Je kleiner, desto seltener</b><span>100 % ist häufig. Werte unter 10 % werden deutlich seltener; unter 1 % beginnt der echte Sammlerbereich.</span></article><article><b>💎 0,7 % ist bereits extrem</b><span>Das interne Basisgewicht entspricht ungefähr nur 1 von ${fmt(odds07)} Motivauswahlen – noch bevor die Pack-Raritätsstufe berücksichtigt wird.</span></article><article class="ultra"><b>🔥 0,1 % ist ein Extremfund</b><span>Die seltenste normale Motivstufe liegt intern nur bei ungefähr 1 von ${fmt(odds01)} Motivauswahlen. Eine solche Karte kann deshalb auf dem Spielermarkt trotz grauem Rahmen sehr hoch gehandelt werden.</span></article><article><b>🤝 Marktpreis bleibt frei</b><span>Es gibt keinen garantierten Verkaufspreis: Spieler bestimmen Angebot und Nachfrage. Level, Rebirth, Rank, Shiny und Ausrüstung können den Wert zusätzlich erhöhen.</span></article></div><div class="bc-market-info-warning"><b>Wichtig:</b> Der auf der Karte angezeigte Raritätswert ist das interne Ziehungsgewicht des Motivs – nicht die fertige Gesamt-Packchance. Die tatsächliche Packchance kann durch die jeweilige Kartenfarbe noch kleiner sein.</div><button class="bc-primary" data-bc-modal-close>Verstanden</button></div>`);
  }
  function marketHtml(){return `<section class="bc-section"><div class="bc-section-title"><div><small>FIREBASE PLAYER MARKET</small><h2>Online-Marktplatz</h2><p>Handel ausschließlich mit Points · 5 % Verkäufergebühr.</p></div><div class="bc-market-title-actions"><button data-bc-market-refresh>Aktualisieren</button><button class="bc-market-info-btn" data-bc-market-info>Info</button></div></div><div class="bc-market-note">Gelistete Karten werden gesperrt. Kauf/Statuswechsel werden über Firestore geprüft. JK-Coins werden hier nicht zwischen Spielern gehandelt.</div><div class="bc-market-grid">${UI.market.length?UI.market.map(marketListingHtml).join(""):`<div class="bc-empty-state"><span>🏪</span><h3>Keine Angebote geladen</h3><p>Drücke auf Aktualisieren oder stelle eine Karte aus deinem Album ein.</p></div>`}</div><h3>Eigene Karte einstellen</h3><div class="bc-market-own">${Object.values(S.instances).filter(x=>!x.listed&&!x.broken&&!S.floors.flat().includes(x.id)&&!x.locked).sort((a,b)=>sellValue(b)-sellValue(a)).slice(0,12).map(x=>`<button data-bc-list-card="${x.id}">${esc(cardMeta(x).name)}<small>${fmt(sellValue(x))} Richtwert</small></button>`).join("")||"Keine freie Karte verfügbar."}</div></section>`}
  ...
  function modInventoryLazyHtml(){
    const cardTotal=Object.keys(S.instances||{}).length;
    return `<div data-bc-mod-inventory-host>${UI.modInventoryData?modInventoryLoadedHtml(UI.modInventoryData):`<section class="bc-mod-inventory"><div class="bc-mod-inventory-head"><div><small>OWNER · PERFORMANCE-MODUS</small><h3>Inventar & Besitz verwalten</h3><p>Das große Karteninventar wird nicht mehr automatisch beim Öffnen des Mod-Menüs berechnet. Dadurch bleibt das Menü sofort bedienbar.</p></div><div class="bc-mod-inventory-stats"><span><b>${fmt(cardTotal)}</b><small>Karten</small></span></div></div><button class="bc-primary" data-bc-mod-load-inventory>Inventar jetzt laden</button></section>`}</div>`;
  }
  ...
  function modHtml(){
    ...
    const feature=featuredCard(),featureMeta=feature?cardMeta(feature):null,cardOpts=feature?`<option value="${feature.id}">${esc(featureMeta.name)} · ${feature.vip?`VIP · ${featureMeta.rarity.name}`:feature.exclusive?"EXCLUSIVE":featureMeta.rarity.name} · Lv ${feature.level}${feature.broken?" · KAPUTT":""}</option>`:"";
