/* BigCards.kl – JK.Games Top Game V405 */
(() => {
  "use strict";

  const VERSION = "2026-08-11-bigcards-v405-auto-opener-4-channel-ui";
  ...
  function refreshAutoOpenerControls(){
    ...
    const time=box.querySelector("[data-bc-auto-time]");if(time)time.textContent=formatAutoWorkTime(S.autoOpenerWorkMs);
  ...
  function packsHtml(){
    ...
    const autoLaneCards=Array.from({length:AUTO_OPENER_MAX_LANES},(_,i)=>{
      const unlockedLane=i<cap,lane=S.autoOpenerLanes?.[i]||{pack:"common",enabled:false};
      if(!unlockedLane)return `<article class="bc-auto-channel locked" data-bc-auto-channel="${i}"><header><button type="button" data-bc-auto-lane="${i}" class="locked">🔒 ${i+1}</button><span>GESPERRT</span></header><div class="bc-auto-channel-locked"><b>Kanal ${i+1}</b><small>${i===0?"Kaufe 1 Auto-Opener-Stunde im JK/Coin-Shop.":`Der ${i+1}. Auto-Opener-Kauf schaltet diesen Kanal dauerhaft frei.`}</small><button type="button" data-bc-jkshop>JK/Coin-Shop öffnen</button></div></article>`;
      return `<article class="bc-auto-channel ${lane.enabled?"running":""} ${i===selected?"selected":""}" data-bc-auto-channel="${i}"><header><button type="button" data-bc-auto-lane="${i}" class="${i===selected?"active":""} ${lane.enabled?"running":""}">${i+1}</button><div><b>Kanal ${i+1}</b><span data-bc-auto-channel-status="${i}">${lane.enabled?"AKTIV":"BEREIT"}</span></div></header><select aria-label="Pack für Auto-Opener Kanal ${i+1}" data-bc-auto-pack="${i}">${RARITIES.map((r,ri)=>`<option value="${r.id}" ${lane.pack===r.id?"selected":""} ${ri>unlocked?"disabled":""}>${r.name}</option>`).join("")}</select><button type="button" data-bc-auto-lane-toggle="${i}" class="bc-auto-channel-toggle ${lane.enabled?"active":""}">${lane.enabled?"Stop":"Start"}</button><div class="bc-auto-lane-result" data-bc-auto-lane-result="${i}">${autoLaneResultHtml(i)}</div></article>`;
    }).join("");
    const autoHtml=`<div class="bc-auto-opener ${cap<1?"locked":"owned"}"><div class="bc-auto-opener-head"><div class="bc-auto-opener-title"><small>AUTO-OPENER</small><button class="bc-auto-info-btn" data-bc-auto-info title="Auto-Opener Info">i</button></div><div class="bc-auto-lanes">${Array.from({length:AUTO_OPENER_MAX_LANES},(_,i)=>`<button type="button" data-bc-auto-lane="${i}" class="${i===selected&&i<cap?"active":""} ${i<cap&&S.autoOpenerLanes[i]?.enabled?"running":""} ${i>=cap?"locked":""}" aria-disabled="${i>=cap}">${i>=cap?"🔒":""}${i+1}</button>`).join("")}</div></div><div class="bc-auto-work-summary"><span><small>ARBEITSZEIT</small><b data-bc-auto-time>${formatAutoWorkTime(S.autoOpenerWorkMs)}</b><em data-bc-auto-meta>${activeCount?`${activeCount} aktiv · ≈ ${formatAutoWorkTime(wallMs)} gleichzeitig`:"pausiert"}</em></span><strong>${cap}/4 Kanäle frei</strong></div><div class="bc-auto-channel-grid">${autoLaneCards}</div></div>`;
