/* BigCards.kl – JK.Games Top Game V393 */
(() => {
  "use strict";

  const VERSION = "2026-08-11-bigcards-v393-wins-rebirth-vip-packs-nav";
  const SAVE_KEY = "jk-games-bigcards-kl-v332";
  const CLOUD_SAVE_COLLECTION = "bigCardsSaves";
  const CLOUD_SCHEMA_VERSION = 393;
  ...
  const WIN_PACKS = Object.freeze(WIN_PACK_DEFS.map((def,packIndex)=>{
    ...
    const cards=names.slice(0,50).map((name,i)=>Object.freeze({
      ...
      packIndex,
      rarity:i===49?Math.min(12,Math.max(rarities[i],focus+1)):clamp(rarities[i],0,12),
  ...
  function addWins(amount){const before=Math.max(0,Number(S?.winsCurrency)||0),gain=Math.max(0,Math.floor(Number(amount)||0));S.winsCurrency=clamp(before+gain,0,WIN_CAP);return Math.max(0,S.winsCurrency-before);}
  ...
  async function smartSetup(floor=UI.floor){
    ...
    try{
      ...
      for(const id in (S.instances||{})){
        ...
        if(inst&&!inst.listed&&inst.id!==S.featuredCardId&&!used.has(inst.id)&&((inst.exclusive&&floor===0)||(inst.vip&&floor<=1)||(!inst.exclusive&&!inst.vip&&(inst.rarity<=max||(hasEarlyFieldVipAccess()&&inst.fieldPermit))))){
  ...
  function spendWins(kind){
    ...
    else if(kind==="repair"){const id=RARITIES[battleUnlockedTier()]?.id||"common";S.repairKits[id]=repairKitCount(id)+1;}
    ...
    S.winsCurrency-=cost;persist();refresh(false);toast(kind==="dust"?"100 Fusionsstaub erhalten.":kind==="repair"?"Passendes Reparatur-Kit erhalten.":"+1 Boss-Bonusticket erhalten.");
  }
  ...
  function battleHtml(){
    ...
    const winsUse=`<section class="bc-wins-use"><div><small>🏆 WINS-WÄHRUNG</small><h3>Siegesserien werden wertvoller</h3><p>1–2er Serie = 1 Win · 3–4 = 2 · 5–6 = 3 · 7–9 = 4 · ab 10 = maximal 5 Wins pro Sieg. Bestand MAX ${WIN_CAP}.</p></div><div class="bc-wins-use-actions"><button data-bc-wins-spend="dust" ${S.winsCurrency<20?"disabled":""}>20 Wins → 100 Fusionsstaub</button><button data-bc-wins-spend="repair" ${S.winsCurrency<35?"disabled":""}>35 Wins → Reparatur-Kit</button><button data-bc-wins-spend="boss" ${S.winsCurrency<50||ensureBossDaily().bonusTickets>=2?"disabled":""}>50 Wins → Boss-Ticket</button></div></section>`;
  ...
  async function syncProfile(force=false){
    ...
    try{
      updateFeaturedEarnings(now());const savedAt=now();S.updatedAt=savedAt;S.version=393;
