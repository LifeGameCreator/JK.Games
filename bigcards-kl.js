/* BigCards.kl – JK.Games Top Game V348 */
(() => {
  "use strict";

  const VERSION = "2026-08-09-bigcards-v347-feature-card-trails";
  ...
  function defaultState(){return {version:348,points:1000,pendingPoints:0,level:1,xp:0,totalRebirths:0,phase:0,phaseRebirths:0,unlockedFloors:1,instances:{},floors:Array.from({length:4},()=>Array(10).fill(null)),collection:{},exclusiveCollection:{},shards:0,auraInventory:{},combatAuraInventory:{},bindInventory:{},repairKits:{},potionInventory:{},trailInventory:{},trailTierUnlocked:0,featuredCardId:null,featuredPendingPoints:0,featuredPendingXp:0,featuredLastAt:now(),jkPackCredits:{},exclusiveCredits:0,autoCollectorUntil:0,autoCollectorPointStep:0,autoOpenerUntil:0,autoPack:"common",autoEnabled:false,battleWins:0,battleLosses:0,battleStreak:0,battleBestStreak:0,battleCooldownUntil:0,battleTierUnlocked:0,battleTierWins:0,battleUpgradeSpent:0,lifetimePointsEarned:0,lifetimeScore:0,maxLevelEver:1,highestProductionEver:0,highestXpProductionEver:0,highestUpgrade:{},shinyMilestones:{},floorScore:{1:true},daily:{day:"",opened:0,upgraded:0,newCards:0,claimed:{}},lastSeen:now(),createdAt:now(),updatedAt:now(),packHistory:[]};}
  ...
  function state(){
    if(S)return S;
    try{const raw=JSON.parse(localStorage.getItem(SAVE_KEY)||"null");S=raw&&typeof raw==="object"?Object.assign(defaultState(),raw):defaultState();}catch{S=defaultState()}
    S.instances||={};S.collection||={};S.exclusiveCollection||={};S.auraInventory||={};S.combatAuraInventory||={};S.bindInventory||={};S.repairKits||={};S.potionInventory||={};S.trailInventory||={};
    ...
    for(const inst of Object.values(S.instances)){if(!inst)continue;if(inst.combatAura===undefined)inst.combatAura=null;if(inst.broken===undefined)inst.broken=false;if(inst.brokenAt===undefined)inst.brokenAt=0;if(inst.battlePotion===undefined)inst.battlePotion=null;if(inst.trail===undefined)inst.trail=null;}
    ...
    S.version=348;const repaired=normalizeFloorUniqueCards();updateFeaturedEarnings(now());if(repaired){try{localStorage.setItem(SAVE_KEY,JSON.stringify(S));}catch{}}return S;
  }
  ...
  function featurePointRate(inst=featuredCard()){if(!inst)return 0;const t=activeTrail(inst);return effectivePoints(inst)*(1+(t?.points||0));}
  function featureXpRate(inst=featuredCard()){if(!inst)return 0;const t=activeTrail(inst);return effectiveXp(inst)*(1+(t?.xp||0));}
  function updateFeaturedEarnings(at=now()){
    if(!S)return;const last=Math.max(0,Number(S.featuredLastAt)||at),dt=Math.max(0,(at-last)/1000);S.featuredLastAt=at;const inst=featuredCard();if(!inst||dt<=0)return;S.featuredPendingPoints=Math.max(0,Number(S.featuredPendingPoints)||0)+featurePointRate(inst)*dt;S.featuredPendingXp=Math.max(0,Number(S.featuredPendingXp)||0)+featureXpRate(inst)*dt;
  }
  ...
  function potionTierIndex(tierId){return tierId==="exclusive"?exclusiveCombatTier():clamp(RARITY_INDEX[tierId]??0,0,RARITIES.length-1);}
  function potionType(typeId){return POTION_TYPES.find(x=>x.id===typeId)||POTION_TYPES[0];}
  function potionKey(tierId,typeId){return `${tierId}:${typeId}`;}
  function potionCount(tierId,typeId){return Math.max(0,Math.floor(Number(S?.potionInventory?.[potionKey(tierId,typeId)])||0));}
  ...
  function potionEffectText(tierId,typeId){const pct=Math.round(potionEffect(tierId,typeId)*100);return typeId==="life"?`+${pct} % Leben`:typeId==="power"?`+${pct} % Schaden`:`-${pct} % erlittener Schaden`;}
  ...
  function combatStats(inst){
    ...
    const aura=combatAuraBy(inst.combatAura),auraMult=aura?.mult||1,trail=activeTrail(inst),trailDamage=1+(trail?.damage||0),trailHp=1+(trail?.hp||0);
    const min=Math.max(1,Math.floor(range.min*quality*levelMult*shinyMult*auraMult*trailDamage));
    const max=Math.max(min+1,Math.ceil(range.max*quality*levelMult*shinyMult*auraMult*trailDamage));
    const avg=(min+max)/2;
    // Persönliche Spuren verstärken Kampfwerte nur, solange die Karte im neuen Karten-Slot aktiv ist.
    const hp=Math.max(45,Math.round((avg*5.5+(tier+1)*35*Math.sqrt(levelMult))*trailHp));
  ...
  function cardPotionSection(inst){const tierId=potionTierId(inst),p=preparedPotionMeta(inst),r=RARITIES[potionTierIndex(tierId)]||RARITIES[0],available=POTION_TYPES.filter(x=>potionCount(tierId,x.id)>0);return `<section class="bc-card-potion-section"><h3>🧪 Kampf-Trank · 1 Kampf</h3><p>${p?`Vorbereitet: <b>${p.icon} ${p.name}</b> · ${p.effectText}. Wird beim nächsten Kampf verbraucht.`:`Kein Trank vorbereitet. Für diese Karte gelten ${tierId==="exclusive"?`Exclusive-Tränke (${r.name}-Skalierung)`:r.name+"-Tränke"}.`}</p><div class="bc-equip-list">${available.map(x=>`<button data-bc-equip-potion="${x.id}" data-card="${inst.id}">${x.icon} ${x.name} · ${potionEffectText(tierId,x.id)} ×${potionCount(tierId,x.id)}</button>`).join("")||"Keine passenden Tränke im Inventar."}${p?`<button data-bc-remove-potion="${inst.id}">Trank entfernen</button>`:""}<button class="bc-potion-open-inline" data-bc-potion-shop>Tränke kaufen</button></div></section>`;}
  ...
  function featureCardHtml(){
    updateFeaturedEarnings(now());const inst=featuredCard(),unlocked=clamp(Math.floor(Number(S.trailTierUnlocked)||0),0,TRAILS.length-1),currentTrail=TRAILS[unlocked],next=TRAILS[unlocked+1]||null;if(!inst)return `<section class="bc-section bc-feature-page"><div class="bc-section-title"><div><small>PERSÖNLICHER KARTEN-SLOT</small><h2>Karte</h2><p>Wähle eine einzelne Karte als persönliche Karte. Sie bekommt einen eigenen unendlichen Sammelspeicher und kann mit Spuren exklusiv verstärkt werden.</p></div></div><div class="bc-empty-state bc-feature-empty"><span>🃏</span><h3>Noch keine persönliche Karte gewählt</h3><p>Die Karte wird aus dem Spielfeld herausgenommen, bleibt aber für Kartenkampf und alle Upgrades verfügbar.</p><button data-bc-feature-picker>Karte auswählen</button></div></section>`;
    const m=cardMeta(inst),t=trailBy(inst.trail),pointRate=featurePointRate(inst),xpRate=featureXpRate(inst),available=TRAILS.filter(x=>trailUnlocked(x.id)&&trailCount(x.id)>0),nextReady=next&&S.level>=next.levelReq&&inst.trail===currentTrail.id&&S.points>=next.unlockCost;
    return `<section class="bc-section bc-feature-page"><div class="bc-section-title"><div><small>PERSÖNLICHER KARTEN-SLOT</small><h2>Karte</h2><p>Diese Karte ist vom normalen Spielfeld getrennt. Ihre Produktion zählt <b>nicht</b> zur Anzeige „Produktion“ oben.</p></div><div class="bc-feature-top-actions"><button data-bc-feature-picker>Karte wechseln</button><button class="secondary" data-bc-feature-clear>Abwählen</button></div></div><div class="bc-feature-layout"><article class="bc-feature-card ${m.className} shiny-${inst.shiny} ${trailClass(inst)} ${inst.broken?"broken":""}">${featuredTrailVisual(inst)}<div class="bc-feature-card-art"><span>${m.icon}</span>${cardEffectBadges(inst)}${inst.broken?'<span class="bc-card-crack" aria-hidden="true"></span>':""}</div><small>${inst.exclusive?"EXCLUSIVE":m.rarity.name} · Rarität ${pct(m.rarityValue)}</small><h2>${esc(m.name)}</h2><div class="bc-feature-card-stats"><span>★ Level ${inst.level}/5</span><span>⚔ ${fmt(m.combat.power)}</span><span>❤️ ${fmt(m.combat.hp)}</span><span>💥 ${fmt(m.combat.min)}–${fmt(m.combat.max)}</span></div><div class="bc-feature-equipment"><span>${inst.aura?`${auraBy(inst.aura)?.icon||"✦"} ${auraBy(inst.aura)?.name}`:"✦ Keine Aura"}</span><span>${inst.combatAura?`⚔ ${combatAuraBy(inst.combatAura)?.name}`:"⚔ Keine Kampf-Aura"}</span><span>${inst.bind?`${bindBy(inst.bind)?.icon||"🔗"} ${bindBy(inst.bind)?.name}`:"🔗 Keine Bindung"}</span><span>${inst.shiny?`⚡ Shiny ${inst.shiny}/3`:"⚡ Kein Shiny"}</span><span>${inst.battlePotion?`🧪 ${preparedPotionMeta(inst)?.name||"Trank"}`:"🧪 Kein Trank"}</span><span>${t?`${t.icon} ${t.name}`:"☄ Keine Spur"}</span></div><button class="bc-feature-detail" data-bc-card="${inst.id}">Karte & Ausrüstung bearbeiten</button></article><div class="bc-feature-side"><section class="bc-feature-vault"><small>UNENDLICHER KARTEN-SLOT-SPEICHER</small><h3>Eigener Ertrag</h3><div class="bc-feature-vault-values"><div><span>Gesammelt</span><b data-bc-feature-pending>${fmt(S.featuredPendingPoints)}</b><small>Points</small></div><div><span>Gesammelte XP</span><b data-bc-feature-xp>${fmt(S.featuredPendingXp)}</b><small>BigCards XP</small></div></div><p>Aktuell <b data-bc-feature-rate>+${fmt(pointRate)}/s</b> Points und <b data-bc-feature-xp-rate>+${fmt(xpRate)}/s XP</b>. Kein Auto-Collector und kein Speicherlimit.</p><button class="bc-feature-collect" data-bc-feature-collect>💰 Geld einsammeln</button></section><section class="bc-feature-trail-panel"><small>SPUR</small><h3>${t?`${t.icon} ${t.name}`:"Keine Spur ausgerüstet"}</h3><p>${t?trailBonusText(t):"Spuren verändern das Design deiner Karte und erhöhen Karten-Slot-Points, XP, Kampfschaden und Kampf-Leben."}</p><div class="bc-feature-trail-actions">${available.map(x=>`<button data-bc-equip-trail="${x.id}" data-card="${inst.id}">${x.icon} ${x.name} ×${trailCount(x.id)}</button>`).join("")||'<span>Keine freigeschaltete Spur im Inventar.</span>'}${t?`<button class="secondary" data-bc-remove-trail="${inst.id}">Spur entfernen</button>`:""}<button data-bc-trail-shop>Spuren kaufen</button></div></section><section class="bc-trail-progression"><small>SPUREN-FREISCHALTUNG ${unlocked+1}/${TRAILS.length}</small><h3>Freigeschaltet bis ${currentTrail.name}</h3>${next?`<p>Für <b>${next.name}</b>: BigCards Level ${next.levelReq}, ${fmt(next.unlockCost)} Points und die <b>${currentTrail.name}</b> muss auf deiner persönlichen Karte ausgerüstet sein.</p><button data-bc-trail-unlock ${nextReady?"":"disabled"}>${nextReady?`${next.name} freischalten · ${fmt(next.unlockCost)}`:`🔒 ${next.name} · Level ${next.levelReq}`}</button>`:'<p><b>MAX:</b> Alle Spuren bis Göttlich freigeschaltet.</p>'}</section></div></div></section>`;
  }

  function sellCard(id){const inst=instance(id);if(!inst)return;if(id===S.featuredCardId)return toast("Wähle die Karte im Tab „Karte“ zuerst ab, bevor du sie verkaufst.");if(inst.broken)return toast("Repariere die Karte vor dem Verkauf.");if(inst.favorite||inst.locked)return toast("Favorisierte/gesperrte Karten können nicht verkauft werden.");if(S.floors.flat().includes(id))return toast("Nimm die Karte zuerst vom Stockwerk.");if(inst.listed)return toast("Die Karte ist im Online-Marktplatz gelistet.");const m=cardMeta(inst),warning=(inst.aura||inst.combatAura||inst.bind)?"\nAura/Kampf-Aura/Bindung werden mitverkauft. Entferne sie vorher, wenn du sie behalten willst.":"";if(!confirm(`${m.name} für ${fmt(m.value)} Points verkaufen?${warning}\nDer Verkauf ist endgültig.`))return;if(inst.battlePotion)returnPreparedPotion(inst);if(inst.trail)returnTrailToInventory(inst);delete S.instances[id];S.points+=m.value;persist();refresh();closeModal();toast(`+${fmt(m.value)} Points`);}
  function shredDuplicate(id){const inst=instance(id);if(!inst||id===S.featuredCardId||inst.favorite||inst.locked||S.floors.flat().includes(id)||inst.listed)return;if(inst.battlePotion)returnPreparedPotion(inst);if(inst.trail)returnTrailToInventory(inst);const gain=Math.max(1,Math.round((inst.exclusive?20:inst.rarity+1)*4/Math.max(.3,Math.sqrt(cardMeta(inst).rarityValue))));delete S.instances[id];S.shards+=gain;persist();refresh();toast(`+${gain} Card Shards`);}
  ...
  function applyOffline(){updateFeaturedEarnings(now());const delta=Math.min(MAX_OFFLINE_MS,Math.max(0,now()-(S.lastSeen||now())));if(delta<30000)return;const gain=productionPerSecond()*(delta/1000)*OFFLINE_RATE;if(gain>0){S.points+=gain;S.lifetimePointsEarned+=gain;setTimeout(()=>toast(`Offline-Ertrag: +${fmt(gain)} Points (${Math.round(OFFLINE_RATE*100)} %)`,5000),400)}S.lastSeen=now();persist();}
  ...
  function startBattle(){
    ...
    const enemy=makeBattleEnemy(player),ps=combatStats(player),es=combatStats(enemy),pm=cardMeta(player),em=cardMeta(enemy),prepared=preparedPotionMeta(player);
    const activePotion=prepared?{...prepared,lifePct:prepared.typeId==="life"?prepared.effect:0,damagePct:prepared.typeId==="power"?prepared.effect:0,guardPct:prepared.typeId==="guard"?prepared.effect:0}:null;
    ...
    UI.battleSession={id:uid(),playerId:player.id,enemy,playerHp:playerMaxHp,playerMaxHp,enemyHp:es.hp,playerState:battleActorState(),enemyState:battleActorState(),playerPotion:activePotion,lastHit:null,turn:"player",round:1,log:[`Gegner aufgedeckt: ${em.name} · ${RARITIES[enemy.rarity]?.name||"Karte"} · Lv ${enemy.level}.${activePotion?` ${activePotion.icon} ${activePotion.name} aktiviert (${activePotion.effectText}).`:""}`],startedAt:now()};
  ...
  function enemyBattleTurn(sessionId){
    ...
    const em=cardMeta(enemy),rawDmg=abilityDamage(enemy,em.combat,ability),dmg=Math.max(1,Math.round(rawDmg*(1-(session.playerPotion?.guardPct||0))));session.playerHp=Math.max(0,session.playerHp-dmg);session.lastHit={target:"player",damage:dmg,icon:ability.icon,name:ability.name,at:now()};applyAbilityUse(enemy,session.enemyState,ability);session.log.push(`Gegner: ${ability.icon} ${ability.name} → ${fmt(dmg)} Schaden${session.playerPotion?.guardPct?` (Schutz-Trank aktiv)`:""}.`);session.log=session.log.slice(-12);
  ...
  function finishBattle(won){
    ...
    UI.battleResult={won,round:session.round,playerId:player?.id||null,playerBroken:!won&&!!player,player:{name:pm?.name||"Karte",icon:pm?.icon||"🃏",className:pm?.className||"",hp:session.playerHp,maxHp:session.playerMaxHp||pm?.combat.hp||0,power:pm?.combat.power||0,min:pm?.combat.min||0,max:pm?.combat.max||0},enemy:{name:em.name,icon:em.icon,className:em.className,hp:session.enemyHp,maxHp:es.hp,power:es.power,min:es.min,max:es.max,level:enemy.level},rewardPoints,rewardShards,firstWin,potion:session.playerPotion||null,log:session.log.slice(-12)};
  ...
  function grantJkCoinPurchase(kind,amount=1){state();const qty=Math.max(1,Math.floor(Number(amount)||1));if(String(kind).startsWith("pack:")){const id=String(kind).split(":")[1];S.jkPackCredits[id]=(S.jkPackCredits[id]||0)+qty}else if(kind==="exclusivePack")S.exclusiveCredits+=qty;else if(kind==="autoOpenerHour")S.autoOpenerUntil=Math.max(now(),S.autoOpenerUntil||0)+qty*3600000;else if(kind==="autoCollectorHour")S.autoCollectorUntil=Math.max(now(),S.autoCollectorUntil||0)+qty*3600000;else if(String(kind).startsWith("aura:")){const id=String(kind).split(":")[1];S.auraInventory[id]=(S.auraInventory[id]||0)+qty}else if(String(kind).startsWith("combatAura:")){const id=String(kind).split(":")[1];S.combatAuraInventory[id]=(S.combatAuraInventory[id]||0)+qty}else if(String(kind).startsWith("bind:")){const id=String(kind).split(":")[1];S.bindInventory[id]=(S.bindInventory[id]||0)+qty}else if(String(kind).startsWith("trail:")){const id=String(kind).split(":")[1];if(!trailBy(id))return false;S.trailInventory[id]=trailCount(id)+qty}else if(String(kind).startsWith("pointsMinutes:")){const mins=Number(String(kind).split(":")[1])||5;S.points+=Math.max(1000,productionPerSecond()*60*mins)*qty}else return false;persist();if(UI.overlay){refresh();toast("JK/Coin-Kauf in BigCards.kl gutgeschrieben.")}return true;}
  ...
  function close(){if(!UI.overlay)return;updateFeaturedEarnings(now());clearTimeout(UI.battleEnemyTimer);UI.battleEnemyTimer=0;UI.battleSession=null;clearFieldDragVisuals();UI.drag=null;S.lastSeen=now();persist();syncProfile();window.removeEventListener("keydown",onKey);clearInterval(tickTimer);clearInterval(autoTimer);UI.overlay.remove();UI.overlay=null;UI.main=null;UI.packReveal=null;document.body.classList.remove("bigcards-kl-open")}
  ...
  window.BigCardsKL=Object.freeze({version:VERSION,open,close,returnToTopGames,grantJkCoinPurchase,getState:()=>state(),rarities:RARITIES,trails:TRAILS,baseNames:BASE_NAMES});
})();
