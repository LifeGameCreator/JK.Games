...
(() => {
  ...
  // V395: 20 Sets mit global eindeutigen Grundkarten. Eine Kartenart gehört nur noch
  // zu genau einem Set. Damit kann z. B. Void Dragon ausschließlich im Dragon-Set
  // vorkommen und nicht gleichzeitig noch im Shadow-Set.
  ...
  const SET_MEMBERSHIP = (()=>{const map=new Map();for(const set of SET_BLUEPRINTS)for(const req of set.requirements){const key=`${req.rarity}:${req.base}`;if(!map.has(key))map.set(key,[set.id]);}return map;})();
  ...
  function expeditionMaterialBonus(inst){const f=fusionVariant(inst),fusion=f==="prismatic"?1.035:f==="holo"?1.02:1,premium=(inst?.exclusive||inst?.vip)?1.05:1;return fusion*premium;}
