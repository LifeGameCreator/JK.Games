/* BigCards.kl – JK.Games Top Game V375 */
(() => {
  "use strict";

  const VERSION = "2026-08-10-bigcards-v375-potion-mastery-queue-mod-drag-balance";
  ...
  function adoptState(raw,{saveLocal=false,userId=""}={}){
    ...
    S.instances||={};S.collection||={};S.exclusiveCollection||={};S.auraInventory||={};S.combatAuraInventory||={};S.bindInventory||={};S.repairKits||={};S.potionInventory||={};S.potionMastery||={};S.trailInventory||={};S.equipmentRewards||={};
    ...
    for(const inst of Object.values(S.instances)){
      if(!inst)continue;
      if(inst.combatAura===undefined)inst.combatAura=null;if(inst.broken===undefined)inst.broken=false;if(inst.brokenAt===undefined)inst.brokenAt=0;if(inst.trail===undefined)inst.trail=null;if(inst.backupCardId===undefined)inst.backupCardId=null;
      ...
      const key=collectionKey(inst),col=inst.exclusive?S.exclusiveCollection:S.collection,scoreLevel=inst.cardRebirth>0?5:clamp(Math.floor(Number(inst.level)||1),1,5);col[key]=col[key]||{firstAt:Number(inst.createdAt)||now(),highestLevel:scoreLevel};col[key].highestLevel=Math.max(Number(col[key].highestLevel)||1,scoreLevel);
    }
  ...
  function combatStats(inst){
    ...
    const exclusiveBalance=inst.exclusive?.95:1; // V375: kleines Combat-only Tuning; Produktion/Dropchance bleiben unverändert.
    const coreMin=Math.max(1,Math.floor(range.min*quality*levelMult*shinyMult*auraMult*trailDamage*exclusiveBalance));
    const coreMax=Math.max(coreMin+1,Math.ceil(range.max*quality*levelMult*shinyMult*auraMult*trailDamage*exclusiveBalance));
  ...
  function onlineCardSnapshot(inst,usePotion=false){
    ...
    return {instanceId:inst.id,name:m.name,icon:m.icon,className:m.className,rarity:inst.exclusive?-1:clamp(Math.floor(Number(inst.rarity)||0),0,RARITIES.length-1),rarityName:inst.exclusive?"Exclusive":m.rarity.name,exclusive:!!inst.exclusive,level:clamp(Math.floor(Number(inst.level)||1),1,5),rebirth:cardRebirth(inst),rebirthBonus:cardRebirthBonus(inst),rank:cardRank(inst),tier:c.tier,min:Math.max(1,Math.floor(c.min)),max:Math.max(1,Math.floor(c.max)),hp:Math.max(1,Math.floor(c.hp)),baseHp:Math.max(1,Math.floor(c.hp)),power:Math.max(1,Math.floor(c.power)),specialBonus:rankActive(inst)?rankSpecialBonus(inst):0,guardReduction:rankActive(inst)?rankDamageReduction(inst):0,potionQueue:potionQueueSnap};
  }
  ...
  function modInventoryCardGroups(){const groups=new Map(),protectedIds=modProtectedCardIds();for(const inst of Object.values(S.instances||{})){if(!inst)continue;const key=collectionKey(inst),g=groups.get(key)||{key,label:cardMeta(inst).name,rarity:inst.exclusive?"Exclusive":cardMeta(inst).rarity.name,rarityClass:inst.exclusive?"rar-exclusive":`rar-${cardMeta(inst).rarity.id}`,icon:cardMeta(inst).icon,count:0,removable:0};g.count++;if(!protectedIds.has(String(inst.id))&&!inst.listed&&!inst.locked&&!inst.favorite)g.removable++;groups.set(key,g);}return [...groups.values()].sort((a,b)=>a.rarity.localeCompare(b.rarity,"de")||a.label.localeCompare(b.label,"de"));}
