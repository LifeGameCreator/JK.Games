/* BigCards.kl – JK.Games Top Game V421 · Modern Owner Mod Menu + Firestore Guard */
(() => {
  "use strict";

  const VERSION = "2026-08-12-bigcards-v421-modern-mod-firestore-guard";
...
autoOpenerStartCollapsed:false
  ...
  const BIGCARDS_UI_PREF_DEFAULTS = Object.freeze({
    ...
    showPackDescriptions:true,compactMobile:true,reduceAnimations:false,
    autoOpenerStartCollapsed:false
  });
...
Hyper-Kerne werden für die hohen Hyper-Generationen benötigt.`,confirmText:"Hyper-Kern herstellen",icon:"⚡",tone:"danger"}))return;
  ...
  function showSectionInfo(id){
    const info={
      ...
      collection:["Sammlung","Oben gibt es F2P-, Rebirth- und Pay-Score getrennt. Jeder Score hat eine eigene Schnellinfo über den runden i-Button und kann in Einstellungen einzeln ausgeblendet werden. Karten-Rebirth-Ränder: RB1 Cyan, RB2 Violett, RB3 Gold, RB4 Pink/Rot, RB5 Regenbogen."],
  ...
  function modCatalogResults(limit=72){
    const q=modCatalogQuery(),type=modCatalogType(),out=[],push=row=>{if(out.length>=limit)return false;const hay=`${row.label} ${row.sub} ${row.key}`.toLocaleLowerCase("de-DE");if(q&&!hay.includes(q))return true;out.push(row);return out.length<limit;};
    if(!q&&type==="all")return[];
    if(type==="all"||type==="normal"){outer:for(let rarity=0;rarity<RARITIES.length;rarity++){const r=RARITIES[rarity];for(let base=0;base<BASE_NAMES.length;base++){if(!push({key:`n:${rarity}:${base}`,label:BASE_NAMES[base],sub:`${r.name} · ${rarityValue(base).toLocaleString("de-DE",{maximumFractionDigits:4})} %`,icon:r.symbol,className:`rar-${r.id}`}))break outer;}}}
    if((type==="all"||type==="exclusive")&&out.length<limit)for(const x of EXCLUSIVES)if(!push({key:`x:${x.id}`,label:x.name,sub:`Exclusive · ${x.chance.toLocaleString("de-DE",{maximumFractionDigits:4})} %`,icon:"🩸",className:"rar-exclusive"}))break;
    if((type==="all"||type==="vip")&&out.length<limit)for(const v of VIP_CARDS)if(!push({key:`v:${v.id}`,label:v.name,sub:`VIP · ${RARITIES[v.rarity]?.name||"Karte"}`,icon:"👑",className:`rar-${RARITIES[v.rarity]?.id||"common"} rar-vip`}))break;
    if((type==="all"||type==="wins")&&out.length<limit)for(const w of WIN_CARDS)if(!push({key:`w:${w.id}`,label:w.name,sub:`Wins · ${RARITIES[w.rarity]?.name||"Karte"} · ${w.chance.toLocaleString("de-DE",{maximumFractionDigits:4})} %`,icon:"🏆",className:"rar-wins"}))break;
    if((type==="all"||type==="hyper")&&out.length<limit)for(const h of HYPER_CARDS)if(!push({key:`h:${h.id}`,label:h.name,sub:`Hyper · ${h.tierName} · ${h.chance.toLocaleString("de-DE",{maximumFractionDigits:4})} %`,icon:"⚡",className:"rar-hyper"}))break;
    return out;
  }
  function modCardGrantResultsHtml(){const rows=modCatalogResults();if(!modCatalogQuery()&&modCatalogType()==="all")return`<div class="bc-mod-card-empty">Suche nach Kartenname, Rarität oder Karten-ID – oder wähle zuerst einen Bereich.</div>`;if(!rows.length)return`<div class="bc-mod-card-empty">Keine Karte für diese Suche gefunden.</div>`;return`<div class="bc-mod-card-results">${rows.map(row=>`<article class="bc-mod-card-result ${row.className}"><span>${row.icon}</span><div><b>${esc(row.label)}</b><small>${esc(row.sub)}</small></div><button data-bc-mod-give-card="${esc(row.key)}">Geben</button></article>`).join("")}</div>${rows.length>=72?`<small class="bc-mod-card-limit">Maximal 72 Treffer sichtbar – Suche genauer.</small>`:""}`;}
  function renderModCardGrantResults(){const host=UI.overlay?.querySelector("[data-bc-mod-card-grant-results]");if(host)host.innerHTML=modCardGrantResultsHtml();}
  function modGrantCardOptions(){const q=s=>UI.overlay?.querySelector(s);return{quantity:clamp(Math.floor(Number(q("[data-bc-mod-give-qty]")?.value)||1),1,50),level:clamp(Math.floor(Number(q("[data-bc-mod-give-level]")?.value)||1),1,5),rebirth:clamp(Math.floor(Number(q("[data-bc-mod-give-rebirth]")?.value)||0),0,CARD_REBIRTH_MAX),generation:clamp(Math.floor(Number(q("[data-bc-mod-give-generation]")?.value)||1),1,HYPER_GENERATION_MAX),rank:clamp(Math.floor(Number(q("[data-bc-mod-give-rank]")?.value)||0),0,FEATURED_RANK_MAX)};}
  function modFinalizeGrantedCard(inst,opt){if(!inst)return;inst.level=opt.level;inst.cardRebirth=opt.rebirth;inst.rank=opt.rank;inst.rankMastery=opt.rank?rankMeta(opt.rank).mastery:0;inst.rankEliteWins=opt.rank?rankMeta(opt.rank).elite:0;inst.rankWins=0;inst.rankedAt=opt.rank?now():0;if(inst.hyper)inst.hyperGeneration=opt.generation;const key=collectionKey(inst),col=cardCollectionMap(inst);col[key]=col[key]||{firstAt:Number(inst.createdAt)||now(),highestLevel:1,highestRebirth:0,...(inst.hyper?{highestGeneration:1}:{})};col[key].highestLevel=Math.max(Number(col[key].highestLevel)||1,opt.rebirth>0?5:opt.level);col[key].highestRebirth=Math.max(Number(col[key].highestRebirth)||0,opt.rebirth);if(inst.hyper)col[key].highestGeneration=Math.max(Number(col[key].highestGeneration)||1,opt.generation);}
  function modCreateSpecificCard(key,opt){const[kind,a,b]=String(key||"").split(":");let added=null;if(kind==="n"){const rarity=clamp(Math.floor(Number(a)||0),0,RARITIES.length-1),base=clamp(Math.floor(Number(b)||0),0,BASE_NAMES.length-1);added=addInstance({rarity,base});}else if(kind==="x"){const x=EXCLUSIVES.find(v=>v.id===a);if(!x)return null;added=addInstance({exclusive:true,exclusiveId:x.id,basePower:strongestUsableNormalBase()*x.strength,rarityValue:x.rarityValue});}else if(kind==="v"){const v=VIP_CARDS.find(row=>row.id===a);if(!v)return null;added=addInstance({vip:true,vipId:v.id,rarity:v.rarity,basePower:vipBasePower(v),rarityValue:v.rarityValue});}else if(kind==="w"){const w=WIN_CARD_BY_ID[a];if(!w)return null;added=addInstance({win:true,winId:w.id,winPack:w.packIndex,rarity:w.rarity,base:w.base,rarityValue:w.chance});}else if(kind==="h"){const h=HYPER_CARD_BY_ID[a];if(!h)return null;added=addInstance({hyper:true,hyperId:h.id,hyperGeneration:opt.generation,rarity:h.rarity,base:h.base,rarityValue:h.chance});}const inst=added?.inst;if(!inst)return null;modFinalizeGrantedCard(inst,opt);return inst;}
  function modGiveSpecificCard(key){if(UI.role!=="owner")return toast("Owner-Rechte erforderlich.");const opt=modGrantCardOptions(),created=[];for(let i=0;i<opt.quantity;i++){const inst=modCreateSpecificCard(key,opt);if(inst)created.push(inst);}if(!created.length)return toast("Karte konnte nicht erstellt werden.");invalidateCardPowerCache();UI.modLoadToken++;UI.modInventoryData=null;persist();refresh(true);const first=cardMeta(created[0]);toast(`🧪 ${created.length}× ${first.name} gegeben · Lv ${opt.level} · RB ${opt.rebirth}${created[0].hyper?` · G${opt.generation}`:""}${opt.rank?` · Rank ${opt.rank}`:""}`,4200);}
  ...
  function modHtml(){
    ...
    return `<section class="bc-section bc-mod-v421"><div class="bc-section-title"><div><small>OWNER ONLY · V421</small><h2>BigCards Mod-Menü</h2><p>Aktueller Teststand für normale Karten, Wins, Exclusive, VIP, Hyper, Prestige, Fusion, Tränke, Reparatur, Auto-Opener und Cloud-Balancing.</p></div></div>
    <section class="bc-mod-card-giver"><div class="bc-mod-latest-head"><div><small>ALLE KARTEN · DIREKT GEBEN</small><h3>🃏 Einzelne Karte suchen & geben</h3><p>Durchsucht den kompletten aktuellen Kartenpool – einschließlich VIP, Wins, Exclusive und Hyper.</p></div></div><div class="bc-mod-card-giver-tools"><select data-bc-mod-card-type><option value="all" ${UI.modCardGrantType==="all"?"selected":""}>Alle Karten</option><option value="normal" ${UI.modCardGrantType==="normal"?"selected":""}>Gewöhnlich–Göttlich</option><option value="exclusive" ${UI.modCardGrantType==="exclusive"?"selected":""}>Exclusive</option><option value="vip" ${UI.modCardGrantType==="vip"?"selected":""}>VIP</option><option value="wins" ${UI.modCardGrantType==="wins"?"selected":""}>Wins</option><option value="hyper" ${UI.modCardGrantType==="hyper"?"selected":""}>Hyper</option></select><input type="search" data-bc-mod-card-search value="${esc(UI.modCardGrantSearch||"")}" placeholder="z. B. Apex Godfang, Dragon, VIP Royal …"></div><div class="bc-mod-card-giver-options"><label>Anzahl<input type="number" min="1" max="50" value="1" data-bc-mod-give-qty></label><label>Kartenlevel<select data-bc-mod-give-level>${[1,2,3,4,5].map(v=>`<option value="${v}">Level ${v}</option>`).join("")}</select></label><label>Karten-Rebirth<select data-bc-mod-give-rebirth>${[0,1,2,3,4,5].map(v=>`<option value="${v}">RB ${v}</option>`).join("")}</select></label><label>Hyper-Generation<select data-bc-mod-give-generation>${[1,2,3,4].map(v=>`<option value="${v}">Generation ${v}</option>`).join("")}</select></label><label>Rank<select data-bc-mod-give-rank>${Array.from({length:11},(_,v)=>`<option value="${v}">Rank ${v}</option>`).join("")}</select></label></div><div data-bc-mod-card-grant-results>${modCardGrantResultsHtml()}</div></section>
