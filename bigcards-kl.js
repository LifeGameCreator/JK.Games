/* BigCards.kl – JK.Games Top Game V380 */
(() => {
  "use strict";

  const VERSION = "2026-08-10-bigcards-v380-full-backup-card-layout";
  ...
  const VIP_CARDS = Object.freeze(VIP_CARD_PREFIXES.flatMap((pre,a)=>VIP_CARD_TITLES.map((title,b)=>`${pre} ${title}`)).map((name,i)=>{
    ...
    return Object.freeze({id:`vip-${String(i+1).padStart(3,"0")}`,name:`VIP ${name}`,rarity,rarityValue:Math.max(.1,Number((30-slot*3.7).toFixed(2))),powerFactor:1.02+slot*.018});
  }));
  ...
  function vipClickerRarityRoll(){const idx=clamp(rarityUnlockedIndex(),0,RARITIES.length-1),table=DROP_TABLES[RARITIES[idx]?.id]||DROP_TABLES.common;return rollWeighted(table);}
  ...
  function makeVipClickerEnemy(boss=false){const rarity=vipClickerRarityRoll(),base=vipClickerDamage(),progress=1+Math.min(2.5,(Number(S?.vipClickerDefeats)||0)/180),maxHp=Math.max(25,Math.round(base*(6+rarity*1.55)*progress*(boss?5.5:1)));return {id:uid(),rarity,boss:!!boss,name:vipClickerEnemyName(rarity,boss),hp:maxHp,maxHp,spawnedAt:now()};}
  ...
  function vipBasePower(meta){const r=RARITIES[meta?.rarity]||RARITIES[0],f=clamp(Number(meta?.powerFactor)||1,1,1.18);return Math.max(1,(r.min+(r.max-r.min)*.58)*f);}
  ...
  function combatStats(inst){
    ...
    if(inst.exclusive){
      ...
      const vm=vipCardBy(inst.vipId),slot=Math.max(0,VIP_CARDS.findIndex(x=>x.id===inst.vipId));quality=1.00+(slot%8)*.012+(vm?.rarity===12?.015:0);
    }
    ...
    const exclusiveBalance=inst.exclusive?.60:1,vipCombatBalance=inst.vip?.94:1,vipWheelDamage=vipWheelMultiplier("damage");
  ...
  function rollVipCard(forceRarity=null){const rarity=forceRarity===null?vipClickerRarityRoll():clamp(Math.floor(Number(forceRarity)||0),0,12),pool=VIP_CARDS.filter(x=>x.rarity===rarity),fallback=VIP_CARDS.filter(x=>x.rarity<=rarity),vm=(pool.length?pool:fallback)[Math.floor(Math.random()*Math.max(1,(pool.length?pool:fallback).length))]||VIP_CARDS[0],added=addInstance({vip:true,vipId:vm.id,rarity:vm.rarity,basePower:vipBasePower(vm),rarityValue:vm.rarityValue});return {...added,vipMeta:vm,actualChance:0,rarityValue:vm.rarityValue};}
  ...
  function showVipInfo(){
    const reward=vipWheelRewardMeta();showModal(`<div class="bc-vip-info"><small>BIGCARDS VIP · DAUERHAFT</small><h2>👑 Das komplette VIP-Programm</h2><p>VIP kostet einmalig <b>500 JK/Coin</b> und bleibt dauerhaft auf deinem BigCards-Spielstand. Es schaltet zwei eigene Bereiche frei: <b>VIP-Glücksrad</b> und <b>VIP-Klicker</b>.</p><div class="bc-vip-info-grid"><article><b>🎡 Tägliches Glücksrad</b><span>1 Spin pro Kalendertag. Der gezogene Bonus läuft bis zum nächsten täglichen Spin/Reset.</span><small>2× XP · 2× Kraft · 2× Points · 2× Produktion · 2× VIP-Klicks</small></article><article><b>👆 VIP-Klicker</b><span>Klicke Karten herunter, sammle VIP-Klicks und verbessere den Klicker bis Level 20.</span><small>Nach jedem besiegten Gegner besteht <b>2 %</b> Chance, dass der nächste Gegner ein VIP-Boss ist.</small></article><article><b>👑 100 VIP-Karten</b><span>VIP-Karten gibt es ausschließlich als seltene Klicker-Belohnung. Ihre möglichen Raritäten folgen dem Drop-Pool deines aktuell freigeschalteten Packs.</span><small>VIP-Karten dürfen auf Stockwerk 1 oder 2 und im Kartenkampf eingesetzt werden.</small></article><article><b>💥 Boss-Specials</b><span>13 Special-Attacken von Gewöhnlich bis Göttlich. Du kaufst sie mit VIP-Klicks, rüstest eine aus und lädst sie durch normale Klicks auf.</span><small>Special-Attacken funktionieren gegen VIP-Bosse und werden mit höheren Raritäten stärker.</small></article></div>${reward&&vipWheelActive(reward.id)?`<div class="bc-vip-current-bonus"><b>Aktueller Tagesbonus: ${reward.icon} ${reward.label}</b><span>${esc(reward.desc)}</span><small>Noch ${timeLeft(S.vipWheelUntil)}</small></div>`:""}<button data-bc-modal-close>Verstanden</button></div>`);
  }
  ...
  function awardVipClickerDefeat(enemy){
    ...
    if(boss){S.vipClickerBosses++;raiseScore(180+(rarity+1)*65);}else raiseScore(8+(rarity+1)*4);S.vipClickerLastReward=row;return row;
  }
  function vipClick(){
  ...
  function enemyBattleTurn(sessionId){const session=UI.battleSession;if(!session||session.id!==sessionId||session.turn!=="enemy")return;const player=instance(session.playerId),enemy=session.enemy;if(!player||!enemy)return;const choices=combatAbilities(enemy,session.enemyState),specials=choices.filter(x=>!x.normal&&!x.locked&&enemy.level>=(x.req||1));let ability=choices[0];if(specials.length&&Math.random()<Math.min(.82,.52+enemy.rarity*.025))ability=specials[Math.floor(Math.random()*specials.length)];const em=cardMeta(enemy),rawDmg=abilityDamage(enemy,em.combat,ability),rankGuard=rankDamageReduction(player),dmg=Math.max(1,Math.round(rawDmg*(1-(session.playerPotion?.guardPct||0))*(1-rankGuard)));session.playerHp=Math.max(0,session.playerHp-dmg);session.lastHit={target:"player",damage:dmg,icon:ability.icon,name:ability.name,at:now()};applyAbilityUse(enemy,session.enemyState,ability);session.log.push(`Gegner: ${ability.icon} ${ability.name} → ${fmt(dmg)} Schaden${session.playerPotion?.guardPct?` (${session.playerPotion.name})`:""}${rankGuard?` (Rank-Schutz ${Math.round(rankGuard*100)} %)`:""}.`);session.log=session.log.slice(-12);
    if(session.playerHp<=0){if(session.activeRole==="primary"&&battleBackupForSession(session)){player.broken=true;player.brokenAt=now();session.primaryFallen=true;session.round++;if(deployBattleBackup("ko")){persist();toast("💥 Hauptkarte K.O. – Backup-Karte übernimmt!",3200);return;}}return finishBattle(false);}
  ...
  function finishBattle(won){
    ...
    if(won){S.battleWins=(S.battleWins||0)+1;S.battleStreak=(S.battleStreak||0)+1;S.battleBestStreak=Math.max(S.battleBestStreak||0,S.battleStreak);S.points+=rewardPoints;S.shards+=rewardShards;addXp(16+(es.tier+1)*4);raiseScore(55+(es.tier+1)*22);if(firstWin){S.combatAuraInventory.basic=(S.combatAuraInventory.basic||0)+1;S.equipmentRewards[equipmentRewardKey("combatAura","basic")]=true;}const progressTier=battleUnlockedTier();if(progressTier<RARITIES.length-1&&primary&&(primary.exclusive||combatTier(primary)===progressTier))S.battleTierWins=(S.battleTierWins||0)+1;if(rankSource&&rankSource.id===S.featuredCardId&&cardRank(rankSource)<FEATURED_RANK_MAX){rankMasteryEarned=rankMasteryGain(enemy);rankSource.rankMastery=cardRankMastery(rankSource)+rankMasteryEarned;rankMasteryNow=rankSource.rankMastery;if(Number(enemy?.difficultyShift)>0){rankSource.rankEliteWins=cardRankEliteWins(rankSource)+1;rankEliteEarned=true;}rankEliteNow=cardRankEliteWins(rankSource);rankNext=nextRankMeta(rankSource);session.log.push(`Meisterschaft: +${fmt(rankMasteryEarned)} MP${rankEliteEarned?" · +1 Elite-Sieg":""}${rankNext?` · Rank ${rankNext.rank}: ${fmt(rankMasteryNow)}/${fmt(rankNext.mastery)} MP${rankNext.elite?` · Elite ${fmt(rankEliteNow)}/${fmt(rankNext.elite)}`:""}`:""}.`);}}else{S.battleLosses=(S.battleLosses||0)+1;S.battleStreak=0;addXp(5+(es.tier+1));if(player){player.broken=true;player.brokenAt=now();}}
