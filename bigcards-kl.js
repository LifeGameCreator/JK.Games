/* BigCards.kl – JK.Games Top Game V373 */
(() => {
  "use strict";

  const VERSION = "2026-08-10-bigcards-v373-mod-inventory-bulk-24h";
  const SAVE_KEY = "jk-games-bigcards-kl-v332";
  const CLOUD_SAVE_COLLECTION = "bigCardsSaves";
  const CLOUD_SCHEMA_VERSION = 373;
  ...
  function adoptState(raw,{saveLocal=false,userId=""}={}){
    ...
    S.version=373;const repaired=normalizeFloorUniqueCards();updateFeaturedEarnings(now(),false);if(saveLocal||repaired||potionInventoryMigrated||bulkLevelMigrated)writeLocalState(userId||cloudUid||currentUidSync());return S;
  }
  function state(){
    if(S)return S;
