/* BigCards.kl – JK.Games Top Game V392 */
(() => {
  "use strict";

  const VERSION = "2026-08-11-bigcards-v392-expedition-boss-sets-fusion-market";
  const SAVE_KEY = "jk-games-bigcards-kl-v332";
  const CLOUD_SAVE_COLLECTION = "bigCardsSaves";
  const CLOUD_SCHEMA_VERSION = 392;
  ...
  // V392: Sets bestehen aus exakt benannten Karten unterschiedlicher Raritäten.
  // Duplikate erhöhen den Set-Fortschritt nicht; dieselbe Grundkarte kann höchstens
  // in zwei Sets vorkommen. Die 10/10-Stufe ist bewusst bis in hohe Raritäten gezogen.
  const SET_BLUEPRINTS = Object.freeze([
    ...
    {id:"blood",name:"Blood",icon:"🩸",cards:["Reaper","Shadow Assassin","Toxic Raven","Shadow Reaper","Infernal Hunter","Toxic Sorcerer","Infernal Knight","Royal Reaper","Arcane Reaper","Celestial Reaper"]},
    {id:"medieval",name:"Medieval",icon:"🏰",cards:["Viking","Knight","Iron Knight","Samurai","Guardian","Royal Guard"].filter(Boolean)},
    {id:"galaxy",name:"Galaxy",icon:"🌌",cards:["Raven","Lunar Ranger","Solar Hunter","Void Sorcerer","Celestial Warden","Solar Phoenix","Lunar Dragon","Void Colossus","Celestial Sorcerer","Celestial Dragon"]},
    {id:"shadow",name:"Shadow",icon:"🌑",cards:["Assassin","Shadow Raven","Shadow Hunter","Shadow Knight","Void Assassin","Phantom Reaper","Shadow Sorcerer","Void Warden","Infernal Reaper","Void Dragon"]},
    {id:"royal",name:"Royal",icon:"👑",cards:["Guardian","Iron Warden","Royal Viking","Royal Samurai","Royal Knight","Royal Guardian","Royal Valkyrie","Royal Colossus","Celestial Guardian","Celestial Warden"]},
    {id:"beast",name:"Beast",icon:"🐺",cards:["Wolf","Beast","Iron Wolf","Storm Beast","Frost Wolf","Flame Beast","Ancient Beast","Titan Wolf","Infernal Beast","Celestial Beast"]},
    {id:"fire",name:"Fire",icon:"🔥",cards:["Phoenix","Flame Viking","Flame Wolf","Flame Phoenix","Infernal Samurai","Infernal Berserker","Solar Phoenix","Flame Dragon","Infernal Dragon","Celestial Phoenix"]},
    {id:"ice",name:"Ice",icon:"❄️",cards:["Wolf","Frost Viking","Crystal Ranger","Frost Raven","Crystal Knight","Frost Sorcerer","Crystal Golem","Frost Dragon","Lunar Valkyrie","Celestial Ranger"]},
    {id:"magic",name:"Magic",icon:"🔮",cards:["Sorcerer","Golem","Arcane Monk","Crystal Sorcerer","Arcane Golem","Lunar Sorcerer","Void Sorcerer","Ancient Sorcerer","Celestial Monk","Celestial Sorcerer"]},
    {id:"warrior",name:"Warrior",icon:"⚔️",cards:["Viking","Samurai","Iron Viking","Knight","Gladiator","Berserker","Ronin","Royal Knight","Titan Gladiator","Celestial Valkyrie"]}
  ].map(def=>{
    const raw=def.cards.slice(0,10);
    // Medieval bekommt ausschließlich tatsächlich vorhandene BASE_NAMES.
    const fallback=["Viking","Knight","Iron Knight","Samurai","Guardian","Royal Knight","Ancient Viking","Titan Gladiator","Celestial Valkyrie","Celestial Knight"];
    const names=(def.id==="medieval"?fallback:raw).slice(0,10);
    const requirements=names.map((name,i)=>({name,base:BASE_NAMES.indexOf(name),rarity:SET_REQUIREMENT_RARITIES[i]})).filter(x=>x.base>=0);
    return Object.freeze({...def,cards:Object.freeze(names),requirements:Object.freeze(requirements)});
  }));
  const SET_BY_ID = Object.freeze(Object.fromEntries(SET_BLUEPRINTS.map(x=>[x.id,x])));
  const SET_MEMBERSHIP = (()=>{const map=new Map();for(const set of SET_BLUEPRINTS)for(const req of set.requirements){const key=`${req.rarity}:${req.base}`,arr=map.get(key)||[];if(arr.length<2&&!arr.includes(set.id))arr.push(set.id);map.set(key,arr);}return map;})();
  ...
  function adoptState(raw,{saveLocal=false,userId=""}={}){invalidateCollectionRenderCache();
    ...
    S.vipUnlocked=!!S.vipUnlocked;S.vipWheelDay=String(S.vipWheelDay||"");S.vipWheelReward=VIP_WHEEL_REWARDS.some(x=>x.id===S.vipWheelReward)?S.vipWheelReward:"";S.vipWheelUntil=Math.max(0,Number(S.vipWheelUntil)||0);S.vipClicks=Math.max(0,Math.floor(Number(S.vipClicks)||0));S.vipClickerLevel=clamp(Math.floor(Number(S.vipClickerLevel)||1),1,20);S.vipClickerDefeats=Math.max(0,Math.floor(Number(S.vipClickerDefeats)||0));S.vipClickerBosses=Math.max(0,Math.floor(Number(S.vipClickerBosses)||0));S.vipClickerSpecialCharge=Math.max(0,Math.floor(Number(S.vipClickerSpecialCharge)||0));S.vipClickerSpecialEquipped=VIP_SPECIAL_ATTACKS.some(x=>x.id===S.vipClickerSpecialEquipped)?S.vipClickerSpecialEquipped:null;if(S.vipClickerEnemy&&typeof S.vipClickerEnemy!=="object")S.vipClickerEnemy=null;
    ...
    for(const inst of Object.values(S.instances)){
      if(!inst)continue;
      if(inst.vip===undefined)inst.vip=false;if(inst.vipId===undefined)inst.vipId=null;if(inst.combatAura===undefined)inst.combatAura=null;if(inst.broken===undefined)inst.broken=false;if(inst.brokenAt===undefined)inst.brokenAt=0;if(inst.trail===undefined)inst.trail=null;if(inst.backupCardId===undefined)inst.backupCardId=null;inst.fusion=inst.fusion==="prismatic"?"prismatic":inst.fusion==="holo"?"holo":"normal";
      ...
      const key=collectionKey(inst),col=inst.vip?S.vipCollection:(inst.exclusive?S.exclusiveCollection:S.collection),scoreLevel=inst.cardRebirth>0?5:clamp(Math.floor(Number(inst.level)||1),1,5);col[key]=col[key]||{firstAt:Number(inst.createdAt)||now(),highestLevel:scoreLevel};col[key].highestLevel=Math.max(Number(col[key].highestLevel)||1,scoreLevel);
    }
  ...
  function vipClickerUpgradeCost(level=S?.vipClickerLevel||1){const l=clamp(Math.floor(Number(level)||1),1,20);return Math.max(100,Math.round(100*Math.pow(1.62,l-1)/10)*10);}
  function vipClickerDamage(){const l=clamp(Math.floor(Number(S?.vipClickerLevel)||1),1,20);return Math.max(5,Math.round(12*Math.pow(1.23,l-1)));}
  function vipClickerCritChance(){return S?.vipClickerLevel>=5?.12:.07;}
  function vipClickerVipChance(boss=false){let c=boss?.14:.012;if(S?.vipClickerLevel>=15)c*=1.5;return c;}
  ...
  function makeVipClickerEnemy(boss=false){const rarity=vipClickerEnemyRarityRoll(),base=vipClickerDamage(),progress=1+Math.min(2.5,(Number(S?.vipClickerDefeats)||0)/180),maxHp=Math.max(25,Math.round(base*(6+rarity*1.55)*progress*(boss?5.5:1)));return {id:uid(),rarity,boss:!!boss,name:vipClickerEnemyName(rarity,boss),hp:maxHp,maxHp,spawnedAt:now()};}
  ...
  function cardBaseXp(inst){const premium=inst?.exclusive||inst?.vip,p=premium?Math.max(1,inst.basePower||1):cardBaseProduction(inst.rarity,inst.base);return Math.max(.25,Math.sqrt(p)*(.55+(inst.exclusive?1:inst.vip?.82:(inst.rarity+1)*.08)));}
  ...
  function repairKitPrice(id){
    ...
    // V376: Exclusive-Reparatur folgt weiterhin dem Kampfbereich, aber mit einer
    // bewusst flachen Kurve. Im aktuellen Exotic-Kampfbereich liegt das Siegel
    // zwischen Legendär und Special statt bei mehreren Millionen.
    const curve=[20000,30000,50000,75000,110000,160000,200000,240000,350000,550000,850000,1300000,2000000];
    return curve[clamp(tier,0,curve.length-1)]||20000;
  }
  ...
  function repairCost(inst){
    ...
    if(inst.exclusive){const seal=repairKitPrice("exclusive"),raw=seal*(.18+level*.045);return Math.max(500,Math.ceil(raw/100)*100);}
  ...
  function collectionOwnedIndex(){
    ...
    const maps=Array.from({length:15},()=>new Map());
    for(const inst of Object.values(S.instances||{})){
      if(!inst)continue;
      const tier=inst.vip?14:inst.exclusive?13:clamp(Math.floor(Number(inst.rarity)||0),0,12),key=collectionKey(inst),map=maps[tier],arr=map.get(key);
  ...
  function collectionScoreSnapshot(){
    ...
    collectionScoreCache={tiers,overall,pay};
    return collectionScoreCache;
  }

  function collectionTierScore(tier){return collectionScoreSnapshot().tiers[clamp(Math.floor(Number(tier)||0),0,14)]||0;}
  ...
  function needsFieldPermit(inst,floor=UI.floor){if(!inst||inst.exclusive||inst.vip)return false;return inst.rarity>floorMaxTier(floor)&&!inst.fieldPermit;}

  function addInstance(data){const id=uid(),inst={id,rarity:Math.floor(data.rarity||0),base:Math.floor(data.base||0),level:1,cardRebirth:0,rank:0,rankMastery:0,rankEliteWins:0,rankWins:0,rankedAt:0,backupCardId:null,aura:null,combatAura:null,bind:null,shiny:0,fusion:"normal",battlePotions:[],battlePotion:null,broken:false,brokenAt:0,favorite:false,locked:false,listed:false,vip:!!data.vip,vipId:data.vipId||null,exclusive:!!data.exclusive,exclusiveId:data.exclusiveId||null,basePower:data.basePower||null,createdAt:now()};S.instances[id]=inst;if(!inst.exclusive&&!inst.vip)invalidateCardPowerCache();const key=collectionKey(inst),col=inst.vip?S.vipCollection:(inst.exclusive?S.exclusiveCollection:S.collection);const wasNew=!col[key];col[key]=col[key]||{firstAt:now(),highestLevel:1};if(wasNew){S.daily.newCards=(S.daily.newCards||0)+1;raiseScore(inst.vip?400+(inst.rarity+1)*90:inst.exclusive?2500+Math.round(20000/(Math.max(.1,data.rarityValue||100))):100+Math.round((inst.rarity+1)*80+250/Math.max(.1,rarityValue(inst.base))),"collection");}return {inst,wasNew};}
  ...
  function strongestUsableNormalBase(){
    ...
    for(const id in (S.instances||{})){
      ...
      if(!inst||inst.exclusive||inst.vip||inst.rarity>maxTier)continue;
  ...
  async function bulkLevelCurrentTier(){
    ...
    let applied=0,spent=0,used=0;for(const step of plan.steps){const inst=instance(step.keeperId);if(!inst||inst.level>=step.next)continue;const valid=step.consumeIds.filter(id=>!!instance(id));if(valid.length!==step.consumeIds.length||S.points<step.cost)break;S.points-=step.cost;spent+=step.cost;for(const rid of valid){delete S.instances[rid];used++;}inst.level=step.next;const col=inst.vip?S.vipCollection:(inst.exclusive?S.exclusiveCollection:S.collection),key=collectionKey(inst);if(col[key])col[key].highestLevel=Math.max(col[key].highestLevel||1,step.next);S.daily.upgraded=(S.daily.upgraded||0)+1;raiseScore(150*step.next*(inst.exclusive?3:inst.rarity+1));addXp(50*step.next);applied++;}
  ...
  async function bulkRebirthCurrentTier(){
    ...
    let applied=0;for(const id of plan.cards){const inst=instance(id);if(!inst||inst.listed||inst.broken||inst.level<5||cardRebirth(inst)>=CARD_REBIRTH_MAX)continue;const key=collectionKey(inst),col=inst.vip?S.vipCollection:(inst.exclusive?S.exclusiveCollection:S.collection);col[key]=col[key]||{firstAt:Number(inst.createdAt)||now(),highestLevel:5};col[key].highestLevel=Math.max(5,Number(col[key].highestLevel)||0);inst.cardRebirth=cardRebirth(inst)+1;inst.level=1;applied++;}
    if(applied)invalidateCardPowerCache();
  ...
  function placeCard(id,floor=UI.floor,slot=UI.selectedSlot){
    const inst=instance(id);if(!inst)return;if(isCardOnExpedition(id))return toast("Diese Karte ist gerade auf Expedition. Hole sie nach Abschluss zuerst zurück.");if(id===S.featuredCardId)return toast("Diese Karte liegt im persönlichen Karten-Slot. Wähle sie dort zuerst ab, bevor du sie aufs Spielfeld setzt.");if(inst.listed)return toast("Diese Karte ist im Online-Marktplatz gesperrt.");if(floor>=S.unlockedFloors)return toast("Stockwerk noch gesperrt.");if(inst.exclusive&&floor!==0)return toast("Exclusive-Karten können im Produktionsfeld ausschließlich auf Stockwerk 1 eingesetzt werden.",3600);if(inst.vip&&floor>1)return toast("VIP-Karten können im Produktionsfeld ausschließlich auf Stockwerk 1 oder 2 eingesetzt werden.",3600);
    const max=floorMaxTier(floor),required=(inst.exclusive||inst.vip)?0:inst.rarity;if(required>max&&!inst.fieldPermit)return toast(`${cardMeta(inst).name} ist regulär noch nicht für Stockwerk ${floor+1} freigeschaltet. Kaufe bei dieser Karteninstanz zuerst die vorzeitige Spielfeld-Freigabe.`);
  ...
  async function smartSetup(floor=UI.floor){
    ...
    try{
      ...
      for(const id in (S.instances||{})){
        ...
        if(inst&&!inst.listed&&inst.id!==S.featuredCardId&&!used.has(inst.id)&&((inst.exclusive&&floor===0)||(inst.vip&&floor<=1)||(!inst.exclusive&&!inst.vip&&(inst.rarity<=max||inst.fieldPermit)))){
  ...
  function showVipInfo(){
    const reward=vipWheelRewardMeta(),vipOdds=vipCardRarityChances(),oddsHtml=vipOdds.map(x=>`<span class="rar-${RARITIES[x.rarity]?.id||"common"}"><b>${RARITIES[x.rarity]?.symbol||"⚪"} ${RARITIES[x.rarity]?.name||"Gewöhnlich"}</b><em>${x.chance.toLocaleString("de-DE",{maximumFractionDigits:x.chance<.1?4:2})} % im VIP-Kartenpool</em></span>`).join("");showModal(`<div class="bc-vip-info"><small>BIGCARDS VIP · DAUERHAFT</small><h2>👑 Das komplette VIP-Programm</h2><p>VIP kostet einmalig <b>500 JK/Coin</b> und bleibt dauerhaft auf deinem BigCards-Spielstand. Es schaltet zwei eigene Bereiche frei: <b>VIP-Glücksrad</b> und <b>VIP-Klicker</b>.</p><div class="bc-vip-info-grid"><article><b>🎡 Tägliches Glücksrad</b><span>1 Spin pro Kalendertag. Der gezogene Bonus läuft bis zum nächsten täglichen Spin/Reset.</span><small>2× XP · 2× Kraft · 2× Points · 2× Produktion · 2× VIP-Klicks</small></article><article><b>👆 VIP-Klicker</b><span>Klicke Karten herunter, sammle VIP-Klicks und verbessere den Klicker bis Level 20.</span><small>Nach jedem besiegten Gegner besteht <b>2 %</b> Chance, dass der nächste Gegner ein VIP-Boss ist.</small></article><article><b>👑 100 VIP-Karten</b><span>VIP-Karten sind kampfseitig ungefähr auf Exclusive-Niveau und skalieren mit deinem echten normalen Kartenfortschritt.</span><small>Stockwerk 1–2 + Kartenkampf. Sie zählen nur zum Pay-Score, niemals zum Free-to-Play-Hauptscore.</small></article><article><b>💥 Boss-Specials</b><span>13 Special-Attacken von Gewöhnlich bis Göttlich. Du kaufst sie mit VIP-Klicks, rüstest eine aus und lädst sie durch normale Klicks auf.</span><small>Special-Attacken funktionieren gegen VIP-Bosse und werden mit höheren Raritäten stärker.</small></article></div><div class="bc-vip-drop-info"><small>VIP-KARTEN-RARITÄTEN · AKTUELLER PACK-FORTSCHRITT</small><p>Die Wahrscheinlichkeit fällt jetzt bei jeder höheren Rarität strikt ab. Noch nicht über deine regulären Packs erreichbare Raritäten sind komplett gesperrt.</p><div>${oddsHtml}</div></div>${reward&&vipWheelActive(reward.id)?`<div class="bc-vip-current-bonus"><b>Aktueller Tagesbonus: ${reward.icon} ${reward.label}</b><span>${esc(reward.desc)}</span><small>Noch ${timeLeft(S.vipWheelUntil)}</small></div>`:""}<button data-bc-modal-close>Verstanden</button></div>`);
  }
  function spinVipWheel(){
    if(!S.vipUnlocked)return window.JKCoinApp?.openForGame?.("bigcards");if(!vipWheelAvailable())return toast(`VIP-Glücksrad morgen wieder verfügbar · aktueller Bonus: ${vipWheelRewardMeta()?.label||"bereits gezogen"}.`,4200);let total=VIP_WHEEL_REWARDS.reduce((n,x)=>n+x.weight,0),r=Math.random()*total,pick=VIP_WHEEL_REWARDS[0];for(const row of VIP_WHEEL_REWARDS){r-=row.weight;if(r<=0){pick=row;break;}}S.vipWheelDay=dailyKey();S.vipWheelReward=pick.id;S.vipWheelUntil=nextLocalDayReset();UI.vipWheelSpinning=true;persist();refresh(false);setTimeout(()=>{UI.vipWheelSpinning=false;if(UI.tab==="vipWheel")refresh(true);toast(`${pick.icon} VIP-Glücksrad: ${pick.label} bis zum nächsten Tagesreset!`,5200);},1250);
  }
  function vipWheelHtml(){
    if(!S.vipUnlocked)return `<section class="bc-section bc-vip-locked"><div class="bc-vip-lock-icon">👑</div><small>BIGCARDS VIP</small><h2>VIP-Glücksrad</h2><p>Dieser Bereich gehört zum permanenten VIP-Programm.</p><b>500 JK/Coin · einmalig · dauerhaft</b><div><button data-bc-jkshop>VIP im JK/Coin-Shop kaufen</button><button class="bc-vip-info-btn" data-bc-vip-info>i</button></div></section>`;
    const reward=vipWheelRewardMeta(),available=vipWheelAvailable(),segments=VIP_WHEEL_REWARDS.map(x=>`<span>${x.icon}<small>${x.label}</small></span>`).join("");return `<section class="bc-section bc-vip-page"><div class="bc-section-title"><div><small>VIP · TÄGLICHER BONUS</small><h2>🎡 VIP-Glücksrad</h2><p>Ein Spin pro Tag. Der Bonus bleibt bis zum nächsten täglichen Reset aktiv.</p></div><button class="bc-vip-info-btn" data-bc-vip-info title="VIP-Programm Info">i</button></div><div class="bc-vip-wheel-layout"><div class="bc-vip-wheel-wrap"><div class="bc-vip-wheel-pointer">▼</div><div class="bc-vip-wheel ${UI.vipWheelSpinning?"spinning":""}"><div class="bc-vip-wheel-labels">${segments}</div><strong>VIP</strong></div><button class="bc-vip-spin" data-bc-vip-spin ${!available||UI.vipWheelSpinning?"disabled":""}>${available?"JETZT DREHEN":"MORGEN WIEDER"}</button></div><div class="bc-vip-wheel-status"><small>DEIN TAGESBONUS</small><h3>${reward&&vipWheelActive(reward.id)?`${reward.icon} ${reward.label}`:"Noch kein Bonus aktiv"}</h3><p>${reward&&vipWheelActive(reward.id)?reward.desc:"Drehe das Rad und erhalte einen von fünf VIP-Multiplikatoren."}</p>${reward&&vipWheelActive(reward.id)?`<b>Restzeit ${timeLeft(S.vipWheelUntil)}</b>`:""}<div class="bc-vip-wheel-prizes">${VIP_WHEEL_REWARDS.map(x=>`<span class="${reward?.id===x.id&&vipWheelActive(x.id)?"active":""}">${x.icon} ${x.label}</span>`).join("")}</div></div></div></section>`;
  }
  function awardVipClickerDefeat(enemy){
    const boss=!!enemy.boss,rarity=clamp(Math.floor(Number(enemy.rarity)||0),0,12),row={at:now(),boss,points:0,xp:0,shards:0,vipCard:"",normalCard:"",item:""},pointMult=vipWheelMultiplier("points"),base=Math.max(1,vipClickerDamage());row.points=Math.round(base*(boss?95:12)*(1+rarity*.45)*pointMult);row.xp=Math.round((boss?85:12)*(rarity+1));row.shards=boss?Math.max(5,5+rarity*2):(Math.random()<.22?1+Math.floor(rarity/3):0);S.points+=row.points;S.lifetimePointsEarned+=row.points;addXp(row.xp);S.shards+=row.shards;
  ...
  function vipClick(){
    if(!S.vipUnlocked)return;const enemy=ensureVipClickerEnemy();if(!enemy)return;let dmg=vipClickerDamage(),crit=Math.random()<vipClickerCritChance();if(crit)dmg=Math.round(dmg*2.5);enemy.hp=Math.max(0,Number(enemy.hp)-dmg);const gain=vipWheelMultiplier("clicks");S.vipClicks+=gain;S.vipClickerSpecialCharge+=1;if(enemy.hp<=0){S.vipClickerDefeats++;const reward=awardVipClickerDefeat(enemy),bossNext=Math.random()<.02;S.vipClickerEnemy=makeVipClickerEnemy(bossNext);persist();refresh(true);return toast(`${enemy.boss?"👑 BOSS BESIEGT":"Karte besiegt"} · ${vipRewardSummaryText(reward)}`,enemy.boss?5200:3200);}persistPassive();refresh(true);if(crit)toast(`💥 Kritischer Klick · ${fmt(dmg)} Schaden`,1200);
  }
  function upgradeVipClicker(){if(!S.vipUnlocked)return;if(S.vipClickerLevel>=20)return toast("VIP-Klicker ist bereits Level 20 MAX.");const cost=vipClickerUpgradeCost();if(S.vipClicks<cost)return toast(`Dir fehlen ${fmt(cost-S.vipClicks)} VIP-Klicks.`);S.vipClicks-=cost;S.vipClickerLevel++;persist();refresh(false);toast(`👑 VIP-Klicker Level ${S.vipClickerLevel} erreicht!`,3400);}
  ...
  function useVipSpecial(){const enemy=ensureVipClickerEnemy(),sp=vipSpecialBy(S.vipClickerSpecialEquipped);if(!enemy?.boss)return toast("Special-Attacken sind für VIP-Bosse reserviert.");if(!sp||!vipSpecialOwned(sp.id))return toast("Kaufe und rüste zuerst eine VIP-Special-Attacke aus.");if(S.vipClickerSpecialCharge<sp.charge)return toast(`Noch ${sp.charge-S.vipClickerSpecialCharge} normale Klicks bis ${sp.name} bereit ist.`);const dmg=Math.round(vipClickerDamage()*sp.mult);S.vipClickerSpecialCharge=0;enemy.hp=Math.max(0,enemy.hp-dmg);if(enemy.hp<=0){S.vipClickerDefeats++;const reward=awardVipClickerDefeat(enemy);S.vipClickerEnemy=makeVipClickerEnemy(Math.random()<.02);persist();refresh(true);return toast(`${sp.icon} ${sp.name}: BOSS BESIEGT · ${vipRewardSummaryText(reward)}`,5600);}persist();refresh(true);toast(`${sp.icon} ${sp.name} · ${fmt(dmg)} Boss-Schaden`,2400);}
  function vipClickerHtml(){
    if(!S.vipUnlocked)return `<section class="bc-section bc-vip-locked"><div class="bc-vip-lock-icon">👑</div><small>BIGCARDS VIP</small><h2>VIP-Klicker</h2><p>100 exklusive VIP-Karten, Bosse, Upgrades und Special-Attacken werden mit dem permanenten VIP-Programm freigeschaltet.</p><b>500 JK/Coin · einmalig · dauerhaft</b><div><button data-bc-jkshop>VIP im JK/Coin-Shop kaufen</button><button class="bc-vip-info-btn" data-bc-vip-info>i</button></div></section>`;
    const enemy=ensureVipClickerEnemy(),pctHp=clamp(Number(enemy.hp)/Math.max(1,Number(enemy.maxHp))*100,0,100),eq=vipSpecialBy(S.vipClickerSpecialEquipped),nextCost=S.vipClickerLevel<20?vipClickerUpgradeCost():0,maxDrop=vipUnlockedMaxRarity();return `<section class="bc-section bc-vip-page bc-vip-clicker-page"><div class="bc-section-title"><div><small>VIP · KLICKER ARENA</small><h2>👆 VIP-Klicker</h2><p>Karten herunterklicken, VIP-Klicks sammeln, Bosse jagen und 100 VIP-Karten entdecken.</p></div><button class="bc-vip-info-btn" data-bc-vip-info title="VIP-Programm Info">i</button></div><div class="bc-vip-clicker-stats"><span><small>VIP-KLICKS</small><b>${fmt(S.vipClicks)}</b>${vipWheelActive("clicks")?"<em>2× aktiv</em>":""}</span><span><small>KLICKER</small><b>Level ${S.vipClickerLevel}/20</b><em>${fmt(vipClickerDamage())} Schaden/Klick</em></span><span><small>BESIEGT</small><b>${fmt(S.vipClickerDefeats)}</b><em>${fmt(S.vipClickerBosses)} Bosse</em></span><span><small>VIP-KARTEN</small><b>${vipCount()}/${VIP_CARDS.length}</b><em>Bonus-Sammlung</em></span></div><div class="bc-vip-clicker-layout"><div class="bc-vip-clicker-arena ${enemy.boss?"boss":""}"><div class="bc-vip-enemy-head"><small>${enemy.boss?"👑 2%-VIP-BOSS":"KLICKER-KARTE"}</small><b>${esc(enemy.name)}</b><span>${RARITIES[enemy.rarity]?.name||"Gewöhnlich"}</span></div><div class="bc-vip-enemy-hp"><i style="width:${pctHp}%"></i><b>${fmt(enemy.hp)} / ${fmt(enemy.maxHp)} HP</b></div><button class="bc-vip-card-target rar-${RARITIES[enemy.rarity]?.id||"common"} ${enemy.boss?"boss":""}" data-bc-vip-click><span>${enemy.boss?"👑":"🃏"}</span><b>${enemy.boss?"BOSS ANGREIFEN":"KARTE KLICKEN"}</b><small>+${vipWheelMultiplier("clicks")} VIP-Klick${vipWheelMultiplier("clicks")>1?"s":""} pro Klick</small></button>${enemy.boss?`<button class="bc-vip-special-use" data-bc-vip-special-use ${!eq||!vipSpecialOwned(eq.id)||S.vipClickerSpecialCharge<(eq?.charge||999)?"disabled":""}>${eq?`${eq.icon} ${esc(eq.name)} · ${fmt(S.vipClickerSpecialCharge)}/${eq.charge}`:"Keine Special-Attacke ausgerüstet"}</button>`:""}${S.vipClickerLastReward?`<div class="bc-vip-last-reward"><small>LETZTE BELOHNUNG</small><b>${esc(vipRewardSummaryText(S.vipClickerLastReward))}</b></div>`:""}</div><aside class="bc-vip-clicker-side"><article><small>KLICKER UPGRADE</small><h3>${S.vipClickerLevel>=20?"👑 Level 20 MAX":`Level ${S.vipClickerLevel} → ${S.vipClickerLevel+1}`}</h3><p>Mehr Grundschaden. Level 5 verbessert Krits, Level 15 erhöht die VIP-Karten-Chance, Level 20 ist MAX.</p><button data-bc-vip-clicker-upgrade ${S.vipClickerLevel>=20||S.vipClicks<nextCost?"disabled":""}>${S.vipClickerLevel>=20?"MAX":`${fmt(nextCost)} VIP-Klicks`}</button></article><article><small>DEIN PACK-FORTSCHRITT</small><h3>Bis ${RARITIES[maxDrop]?.symbol||"⚪"} ${RARITIES[maxDrop]?.name||"Gewöhnlich"} im VIP-Drop</h3><p>VIP-Karten dürfen nur bis zu dieser über deine Packs erreichbaren Rarität droppen. Innerhalb des VIP-Pools wird jede höhere Rarität deutlich seltener als die vorherige.</p></article></aside></div><section class="bc-vip-specials"><div><small>BOSS-SPECIAL-ATTACKEN</small><h3>Eine Attacke ausrüsten</h3><p>Mit VIP-Klicks kaufen. Höhere Attacken werden mit deinem normalen Pack-Fortschritt freigeschaltet.</p></div><div class="bc-vip-special-grid">${VIP_SPECIAL_ATTACKS.map(sp=>{const owned=vipSpecialOwned(sp.id),available=vipSpecialAvailable(sp),active=S.vipClickerSpecialEquipped===sp.id;return `<button data-bc-vip-special="${sp.id}" class="rar-${RARITIES[sp.rarity].id} ${active?"active":""}" ${!available?"disabled":""}><span>${sp.icon}</span><b>${esc(sp.name)}</b><small>${RARITIES[sp.rarity].name} · ×${sp.mult.toFixed(2).replace(".",",")} Boss-Schaden</small><em>${active?"AUSGERÜSTET":owned?"Ausrüsten":available?`${fmt(sp.cost)} Klicks`:"🔒 Pack-Fortschritt"}</em></button>`}).join("")}</div></section></section>`;
  }

  async function doRebirth(skip=false){
    const requiredLevel=rebirthRequiredLevel();
  ...
  function showAutoOpenerStatus(results,message=""){if(!UI.overlay)return;let el=UI.overlay.querySelector("[data-bc-auto-notice]");if(!el){el=document.createElement("aside");el.className="bc-auto-opener-notice";el.dataset.bcAutoNotice="1";UI.overlay.append(el);bindAutoNoticeDrag(el);applyAutoNoticePosition(el);}const newCount=(results||[]).filter(x=>x.wasNew).length,dupes=Math.max(0,(results||[]).length-newCount),rarities=autoRaritySummary(results);el.innerHTML=results?`<small>📦 AUTO-OPENER</small><div class="bc-auto-notice-counts"><b class="new">NEU ×${newCount}</b><b class="dupe">DUPLIKATE ×${dupes}</b></div><div class="bc-auto-notice-rarities">${rarities.map(x=>`<span class="auto-rar-${x.r.id}" title="${esc(x.r.name)}${x.newCount?` · ${x.newCount} neu`:""}">${x.r.symbol}<strong>×${x.count}</strong>${x.newCount?`<em>+${x.newCount}</em>`:""}</span>`).join("")}</div>`:`<small>📦 AUTO-OPENER</small><b class="bc-auto-notice-message">${esc(message)}</b>`;el.classList.add("show");clearTimeout(UI.autoNoticeTimer);UI.autoNoticeTimer=setTimeout(()=>{if(!UI.autoNoticeDrag)el.classList.remove("show")},results?4300:2300);}
  function showAutoOpenerInfo(){showModal(`<div class="bc-auto-info"><small>AUTO-OPENER · INFO</small><h2>📦 Vollautomatisch Packs öffnen</h2><p>Der Auto-Opener läuft nach <b>Starten</b> selbstständig weiter, solange BigCards geöffnet ist, deine Auto-Opener-Zeit aktiv ist und du genug Points für das ausgewählte Pack hast.</p><div class="bc-auto-info-grid"><article><b>▶ Starten</b><span>Pack-Rarität wählen und einmal starten. Danach musst du kein Pack mehr antippen.</span></article><article><b>⏹ Stoppen</b><span>Stoppt nur das automatische Öffnen; deine verbleibende Zeit bleibt erhalten.</span></article><article><b>🧭 Parallel weiterspielen</b><span>Spielfeld, Sammlung, Karte, Markt und Kampf bleiben bedienbar. Kein großes Reveal-Fenster blockiert dich.</span></article><article><b>↕ Anzeige frei verschieben</b><span>Die kleine Ergebnisanzeige kannst du mit Maus oder Finger greifen und an eine beliebige Stelle ziehen. Die Position wird auf diesem Gerät gespeichert.</span></article></div><p><b>Anzeige:</b> Du siehst nur „NEU“, „DUPLIKATE“ und Raritätsfarben – keine störende Kartenliste. Ist das Pack gesperrt oder fehlen Points, wartet der Auto-Opener automatisch.</p><button class="bc-primary" data-bc-modal-close>Verstanden</button></div>`);}
  ...
  function setMembershipIds(inst){if(!inst||inst.exclusive||inst.vip)return[];return SET_MEMBERSHIP.get(`${Math.floor(inst.rarity||0)}:${Math.floor(inst.base||0)}`)||[];}
  function setOwnedVariants(){if(setOwnedVariantCache&&setOwnedVariantCacheRev===cardPowerRevision)return setOwnedVariantCache;const set=new Set();for(const inst of Object.values(S?.instances||{})){if(!inst||inst.exclusive||inst.vip)continue;set.add(`${Math.floor(inst.rarity||0)}:${Math.floor(inst.base||0)}`);}setOwnedVariantCacheRev=cardPowerRevision;setOwnedVariantCache=set;return set;}
  function setProgress(setId){const def=SET_BY_ID[setId];if(!def)return {owned:0,total:0,stage:0,bonus:0,requirements:[]};const owned=setOwnedVariants(),rows=def.requirements.map(req=>({...req,owned:owned.has(`${req.rarity}:${req.base}`)})),count=rows.filter(x=>x.owned).length,stage=count>=10?10:count>=5?5:count>=3?3:0,bonus=stage>=10?.04:stage>=5?.02:stage>=3?.01:0;return {owned:count,total:rows.length,stage,bonus,requirements:rows};}
  ...
  function claimSetMission(kind){const w=ensureSetWeekly(),def=SET_BY_ID[w.setId];const target=kind==="expedition"?3:kind==="upgrade"?2:Math.max(5000,Math.floor((Number(S.bossWeek?.goal)||50000)*.25)),value=kind==="expedition"?w.expeditions:kind==="upgrade"?w.upgrades:w.bossDamage;if(w.claimed[kind])return toast("Diese Set-Mission ist bereits abgeholt.");if(value<target)return toast(`Noch nicht erfüllt: ${fmt(value)} / ${fmt(target)}.`);w.claimed[kind]=true;const mastery=kind==="boss"?40:25;addSetMastery([w.setId],mastery);S.fusionDust+=kind==="boss"?120:70;S.auraMaterial+=kind==="expedition"?35:15;S.cosmeticFragments+=kind==="upgrade"?3:1;persist();refresh(false);toast(`${def?.icon||"🧩"} Wochenmission erfüllt · +${mastery} Set-XP · +Fusionsmaterial`,3800);}
  ...
  function expeditionCardEligible(inst){if(!inst||inst.listed||inst.broken||isCardOnExpedition(inst.id)||inst.id===S.featuredCardId)return false;if(S.floors.flat().includes(inst.id))return false;return true;}
  ...
  function bossTeamStats(){const def=bossDefinition(),phase=bossPhase(),cards=bossLoadoutCards();let base=0,tagBonus=0,setBonus=0,premiumBonus=0;cards.forEach((inst,i)=>{if(!inst)return;const power=combatStats(inst).power*fusionBossMultiplier(inst)*(i===0?1:.25);base+=power;const ids=bossCardTags(inst);if(ids.some(id=>def.tags.includes(id)))tagBonus+=.05;for(const id of ids){const stage=setProgress(id).stage;if(stage>=10)setBonus+=.04;else if(stage>=5)setBonus+=.02;}if(inst.exclusive||inst.vip)premiumBonus=Math.max(premiumBonus,.06);});tagBonus=Math.min(.15,tagBonus);setBonus=Math.min(.10,setBonus);const total=Math.max(0,base*(1+tagBonus+setBonus+premiumBonus)*phase.damageMult);return {cards,base,tagBonus,setBonus,premiumBonus,total,def,phase};}
  ...
  function setsHtml(){const selected=SET_BY_ID[UI.setSelected]||SET_BLUEPRINTS[0],progress=setProgress(selected.id),weekly=ensureSetWeekly(),missionBossTarget=Math.max(5000,Math.floor((Number(S.bossWeek?.goal)||50000)*.25)),mastery=setMasteryLevel(selected.id),unlocks=setMasteryUnlocks(selected.id);return `<section class="bc-section bc-sets"><div class="bc-section-title"><div><small>SET-SAMMLUNGEN · GEMISCHTE RARITÄTEN</small><h2>🧩 Set-Sammlungen</h2><p>Eine Gewöhnliche Karte kann genauso wichtig sein wie Legendär, Mythisch oder Galaxy. Duplikate zählen nicht mehrfach.</p></div><div class="bc-set-score"><b>${progress.owned}/${progress.total}</b><span>${progress.stage>=10?"Vollständig":progress.stage>=5?"Aktiv":progress.stage>=3?"Entdeckt":"Im Aufbau"}</span></div></div><div class="bc-set-tabs">${SET_BLUEPRINTS.map(set=>{const p=setProgress(set.id);return `<button data-bc-set-select="${set.id}" class="${selected.id===set.id?"active":""}"><span>${set.icon}</span><b>${set.name}</b><small>${p.owned}/${p.total}</small></button>`}).join("")}</div><div class="bc-set-main"><section class="bc-set-book"><header><span>${selected.icon}</span><div><small>${weekly.setId===selected.id?"⭐ DIESES WOCHEN-SET":"SET-BUCH"}</small><h3>${selected.name} Set · ${progress.owned}/${progress.total}</h3></div></header><div class="bc-set-requirements">${progress.requirements.map((req,i)=>{const r=RARITIES[req.rarity],rv=rarityValue(req.base);return `<article class="${req.owned?"owned":"missing"} rar-${r.id}"><span>${req.owned?"✓":"?"}</span><div><b>${esc(req.name)}</b><small>${r.name} · ${pct(rv)} Motivgewicht${rv<=1?" · 💎 Sammler-Rarität":""}</small></div><em>${req.owned?"BESITZT":"FEHLT"}</em></article>`}).join("")}</div></section><section class="bc-set-bonuses"><h3>Set-Stufen</h3><article class="${progress.stage>=3?"done":""}"><b>3/10 · Set entdeckt</b><span>+1 % Points auf Karten dieses Sets · Badge</span></article><article class="${progress.stage>=5?"done":""}"><b>5/10 · Set aktiv</b><span>+2 % Points/Expedition · +2 % Boss-Schaden</span></article><article class="${progress.stage>=10?"done":""}"><b>10/10 · Set vollständig</b><span>+4 % Points/Boss · +5 % Set-Materialchance · animiertes Prestige</span></article><p>Mehrere Sets werden gedeckelt: max. +8 % Points auf eine Karte und max. +10 % Boss-Setbonus.</p><h3>Set-Mastery · Lv ${mastery}/100</h3><div class="bc-set-mastery"><span style="width:${mastery}%"></span></div><small>${fmt(setMasteryXp(selected.id))} Set-XP · Freigeschaltet: ${unlocks.join(" · ")||"noch keine Prestige-Meilensteine"}</small></section></div><section class="bc-set-weekly"><div><small>WÖCHENTLICHE SET-MISSIONEN · ${SET_BY_ID[weekly.setId]?.icon} ${SET_BY_ID[weekly.setId]?.name}</small><h3>Diese Woche zählt ein rotierendes Set</h3></div>${[{id:"expedition",label:"3 Set-Expeditionen",v:weekly.expeditions,t:3},{id:"boss",label:"Boss-Schaden mit Set-Karten",v:weekly.bossDamage,t:missionBossTarget},{id:"upgrade",label:"2 Set-Karten verbessern",v:weekly.upgrades,t:2}].map(q=>`<article><span><b>${q.label}</b><small>${fmt(Math.min(q.v,q.t))} / ${fmt(q.t)} · Set-XP + Material</small></span>${weekly.claimed[q.id]?"<em>✓</em>":`<button data-bc-set-mission="${q.id}" ${q.v>=q.t?"":"disabled"}>Holen</button>`}</article>`).join("")}</section></section>`;}

  function marketCardKeyFromCard(c){if(!c)return"unknown";const f=c.fusion==="prismatic"?"prismatic":c.fusion==="holo"?"holo":"normal";if(c.vip)return `v:${String(c.vipId||"vip")}:${f}`;if(c.exclusive)return `x:${String(c.exclusiveId||"exclusive")}:${f}`;return `n:${Math.floor(Number(c.rarity)||0)}:${Math.floor(Number(c.base)||0)}:${f}`;}
  function marketCardKeyFromInst(inst){if(!inst)return"unknown";return marketCardKeyFromCard({vip:!!inst.vip,vipId:inst.vipId,exclusive:!!inst.exclusive,exclusiveId:inst.exclusiveId,rarity:inst.rarity,base:inst.base,fusion:fusionVariant(inst)});}
  ...
  function renderNav(){const nav=UI.overlay?.querySelector("[data-bc-nav]");if(!nav)return;nav.classList.toggle("owner-nav",UI.role==="owner");nav.classList.toggle("player-nav",UI.role!=="owner");nav.innerHTML=navItems().map(([id,label,icon,group])=>`<button data-bc-tab="${id}" class="nav-${group} ${UI.tab===id?"active":""}"><span>${icon}</span><b>${label}</b></button>`).join("");}
  ...
  function collectionHtml(){
    const tier=UI.collectionTier,vip=tier===14,exclusive=tier===13,search=UI.collectionSearch.trim().toLowerCase(),ownedByKey=collectionOwnedIndex()[tier]||new Map();
    let entries=vip?VIP_CARDS.map((vm,i)=>({key:`v:${vm.id}`,name:vm.name,base:i,vip:true,rarityValue:vm.rarityValue,rarity:vm.rarity})):
  ...
  async function syncProfile(force=false){
    ...
    try{
      updateFeaturedEarnings(now());const savedAt=now();S.updatedAt=savedAt;S.version=388;
      ...
      for(let n=0;n<changed.length;n++){
        ...
        if(n<changed.length-1)await new Promise(r=>setTimeout(r,700));
      }
      if(changed.length)await new Promise(r=>setTimeout(r,500));
