/* BigCards.kl – JK.Games Top Game V511 · Cloud cache recovery + payout permissions */
(() => {
  "use strict";

  const VERSION = "2026-08-19-bigcards-v511-cloud-cache-recovery";
...
Hyper-Kerne werden für die hohen Hyper-Generationen benötigt.`,confirmText:"Hyper-Kern herstellen",icon:"⚡",tone:"danger"}))return;
  ...
  async function syncProfile(force=false){
    ...
    if(localSafetyReason){
      ...
      if(onlyMissingLoad){
        console.warn("BigCards V511: leerer lokaler Zwischenstand wird nicht gespeichert; sicherer Cloud-Stand bleibt aktiv.",localSafetyReason);
  ...
  async function initializeCloudSession(localPreviewPromise=null){
    ...
    if(remoteRaw){
      const localCardCounts=cloudCacheCounts(localRaw),remoteCardCounts=cloudCacheCounts(remoteRaw),localHasCards=localCardCounts.instances>0||localCardCounts.collection>0,remoteHasCards=remoteCardCounts.instances>0||remoteCardCounts.collection>0||Number(root.collectionDiscovered||0)>0||Number(root.instanceCount||0)>0;
      const localUnsyncedCandidate=meta.saveId===root.saveId&&Number(localRaw?.updatedAt||0)>Number(meta.syncedLocalUpdatedAt||0)+1000;
      // V511: Ein später erzeugter leerer Browser-Zwischenstand darf einen erfolgreich
      // geladenen Cloud-Spielstand niemals als "neuere lokale Änderung" verdrängen.
      const localUnsynced=localUnsyncedCandidate&&(!remoteHasCards||localHasCards);
      ...
      else if(localUnsyncedCandidate&&remoteHasCards&&!localHasCards){
        adoptState(remoteRaw,{saveLocal:true,userId:u.uid});cloudLastSaveId=root.saveId;cloudLastRemoteUpdatedAt=Number(root.updatedAtMs)||0;cloudDirty=false;markCloudCacheVerified(u.uid,root.saveId,cloudLastRemoteUpdatedAt,Number(S.updatedAt)||cloudLastRemoteUpdatedAt,S);
        writeLocalState(u.uid,true);
      }
      else{
    ...
    startRuntimeTimers();void loadRole();if(!cloudMigrationPending)setTimeout(()=>{if(UI.overlay&&cloudReady)void claimPayouts();},900);
  ...
  let payoutPermissionNoticeAt=0;
  async function claimPayouts(){const fb=await firebase(),u=await currentUser();if(!fb||!u||cloudMigrationPending)return false;try{const q=fb.query(fb.collection(fb.db,PAYOUT_COLLECTION,u.uid,"items"),fb.where("status","==","pending"),fb.limit(50)),snap=await fb.getDocs(q);let sum=0,removedCard=false;for(const d of snap.docs){sum+=Math.max(0,Number(d.data().amount)||0);const sold=Object.values(S.instances).find(x=>x.listed===d.id);if(sold){for(const row of S.floors){const i=row.indexOf(sold.id);if(i>=0)row[i]=null}delete S.instances[sold.id];removedCard=true}await fb.updateDoc(d.ref,{status:"claimed",claimedAt:fb.serverTimestamp()})}if(removedCard)invalidateCardPowerCache();if(sum){S.points+=sum;persist();toast(`Marktplatz-Erlös: +${fmt(sum)} Points`)}return true}catch(e){const msg=String(e?.message||e||"").toLowerCase(),permission=String(e?.code||"").includes("permission")||msg.includes("permission")||msg.includes("insufficient");if(permission){if(now()-payoutPermissionNoticeAt>120000){payoutPermissionNoticeAt=now();console.info("BigCards V511: Payout-Lesen wartet auf veröffentlichte BigCards-Firestore-Regeln; der Spielstand selbst bleibt vollständig geladen.");}return false}console.warn("BigCards payout",e);return false}}
