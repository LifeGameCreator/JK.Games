(() => {
  ...
  function ensureLauncher() {
    ...
    wrap.innerHTML = `
      <div><small>HILFE & SUPPORT</small><b>Support-Ticket</b><p>Fehler, verlorene Gegenstände oder Fragen direkt an das JK.Games-Team senden.</p></div>
  ...
  function ensureOverlay() {
    ...
    overlay.innerHTML = `
      <section class="support-ticket-card" role="dialog" aria-modal="true" aria-labelledby="supportTicketTitle">
        <header><div><p class="eyebrow">JK.Games Support</p><h2 id="supportTicketTitle">Support-Tickets</h2></div><button class="icon-button" type="button" data-support-close>×</button></header>
  ...
  async function requireUser() {
    ...
    if (!user && window.LifeBuilderFirebaseCore?.waitForAuth) user = await window.LifeBuilderFirebaseCore.waitForAuth(3500);
    if (user) {
      if (window.LifeBuilderOnline?.verifyDatabase) await window.LifeBuilderOnline.verifyDatabase().catch(() => {});
      return user;
    }
    if (window.LifeBuilderOnline?.showLogin) {
      window.LifeBuilderOnline.showLogin();
      throw new Error("Bitte zuerst im JK.Games-Account anmelden.");
    }
    throw new Error("Bitte zuerst im JK.Games-Account anmelden.");
  }
