...
import { ESCAPE_WORLD_DEFS as WORLD_DEFS, escapeWorldById } from './escape-kl-worlds.js?v=20260818-escape-v486-world-shop-prep';
...
const REVIVE_COSTS = Object.freeze({'keyboard-lab':10,'candy-keys':20,'toxic-keyboard':30});
...
const WORLD_SHOP_TIERS = Object.freeze({
  ...
  'cyber-city':Object.freeze({number:4,name:'World 4 Shop',costMult:12,maxStepTier:7,maxItemLevel:5,maxCoreTier:3,dailyMult:16,next:'World 5',comingSoon:true})
});
...
function defaultProgress(){
  return {
    ...
    worldProgress:{'keyboard-lab':emptyWorldProgress(),'candy-keys':emptyWorldProgress(),'toxic-keyboard':emptyWorldProgress()},worldRebirths:{'keyboard-lab':0,'candy-keys':0,'toxic-keyboard':0},activeTrainingWorld:'keyboard-lab',
...
function normalizeProgress(raw){
  ...
  for(const wid of ['keyboard-lab','candy-keys','toxic-keyboard'])d.worldRebirths[wid]=Math.max(0,Math.floor(Number(d.worldRebirths[wid])||0));
...
function updatePlatforms(t,dt=.016){
  for(const p of G.platforms){
    ...
    if(p.blink){
      // V482: GL/Glitch-Keycaps bleiben IMMER kollidierbar. Der alte Blink-Code
      // schaltete die physische Plattform zeitweise ab und ließ Spieler zufällig
      // hindurchfallen. Ab jetzt ist der Glitch ausschließlich ein Lichteffekt.
      const phase=(t*.55+(Math.abs(p.base.z)%7)*.13)%1,pulse=.72+.28*(.5+.5*Math.sin(phase*Math.PI*2));
      p.active=true;p.mesh.visible=true;p.mesh.material.forEach?.(m=>{m.transparent=true;m.opacity=pulse;});
    }else p.active=true;
...
function rememberRevivePlatform(p){
  if(!isEscapeWorld()||!p||p.scope!==G.world||p.kind==='win-pad'||!p.mesh)return false;
