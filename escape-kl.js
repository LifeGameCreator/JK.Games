...
const REVIVE_COSTS = Object.freeze({'keyboard-lab':10,'candy-keys':20,'toxic-keyboard':30,'world-4':40,'world-5':50});
...
const PET_DEFS = Object.freeze([
  ...
  {id:'free-butterfly',name:'Free Butterfly Pet',cost:60000,currency:'wins',category:'free',speedBonus:.002,winsBonus:.002,asset:'./escape-pet-free-butterfly.glb?v=20260819-escape-v508-butterfly-start-trim',movement:'fly-follow',targetHeight:.62,animationHint:/armature|action|fly|flight|wing/i,animationTrimStart:1,desc:'Animiertes Free-Flug-Pet. V508: 180° korrekt ausgerichtet; diesmal wird die erste Sekunde der Fluganimation übersprungen, während die Schlusssequenz wieder vollständig enthalten ist. Auf jeder Upgrade-Stufe exakt +0,1 Prozentpunkt stärker als das Cat Pet.'},
...
const EGG_WORLD_CONFIG = Object.freeze({
  'keyboard-lab':Object.freeze({world:1,name:'Wind Egg',cost:5000,color:0x58ddff,bonuses:[.0025,.0030,.0035,.0040,.0045]}),
  'candy-keys':Object.freeze({world:2,name:'Candy Egg',cost:80000,color:0xff77bb,bonuses:[.0055,.0062,.0069,.0077,.0085]}),
  'toxic-keyboard':Object.freeze({world:3,name:'Toxic Egg',cost:25000000,color:0x75ff72,bonuses:[.0100,.0110,.0120,.0130,.0140]}),
  'world-4':Object.freeze({world:4,name:'Water Egg',cost:1200000000,color:0x4fd8ff,bonuses:[.0150,.0162,.0174,.0187,.0200]})
});
const EGG_RARITIES = Object.freeze([
  Object.freeze({id:'common',name:'COMMON',weight:45,color:0xc9d2dc}),
  Object.freeze({id:'uncommon',name:'UNCOMMON',weight:28,color:0x70e985}),
  Object.freeze({id:'rare',name:'RARE',weight:16,color:0x66b7ff}),
  Object.freeze({id:'epic',name:'EPIC',weight:8,color:0xc57cff}),
  Object.freeze({id:'legendary',name:'LEGENDARY',weight:3,color:0xffd45c})
]);
const EGG_PET_DEFS = Object.freeze(OWNER_PREVIEW_PETS.map((base,index)=>{
  const worldIndex=Math.floor(index/5),slot=index%5,worldId=['keyboard-lab','candy-keys','toxic-keyboard','world-4'][worldIndex],cfg=EGG_WORLD_CONFIG[worldId],rar=EGG_RARITIES[slot],bonus=cfg.bonuses[slot];
  return Object.freeze({...base,currency:'egg',category:'egg',eggWorldId:worldId,eggWorld:cfg.world,rarity:rar.id,rarityName:rar.name,rarityWeight:rar.weight,rarityColor:rar.color,speedBonus:bonus,winsBonus:bonus,proceduralDefId:base.id,movement:base.float?'fly-follow':'ground-rock',targetHeight:base.float?.78:.72,desc:`${cfg.name} Pet · ${rar.name} · World ${cfg.world}. +${(bonus*100).toFixed(2).replace('.',',')} % Speed und Wins.`});
}));
function petDefById(id){return PET_DEFS.find(p=>p.id===String(id||''))||EGG_PET_DEFS.find(p=>p.id===String(id||''))||null;}
function allPetDefs(){return [...PET_DEFS,...EGG_PET_DEFS];}
...
const DUEL_RACE_MIN_SPEED = 300;
const DUEL_RACE_START_Z = 14;
const DUEL_RACE_FINISH_Z = -166;
const DUEL_RACE_HALF_WIDTH = 8.2;
const DUEL_SHIELD_BASE_COST = 50000;
...
const MOVEMENT_REPRESS_KEYS = Object.freeze(['KeyW','KeyA','KeyS','KeyD']);
const MOVEMENT_REPRESS_KEY_SET = new Set(MOVEMENT_REPRESS_KEYS);
...
const G = {
  ...
  keys:new Set(), physicalMoveKeys:new Set(), movementRepressRequired:false, mobileMoveHeld:false, inputX:0, inputY:0, mobileX:0, mobileY:0, sprint:false,mobileSprint:false,moveIntensity:0,jumpHeld:false,jumpQueuedUntil:0,lastGroundedAt:0,coyoteAvailable:false,
...
function defaultProgress(){
  return {
    version:16,speed:0,wins:0,lifetimeWins:0,stageWinsCollected:0,runPoints:0,rebirths:0,stepButtonTier:0,bestRunCombo:0,
    trail:'none',ownedTrails:['none'],trailPlacement:'feet',aura:'none',ownedAuras:['none'],worldsUnlocked:['keyboard-lab'],
    worldProgress:{'keyboard-lab':emptyWorldProgress(),'candy-keys':emptyWorldProgress(),'toxic-keyboard':emptyWorldProgress(),'world-4':emptyWorldProgress(),'world-5':emptyWorldProgress()},worldRebirths:{'keyboard-lab':0,'candy-keys':0,'toxic-keyboard':0,'world-4':0,'world-5':0},activeTrainingWorld:'keyboard-lab',
    ...
    vendingOwned:false,vendingLevel:0,vendingPotionSpeedUntil:0,vendingPotionSpeedPct:0,vendingPotionWinsUntil:0,vendingPotionWinsPct:0,vendingPotionTreadmillUntil:0,vendingPotionTreadmillPct:0,vendingJkWinsBonus:0,vendingJkWinsCharges:0,vendingDuelShieldCharges:0,
...
function normalizeProgress(raw){
  ...
  for(const wid of ['keyboard-lab','candy-keys','toxic-keyboard','world-4','world-5'])d.worldRebirths[wid]=Math.max(0,Math.floor(Number(d.worldRebirths[wid])||0));
  ...
  d.ownedPets=Array.isArray(d.ownedPets)?[...new Set(d.ownedPets.filter(id=>!!petDefById(id)))]:['none'];
  ...
  d.activePets=[...new Set(requestedPets.filter(id=>id!=='none'&&d.ownedPets.includes(id)&&!!petDefById(id)))].slice(0,petLimit);
  ...
  d.vendingJkWinsCharges=Math.max(0,Math.min(1,Math.floor(Number(d.vendingJkWinsCharges)||0)));d.vendingDuelShieldCharges=Math.max(0,Math.min(1,Math.floor(Number(d.vendingDuelShieldCharges)||0)));
...
function activePetDefs(){
  ...
  return ids.map(id=>petDefById(id)).filter(Boolean).slice(0,maxActivePets());
}
...
function duelRaceWorldIds(){return ['keyboard-lab','candy-keys','toxic-keyboard','world-4'];}
function bestDuelRaceWorldId(){
  let best='keyboard-lab',bestSpeed=-Infinity;
  for(const id of duelRaceWorldIds()){const speed=currentSpeedStat(id);if(speed>bestSpeed){bestSpeed=speed;best=id;}}
  return best;
}
function bestDuelRaceSpeed(){return Math.max(0,currentSpeedStat(bestDuelRaceWorldId()));}
function canEnterDuelRace(){return bestDuelRaceSpeed()+1e-6>=DUEL_RACE_MIN_SPEED;}
function enterDuelRace(){
  const id=bestDuelRaceWorldId(),speed=bestDuelRaceSpeed(),name=escapeWorldById(id)?.name||'Welt';
  if(speed+1e-6<DUEL_RACE_MIN_SPEED)return toast(`🔒 1 VS 1 SPEED RACE · mindestens Speed ${DUEL_RACE_MIN_SPEED} nötig. Dein bester Wert: ${Math.round(speed)} in ${name}.`,'bad',3200),false;
  setWorld('duel-race');return true;
}
...
function escapeStaffRole(){
  try{
    const api=window.JKGamesSettingsMenu||window.LifeBuilderSettingsMenu||window.LifeBuilderOnlineMod;
    const live=String(api?.getRole?.()?.role||api?.getRole?.()?.roleKey||'').toLowerCase();if(live)return live;
    const session=JSON.parse(sessionStorage.getItem('lifebuilder-2026-online-mod-session')||'null');return String(session?.role||session?.roleKey||'').toLowerCase();
  }catch{return ''}
}
function isEscapeOwner(){
  try{const api=window.JKGamesSettingsMenu||window.LifeBuilderSettingsMenu||window.LifeBuilderOnlineMod;return !!(api?.isOwner?.()||escapeStaffRole()==='owner');}catch{return escapeStaffRole()==='owner';}
}
...
function movementSpeedStat(){
  ...
  if(G.world==='only-up')return SKYRUN_SPEED_STAT;
  // 1 VS 1 uses the strongest effective Speed from any Escape world. Bonuses are
  // already included here and must therefore not be applied a second time below.
  if(G.world==='duel-race')return bestDuelRaceSpeed();
...
function movementSpeed(){
  ...
  // SKYRUN ignores normal bonuses; 1 VS 1 already receives its effective best-world
  // Speed through bestDuelRaceSpeed(), so both modes return the calculated value directly.
  return (G.world==='only-up'||G.world==='duel-race')?baseMove:baseMove*(1+totalSpeedBonus()+levelSpeedOverdrivePct(progressWorldId()));
}
...
function addWorldGlbModel({
  ...
  const loadModel=()=>{
    ...
    if(Number(fitWidth)>0&&size.x>1e-6){
      const fitted=Number(fitWidth)/size.x;root.scale.multiplyScalar(fitted);root.updateMatrixWorld(true);
      box=new THREE.Box3().setFromObject(root);size=box.getSize(new THREE.Vector3());
    }
    const center=box.getCenter(new THREE.Vector3());
    if(centerX)root.position.x-=center.x;
    if(centerY)root.position.y-=center.y;
    if(centerZ)root.position.z-=center.z;
    root.updateMatrixWorld(true);
    if(Number.isFinite(Number(floorY))){
      box=new THREE.Box3().setFromObject(root);
      root.position.y+=Number(floorY)-box.min.y;
      root.updateMatrixWorld(true);
    }
...
function duelHazardRand(h){h.seed=((Number(h.seed)||1)*1664525+1013904223)>>>0;return h.seed/4294967296;}
function scheduleDuelDropHazard(h,now=performance.now(),first=false){
  if(!h)return;const rx=duelHazardRand(h),rz=duelHazardRand(h);
  const x=h.centerX+(rx-.5)*h.areaW,z=h.centerZ+(rz-.5)*h.areaD;
  h.marker.position.set(x,h.markerY,z);h.mesh.position.set(x,h.topY,z);
  h.whiteMs=520+duelHazardRand(h)*650;h.redMs=170+duelHazardRand(h)*230;h.fallMs=260+duelHazardRand(h)*230;h.impactMs=150+duelHazardRand(h)*180;h.gapMs=140+duelHazardRand(h)*430;
  h.cycleStartAt=now+(first?duelHazardRand(h)*1250:h.gapMs);h.started=true;h.active=false;h.marker.visible=false;h.mesh.visible=false;h.phase='wait';
}
function addDuelDropHazard({centerX=0,centerZ=-82,areaW=14.6,areaD=142,markerY=.78,topY=12.5,impactY=1.30,seed=1}={}){
  const markerMat=new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:.95,depthWrite:false,toneMapped:false});G.materials.set(`duel-x-${seed}`,markerMat);
  const marker=tagScope(new THREE.Group());marker.visible=false;G.scene.add(marker);G.decorative.push(marker);
  for(const rot of[Math.PI/4,-Math.PI/4]){const bar=new THREE.Mesh(geo('duel-warning-x-bar',()=>new THREE.BoxGeometry(1.65,.055,.24)),markerMat);bar.rotation.y=rot;marker.add(bar);}
  const meteorMat=new THREE.MeshStandardMaterial({color:0x263643,emissive:0xff4b42,emissiveIntensity:.82,roughness:.38,metalness:.48});G.materials.set(`duel-drop-${seed}`,meteorMat);
  const mesh=tagScope(new THREE.Mesh(geo('duel-drop-block',()=>new THREE.DodecahedronGeometry(.78,0)),meteorMat));mesh.visible=false;mesh.castShadow=true;G.scene.add(mesh);G.decorative.push(mesh);
  const h={mesh,marker,markerMat,scope:G.buildScope,kind:'duel-drop',duelDropHazard:true,w:1.25,h:1.55,d:1.25,active:false,started:true,centerX,centerZ,areaW,areaD,markerY,topY,impactY,seed:Number(seed)||1,cycleStartAt:0,whiteMs:0,redMs:0,fallMs:0,impactMs:0,gapMs:0,phase:'wait'};
  scheduleDuelDropHazard(h,performance.now(),true);G.hazards.push(h);return h;
}
...
function resetHazardsForWorld(scope=G.world){
  ...
  for(const h of G.hazards){
    ...
    if(h.duelDropHazard){scheduleDuelDropHazard(h,now,true);h.marker.visible=false;h.mesh.visible=false;h.active=false;}
    if(!h.mazeBoss&&!h.pulseHazard&&!h.duelDropHazard)h.mesh.visible=h.scope===G.world&&!h.hiddenUntilStart;
  }
...
function updateHazards(dt){
  ...
  for(const h of G.hazards){
    if(h.scope!==G.world){h.mesh.visible=false;if(h.marker)h.marker.visible=false;continue;}

    if(h.duelDropHazard){
      if(G.duelRacePhase!=='running'){h.marker.visible=false;h.mesh.visible=false;h.active=false;continue;}
      if(!h.started)scheduleDuelDropHazard(h,now,true);
      if(now<h.cycleStartAt){h.marker.visible=false;h.mesh.visible=false;h.active=false;continue;}
      const elapsed=now-h.cycleStartAt,total=h.whiteMs+h.redMs+h.fallMs+h.impactMs;
      if(elapsed>=total){scheduleDuelDropHazard(h,now,false);continue;}
      h.marker.visible=true;
      if(elapsed<h.whiteMs){h.phase='white';h.markerMat.color.setHex(0xffffff);h.mesh.visible=false;h.active=false;}
      else if(elapsed<h.whiteMs+h.redMs){h.phase='red';h.markerMat.color.setHex(0xff3344);h.mesh.visible=false;h.active=false;}
      else{
        h.phase='drop';h.markerMat.color.setHex(0xff3344);h.mesh.visible=true;h.active=true;
        const fallElapsed=elapsed-h.whiteMs-h.redMs,progress=Math.max(0,Math.min(1,fallElapsed/Math.max(1,h.fallMs)));
        h.mesh.position.y=h.topY+(h.impactY-h.topY)*(1-Math.pow(1-progress,2));h.mesh.rotation.x+=dt*4.2;h.mesh.rotation.z+=dt*3.1;
        if(progress>=1)h.mesh.position.y=h.impactY;
      }
    }else if(h.mazeBoss){
    ...
    if(!reviveProtected&&hazardTouchesPlayer(h)){
      if(h.duelDropHazard){
        if(performance.now()<Number(G.duelShieldInvulnerableUntil||0)){scheduleDuelDropHazard(h,now,false);continue;}
        if(consumeDuelShieldHit()){scheduleDuelDropHazard(h,now,false);return true;}
        toast('💥 TREFFER · zurück an den Start!','bad',1300);resetDuelRaceRun('Treffer');return true;
      }
...
function eggConfigForWorld(worldId=G.world){return EGG_WORLD_CONFIG[String(worldId||'')]||null;}
function eggPetsForWorld(worldId=G.world){return EGG_PET_DEFS.filter(p=>p.eggWorldId===String(worldId||''));}
function weightedEggPet(pool){
  const total=pool.reduce((n,p)=>n+Math.max(1,Number(p.rarityWeight)||1),0);let roll=Math.random()*total;
  for(const p of pool){roll-=Math.max(1,Number(p.rarityWeight)||1);if(roll<=0)return p;}return pool[pool.length-1]||null;
}
function openWorldPetEgg(worldId=G.world){
  const cfg=eggConfigForWorld(worldId);if(!cfg)return toast('In diesem Bereich gibt es kein World-Ei.','bad',1800);
  const pets=eggPetsForWorld(worldId),owned=pets.filter(p=>G.state.ownedPets.includes(p.id));
  const wrap=openModal(`<div class="ekl-modal"><div class="ekl-modal-head"><div><small>WORLD ${cfg.world} · PET EGG</small><h2>🥚 ${cfg.name}</h2><p>Jede Welt besitzt fünf eigene Pets. Spätere Welten geben deutlich stärkere Begleiter. Das Ei kostet <b>${cfg.cost.toLocaleString('de-DE')} Wins</b>.</p></div><button data-ekl-modal-close>×</button></div><div class="ekl-big-stat"><div><small>GESAMMELT</small><b>${owned.length}/5</b></div><div><small>KOSTEN</small><b>${fmt(cfg.cost)}</b></div><div><small>WELT</small><b>${cfg.world}</b></div></div><div class="ekl-shop-grid">${pets.map(p=>`<article class="${G.state.ownedPets.includes(p.id)?'owned':''}"><small>${p.rarityName}</small><h3>${G.state.ownedPets.includes(p.id)?p.name:'???'}</h3><p>${G.state.ownedPets.includes(p.id)?`+${petPercent(p.speedBonus)} Speed / Wins`:'Noch nicht entdeckt'}</p></article>`).join('')}</div><div class="ekl-modal-actions"><button class="gold" data-ekl-open-world-egg>🥚 Ei öffnen · ${cfg.cost.toLocaleString('de-DE')} Wins</button><button data-ekl-modal-close>Schließen</button></div></div>`);
  wrap.querySelector('[data-ekl-open-world-egg]')?.addEventListener('click',()=>hatchWorldPetEgg(worldId));
}
function hatchWorldPetEgg(worldId=G.world){
  const cfg=eggConfigForWorld(worldId),pets=eggPetsForWorld(worldId);if(!cfg||!pets.length)return false;
  if(G.state.wins<cfg.cost)return toast(`Du brauchst ${cfg.cost.toLocaleString('de-DE')} Wins für das ${cfg.name}.`,'bad',2200),false;
  G.state.wins-=cfg.cost;
  const missing=pets.filter(p=>!G.state.ownedPets.includes(p.id)),pool=missing.length?missing:pets,pet=weightedEggPet(pool);if(!pet)return false;
  const isNew=!G.state.ownedPets.includes(pet.id);if(isNew)G.state.ownedPets.push(pet.id);else{const refund=Math.floor(cfg.cost*.25);G.state.wins+=refund;}
  queuePersist(50);updateHud(true);soundBuy();refreshCompanionVisuals();
  const wrap=openModal(`<div class="ekl-modal"><div class="ekl-modal-head"><div><small>🥚 EI GEÖFFNET · WORLD ${cfg.world}</small><h2>${isNew?'✨ NEUES PET':'♻ DUPLIKAT'} · ${pet.name}</h2><p>${pet.rarityName} · <b>+${petPercent(pet.speedBonus)} Speed</b> · <b>+${petPercent(pet.winsBonus)} Wins</b>${isNew?'':`<br>Du besitzt es schon – 25 % des Ei-Preises wurden zurückgegeben.`}</p></div><button data-ekl-modal-close>×</button></div><div class="ekl-world-economy-note"><b>${pet.name}</b><span>Dieses Pet ist komplett prozedural programmiert und kann sofort als Begleiter ausgerüstet werden.</span></div><div class="ekl-modal-actions"><button class="gold" data-ekl-hatch-equip>🪽 ${petEquipped(pet.id)?'Ablegen':'Ausrüsten'}</button><button data-ekl-open-again>🥚 Noch ein Ei</button><button data-ekl-modal-close>Schließen</button></div></div>`);
  wrap.querySelector('[data-ekl-hatch-equip]')?.addEventListener('click',()=>buyOrEquipPet(pet.id,'menu'));
  wrap.querySelector('[data-ekl-open-again]')?.addEventListener('click',()=>openWorldPetEgg(worldId));return true;
}
function buildWorldPetEggStation(worldId){
  const cfg=eggConfigForWorld(worldId);if(!cfg)return;const x=-10.5,z=-62.5,floorY=.38;
  boxDeco(x,floorY,z,5.0,.55,4.5,0x23323b,cfg.color);addRingDeco(x,1.05,z,1.25,.10,cfg.color,0,Math.PI/2,0);
  const material=mat(`egg-world-${cfg.world}`,{color:cfg.color,emissive:cfg.color,emissiveIntensity:.16,roughness:.38,metalness:.08});
  const egg=tagScope(new THREE.Mesh(geo(`egg-world-geo-${cfg.world}`,()=>new THREE.SphereGeometry(1,18,14)),material),worldId);egg.position.set(x,1.72,z);egg.scale.set(.78,1.12,.78);G.scene.add(egg);G.decorative.push(egg);
  addGlowLight(x,2.0,z,cfg.color,.75,8);addSign(`WORLD ${cfg.world} PET EGG`,{x,y:3.65,z:z+.15},cfg.color,.34);addSign(`${cfg.cost.toLocaleString('de-DE')} WINS`,{x,y:3.08,z:z+.15},0xffffff,.22);
  addInteractable(`pet-egg-${worldId}`,`${cfg.name} öffnen`,x,1.2,z,3.1,()=>openWorldPetEgg(worldId));
}
...
function refreshHubWorldPortalStatus(){
  ...
  for(const obj of G.decorative){
    ...
    if(statusId){
      ...
      const text=w.number===1?'BETRETEN':unlocked?'FREIGESCHALTET':status.requiredRuns>0?`LEVEL ${status.requiredLevel} + VORWELT`:`LEVEL ${status.requiredLevel} IN ${escapeWorldById(status.sourceId)?.name?.toUpperCase()||'VORWELT'}`;
...
function buildHub(){
  ...
  // --- WORLD STREET: World 5 is now the Galaxy preview. World 6 stays Coming Soon. ---
  const worldHouses=[
    ...
    {id:'world-5',number:5,name:'GALAXY WORLD',ownerOnly:false,x:31,w:11,d:7,h:7.1,wall:0x27213a,roof:0x151022,accent:0xa978ff},
  ...
  const utilityHouses=[
    ...
    {id:'duel-race-house',label:'1 VS 1 SPEED RACE',x:47,w:9,d:7,h:6.2,wall:0xd4d8df,roof:0x394956,accent:0x66cfff,onEnter:enterDuelRace}
  ];
  for(const h of utilityHouses){
    // V508: keine kleinen Gebäudeschilder mehr. Alle Namen stehen groß über den
    // Funktionshäusern – genau wie bei den World-Häusern auf der Gegenseite.
    const b=addHubBuilding({x:h.x,z:50,w:h.w,d:h.d,h:h.h,wall:h.wall,roof:h.roof,accent:h.accent,label:''});
    addHousePath(b);addSign(h.label,{x:b.x,y:b.h+1.65,z:b.z-4.05},h.accent,h.label.length>13?.48:.66);buildingDoorTrigger(h.id,b,h.onEnter);
  }
...
function ensureDuelRaceShieldVisual(){
  if(!G.playerRoot)return null;
  if(G.duelRaceShieldVisual&&G.duelRaceShieldVisual.parent!==G.playerRoot){G.duelRaceShieldVisual.removeFromParent?.();G.duelRaceShieldVisual=null;}
  if(G.duelRaceShieldVisual)return G.duelRaceShieldVisual;
  const material=new THREE.MeshBasicMaterial({color:0x4aa8ff,transparent:true,opacity:.17,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending,toneMapped:false});G.materials.set(`duel-shield-${Date.now()}`,material);
  const sphere=new THREE.Mesh(new THREE.SphereGeometry(1.05,22,16),material);sphere.name='escape-duel-magic-shield';sphere.scale.set(1,1.42,1);sphere.position.y=.02;sphere.visible=false;sphere.renderOrder=7;G.playerRoot.add(sphere);G.duelRaceShieldVisual=sphere;return sphere;
}
function updateDuelRaceShieldVisual(t=0){
  const show=G.world==='duel-race'&&G.duelRacePhase==='running'&&Number(G.state?.vendingDuelShieldCharges||0)>0;
  const sphere=show?ensureDuelRaceShieldVisual():G.duelRaceShieldVisual;if(!sphere)return;
  sphere.visible=show;if(show){const pulse=1+Math.sin(t*5.4)*.035;sphere.scale.set(pulse,1.42*pulse,pulse);sphere.rotation.y=t*.38;sphere.material.opacity=.15+Math.sin(t*4.2)*.035;}
}
function consumeDuelShieldHit(){
  if(G.world!=='duel-race'||G.duelRacePhase!=='running'||Number(G.state?.vendingDuelShieldCharges||0)<=0)return false;
  G.state.vendingDuelShieldCharges=0;G.duelShieldInvulnerableUntil=performance.now()+1100;queuePersist(50);updateDuelRaceShieldVisual();tone(660,.15,'sine',.026,180);toast('🔵 ZAUBERSCHUTZ! Ein Treffer wurde komplett abgefangen.','good',2200);return true;
}
function buildDuelRaceCourse(){
  const floorY=.40,topY=floorY+.56/2;
  addPlatform({x:0,y:floorY,z:29,w:18,h:.56,d:16,color:0x243544,label:'LOBBY',kind:'duel-lobby'});
  addSign('1 VS 1 SPEED RACE',{x:0,y:5.3,z:36.3},0x66cfff,.95);addSign('MINIMUM SPEED 300',{x:0,y:4.05,z:36.25},0xffd56b,.42);
  boxDeco(0,topY+.05,18,17.2,.08,.22,0x66cfff,0x66cfff);boxDeco(0,topY+.05,-166,17.2,.08,.30,0xffd05d,0xff8d31);
  addSign('START',{x:0,y:2.9,z:17.7},0x66cfff,.48);addSign('FINISH',{x:0,y:3.0,z:-169.7},0xffd05d,.52);
  // Kleine Lobby: hier kann die eigene Vending Machine platziert und der
  // Zauberschutztrank vor dem eigentlichen Lauf getrunken werden.
  boxDeco(-6.4,1.15,31.8,3.3,1.7,.35,0x1b2730);addSign('VENDING ZONE',{x:-6.4,y:2.45,z:31.55},0x75d8ff,.30);
  addInteractable('duel-race-console','1 VS 1 / SOLO starten',0,1.35,25.2,3.2,openDuelRaceStartMenu);
  addInteractable('duel-race-return','Zum Hub zurück',6.2,1.25,31.4,2.6,()=>setWorld('hub'));
  // Lange, gerade Speed-Strecke mit breiter Ausweichzone.
  addPlatform({x:0,y:floorY,z:-76,w:18,h:.56,d:184,color:0x17242d,label:'',kind:'duel-track'});
  for(const x of[-9.15,9.15]){boxDeco(x,1.10,-76,.28,1.05,184,0x293e4b);for(let z=10;z>=-164;z-=10)addGlowLight(x*.94,1.35,z,0x56c9ff,.26,6);}
  for(let z=10;z>=-160;z-=8){boxDeco(0,topY+.04,z,.12,.035,3.6,z%16===10?0xf3f5f8:0x8ea7b5);}
  for(let z=4;z>=-156;z-=20){addSign(`${Math.abs(z-14)}m`,{x:-6.9,y:1.65,z},0x9fb4c4,.18);}
  // Viele unabhängige Warnfelder: X wird weiß, dann rot, danach fällt der Block.
  for(let i=0;i<13;i++)addDuelDropHazard({centerX:0,centerZ:-77,areaW:14.4,areaD:150,markerY:topY+.035,topY:12.8+(i%3)*.8,impactY:topY+.82,seed:50817+i*7919});
  addAutoTrigger('duel-race-finish',0,DUEL_RACE_FINISH_Z,17.5,4.0,()=>{if(G.duelRacePhase==='running')finishDuelRace();});
}
function openDuelRaceStartMenu(){
  const speed=bestDuelRaceSpeed(),world=escapeWorldById(bestDuelRaceWorldId())?.name||'Welt',shield=Number(G.state?.vendingDuelShieldCharges||0)>0;
  const wrap=openModal(`<div class="ekl-modal"><div class="ekl-modal-head"><div><small>1 VS 1 SPEED RACE · LOBBY</small><h2>⚔️ SPEED RACE</h2><p>Du brauchst mindestens <b>Speed ${DUEL_RACE_MIN_SPEED}</b>. Auf der Strecke erscheinen weiße X-Markierungen. Werden sie rot, schlägt dort direkt danach ein Hindernis ein. Treffer = Neustart am Anfang.</p></div><button data-ekl-modal-close>×</button></div><div class="ekl-big-stat"><div><small>BESTER SPEED</small><b>${Math.round(speed)}</b></div><div><small>QUELLE</small><b>${world}</b></div><div><small>ZAUBERSCHUTZ</small><b>${shield?'1 TREFFER':'KEINER'}</b></div></div><div class="ekl-world-economy-note"><b>🥤 Vending-Tipp</b><span>Platziere deine Vending Machine in dieser Lobby und trinke den Zauberschutztrank. Die blaue Schutzkugel erscheint erst nach dem Rennstart und fängt genau einen Treffer ab.</span></div><div class="ekl-modal-actions"><button class="gold" data-duel-start="duel">⚔️ 1 VS 1 STARTEN</button><button data-duel-start="solo">🏃 SOLO TRAINING</button><button data-ekl-modal-close>Schließen</button></div></div>`);
  wrap.querySelectorAll('[data-duel-start]').forEach(b=>b.onclick=()=>startDuelRace(b.dataset.duelStart));
}
function startDuelRace(mode='solo'){
  if(!canEnterDuelRace())return closeModal(),enterDuelRace();
  G.overlay?.querySelector('[data-ekl-complete]')?.remove();closeModal();resetMovementControls(true);removeSummonedVending();
  G.duelRaceMode=mode==='duel'?'duel':'solo';G.duelRacePhase='countdown';G.runFinished=false;G.duelRaceHits=0;G.deaths=0;G.stage=1;G.paused=true;G.checkpoint={x:0,y:1.52,z:DUEL_RACE_START_Z};teleport(0,1.52,DUEL_RACE_START_Z);resetHazardsForWorld('duel-race');updateDuelRaceShieldVisual();
  toast('3 …','good',650);
  setTimeout(()=>{if(G.world==='duel-race'&&G.duelRacePhase==='countdown')toast('2 …','good',650)},700);
  setTimeout(()=>{if(G.world==='duel-race'&&G.duelRacePhase==='countdown')toast('1 …','good',650)},1400);
  setTimeout(()=>{if(G.world!=='duel-race'||G.duelRacePhase!=='countdown')return;G.duelRacePhase='running';G.paused=false;G.runStartedAt=performance.now();resetHazardsForWorld('duel-race');updateDuelRaceShieldVisual();toast('GO! · WEISSES X → ROTES X → EINSCHLAG','good',1800);tone(620,.10,'square',.018,160);},2100);
}
function resetDuelRaceRun(reason='Treffer'){
  if(G.world!=='duel-race')return false;G.duelRaceHits++;G.deaths++;G.runFinished=false;G.duelRacePhase='running';G.runStartedAt=performance.now();G.duelShieldInvulnerableUntil=performance.now()+650;resetHazardsForWorld('duel-race');teleport(0,1.52,DUEL_RACE_START_Z);resetMovementControls(true);toast(`💥 ${reason} · Versuch ${G.duelRaceHits+1} startet wieder ganz vorne.`,'bad',1800);return true;
}
function finishDuelRace(){
  if(G.world!=='duel-race'||G.duelRacePhase!=='running'||G.runFinished)return false;
  G.runFinished=true;G.duelRacePhase='finished';G.paused=true;updateDuelRaceShieldVisual();soundFinish();const sec=Math.max(0,(performance.now()-G.runStartedAt)/1000),mode=G.duelRaceMode==='duel'?'duel':'solo',key=`duel-speed-race-${mode}`,previous=Number(G.state.bestTimes[key]||0),record=!previous||sec<previous;if(record)G.state.bestTimes[key]=sec;G.state.completions[key]=Math.max(0,Number(G.state.completions[key]||0))+1;queuePersist(50);
  const wrap=document.createElement('div');wrap.className='ekl-complete';wrap.dataset.eklComplete='1';wrap.innerHTML=`<div class="ekl-complete-card"><small>${mode==='duel'?'1 VS 1 SPEED RACE':'SOLO SPEED TRAINING'}</small><h2>${record?'🏆 Neue Bestzeit!':'⚔️ Strecke geschafft!'}</h2><div class="ekl-stars">${record?'★★★':'★★☆'}</div><p>Speed <b>${Math.round(bestDuelRaceSpeed())}</b> · Zeit <b>${timeText(sec)}</b>${previous?` · Vorher ${timeText(previous)}`:''}<br>Neustarts durch Treffer <b>${G.duelRaceHits}</b> · Schutztrank <b>${Number(G.state.vendingDuelShieldCharges||0)>0?'noch geladen':'verbraucht / keiner'}</b></p><div class="ekl-modal-actions"><button data-duel-again class="gold">Nochmal</button><button data-duel-lobby>Lobby</button><button data-duel-hub>Zum Hub</button></div></div>`;G.overlay.append(wrap);wrap.querySelector('[data-duel-again]').onclick=()=>{wrap.remove();startDuelRace(mode)};wrap.querySelector('[data-duel-lobby]').onclick=()=>{wrap.remove();setWorld('duel-race')};wrap.querySelector('[data-duel-hub]').onclick=()=>{wrap.remove();setWorld('hub')};updateHud(true);return true;
}
...
function petPlaybackClip(def,clips=[]){
  const clip=petAnimationClip(def,clips);if(!clip)return null;
  const trimStart=Math.max(0,Number(def?.animationTrimStart)||0),trimEnd=Math.max(0,Number(def?.animationTrimEnd)||0),duration=Math.max(0,Number(clip.duration)||0);
  if((trimStart>0||trimEnd>0)&&duration>trimStart+trimEnd+.18){
    const shortened=clip.clone(),end=Math.max(trimStart+.18,duration-trimEnd);
    shortened.name=`${clip.name||def.id}-v508-loop`;
    shortened.tracks=(shortened.tracks||[]).map(track=>{const t=track.clone();t.trim(trimStart,end);t.shift(-trimStart);return t;});
    shortened.duration=Math.max(.18,end-trimStart);
    return shortened;
  }
  return clip;
}
function loadPetVisuals(){
  ...
  const finalizeProcedural=(pet,slot)=>{
    if(seq!==G.petLoadSeq||!G.scene||!activePetDefs().some(p=>p.id===pet.id))return;
    const def=ownerPreviewPetDef(pet.proceduralDefId||pet.id);if(!def)return;
    const built=createOwnerPreviewPetModel(def),wrap=new THREE.Group();wrap.name=`escape-pet-wrapper-${pet.id}`;
    const model=built.root;model.position.set(0,0,0);model.rotation.y=Math.PI;model.scale.setScalar(.48);wrap.add(model);G.scene.add(wrap);
    const visual={def:pet,slot,wrapper:wrap,model,mixer:null,action:null,groundMoving:false,bindPose:null,idleRig:null,rockBase:model.rotation.clone(),procRig:built.rig};G.petVisuals.push(visual);
    const yaw=G.playerRoot?.rotation.y||0,forwardX=Math.sin(yaw),forwardZ=Math.cos(yaw),layout=petFollowerLayout(slot,pets.length),side=layout.side,rightX=Math.cos(yaw),rightZ=-Math.sin(yaw),back=pet.movement==='fly-follow'?layout.back+.60:layout.back;
  ...
  pets.forEach((pet,slot)=>{
    if(pet?.proceduralDefId){finalizeProcedural(pet,slot);return;}
    if(!pet?.asset)return;
    createSharedLoader().load(pet.asset,gltf=>{
      ...
      if(pet.id==='reptisect')model.rotation.y+=Math.PI;
      if(pet.id==='free-butterfly')model.rotation.y+=Math.PI; // V504: Butterfly-Front war exakt entgegengesetzt ausgerichtet.
      ...
      if(pet.id==='reptisect'){
        ...
        model.traverse(node=>{if(!node?.isBone)return;bindPose.push({node,position:node.position.clone(),quaternion:node.quaternion.clone(),scale:node.scale.clone()});const name=String(node.name||'');if(/^head_/i.test(name))head=node;if(/^tail \d+_/i.test(name))tails.push(node);});
        tails.sort((a,b)=>(parseInt(String(a.name).match(/^tail (\d+)_/i)?.[1]||'0',10)-parseInt(String(b.name).match(/^tail (\d+)_/i)?.[1]||'0',10)));idleRig={head,tails,headBase:head?.rotation.clone?.()||null,tailBases:tails.map(node=>node.rotation.clone())};
      }
      const clips=gltf.animations||[],clip=petPlaybackClip(pet,clips);let mixer=null,action=null;
...
function updateCustomVisuals(dt,t){
  ...
  for(const visual of G.petVisuals||[]){
    ...
    if(visual.procRig){
      for(const wingData of visual.procRig.wings||[]){const {obj,base,side,amp}=wingData;obj.rotation.copy(base);obj.rotation.z+=side*Math.sin(t*7.4+slot*.7)*(amp||.55);}
      for(const tailData of visual.procRig.tails||[]){const {obj,base,amp}=tailData;obj.rotation.copy(base);obj.rotation.y+=Math.sin(t*3.4+slot*.5)*(amp||.28);}
      for(const spin of visual.procRig.spin||[])spin.rotation.y+=dt*1.25;
    }
...
function clearCharacterOnly(){
  if(G.duelRaceShieldVisual){G.duelRaceShieldVisual.removeFromParent?.();G.duelRaceShieldVisual=null;}
  clearCustomVisuals();
...
function petCardHtml(p){
  ...
  if(p.currency==='egg')return `<article class="${owned?'owned':''}"><small>🥚 WORLD ${p.eggWorld} · ${p.rarityName}</small><h3>${p.name}</h3><p>${p.desc}<br><b>+${petPercent(bonus.speed)} Speed</b> · <b>+${petPercent(bonus.wins)} Wins</b></p><button data-ekl-pet-pick="${p.id}" ${owned?'':'disabled'}>${active?'Ablegen':owned?'Ausrüsten':'Noch nicht aus Ei erhalten'}</button></article>`;
...
function buyOrEquipPet(id,returnTo='menu'){
  const pet=petDefById(id);if(!pet||id==='none')return false;
  ...
  if(!G.state.ownedPets.includes(id)){
    ...
    }else if(pet.currency==='egg')return toast(`${pet.name} bekommst du ausschließlich aus dem World-${pet.eggWorld}-Ei.`,'bad',2100),false;
...
function equipBestPets(){
  const owned=allPetDefs().filter(p=>p.id!=='none'&&G.state.ownedPets.includes(p.id)).sort((a,b)=>{const A=petEffectiveBonuses(a),B=petEffectiveBonuses(b);return ((B.speed+B.wins)-(A.speed+A.wins))||(B.speed-A.speed)||(B.wins-A.wins);});
...
function openPetEquipMenu(){
  ...
  const active=activePetDefs(),limit=maxActivePets(),free=PET_DEFS.filter(p=>p.id!=='none'&&(p.category==='free'||p.currency==='wins')),paid=PET_DEFS.filter(p=>p.id!=='none'&&p.currency==='jk'),eggPets=EGG_PET_DEFS.filter(p=>G.state.ownedPets.includes(p.id));
  ...
  const wrap=openModal(`<div class="ekl-modal"><div class="ekl-modal-head"><div><small>ESCAPE.KL · PETS</small><h2>🪽 Pets ausrüsten</h2><p>Standardmäßig sind zwei Pets gleichzeitig möglich. Der dritte Slot ist eine dauerhafte JK/Coin-Freischaltung. Free Pets kaufst und levelst du weiterhin mit Wins. Zusätzlich stehen in World 1–4 eigene Eier mit je fünf sammelbaren Pets.</p></div><button data-ekl-modal-close>×</button></div><div class="ekl-big-stat"><div><small>SLOTS</small><b>${active.length}/${limit}</b></div><div><small>PET-SPEED</small><b>+${petPercent(petBonusTotals().speed)}</b></div><div><small>PET-WINS</small><b>+${petPercent(petBonusTotals().wins)}</b></div></div>${slotHtml}<div class="ekl-modal-actions"><button class="gold" data-ekl-best-pets>⭐ Beste Pets</button><button data-ekl-clear-pets>Alle ablegen</button><button class="jk" data-ekl-pets-jk-shop>◆ JK-Shop</button></div><div class="ekl-character-special-head"><b>🆓 Free Pets</b><span>Kostenlos erspielte Pets</span></div>${free.length?`<div class="ekl-shop-grid">${free.map(petCardHtml).join('')}</div>`:`<div class="ekl-world-economy-note"><b>Noch keine Free Pets im aktuellen Katalog.</b><span>Die Kategorie ist vorbereitet; spätere kostenlose Pets erscheinen automatisch hier.</span></div>`}<div class="ekl-character-special-head"><b>🥚 World-Egg Pets</b><span>20 neue Pets · 5 pro Welt · spätere Welten sind stärker</span></div>${eggPets.length?`<div class="ekl-shop-grid">${eggPets.map(petCardHtml).join('')}</div>`:`<div class="ekl-world-economy-note"><b>Noch kein World-Egg Pet erhalten.</b><span>In jeder Escape-Welt steht direkt beim Start ein Ei. Öffne es mit E und sammle die fünf Pets der jeweiligen Welt.</span></div>`}<div class="ekl-character-special-head"><b>◆ JK Pets</b><span>Premium-Pets · dauerhaft im Besitz</span></div><div class="ekl-shop-grid">${paid.map(petCardHtml).join('')}</div><div class="ekl-world-economy-note"><b>Aktiv:</b><span>${petSelectionSummary()}</span></div></div>`);
...
function respawn(){
  ...
  if(G.world==='duel-race'){resetDuelRaceRun('Sturz');return;}
  if(G.world==='only-up'){
...
function vendingShieldCost(){
  const level=Math.max(1,vendingLevel()),tierNumber=Math.max(1,Math.min(4,Number(shopTier(progressWorldId())?.number)||1));
  return Math.max(5000,Math.round(DUEL_SHIELD_BASE_COST*(VENDING_WORLD_COST_MULT[tierNumber]||1)*(1+(level-1)*.20)));
}
...
function activateDuelShieldPotion(){
  if(!G.state?.vendingOwned)return false;
  if(Number(G.state.vendingDuelShieldCharges||0)>0)return toast('🔵 Zauberschutz ist bereits geladen · genau ein Treffer wird abgefangen.','good',1900),false;
  const cost=vendingShieldCost();if(G.state.wins<cost)return toast(`Du brauchst ${cost.toLocaleString('de-DE')} Wins für den Zauberschutztrank.`,'bad',2000),false;
  G.state.wins-=cost;G.state.vendingDuelShieldCharges=1;soundBuy();queuePersist(50);updateHud(true);updateDuelRaceShieldVisual();toast('🔵 Zauberschutz geladen · im laufenden 1 VS 1 Race erscheint die blaue Schutzkugel und fängt genau einen Treffer ab.','good',3000);openVendingMachine();return true;
}
...
function processMovement(dt,t){
  ...
  else if(G.world==='duel-race'){
    G.pos.x=Math.max(-DUEL_RACE_HALF_WIDTH,Math.min(DUEL_RACE_HALF_WIDTH,G.pos.x));G.pos.z=Math.max(DUEL_RACE_FINISH_Z-5,Math.min(36,G.pos.z));resolveScopedColliders(prevX,prevZ);
...
function updateHud(force=false){
  ...
  const worldId=progressWorldId(),lp=levelProgress(worldId),nextRb=nextRebirthDef(),speed=G.world==='duel-race'?bestDuelRaceSpeed():currentSpeedStat(worldId),displaySpeedCap=G.world==='only-up'?SKYRUN_SPEED_STAT:REGULAR_SPEED_CAP;
...
function releaseMovementRepressIfReady(){
  if(!G.movementRepressRequired)return false;
  if((G.physicalMoveKeys?.size||0)>0||G.mobileMoveHeld)return false;
  G.movementRepressRequired=false;
  return true;
}
function armMovementRepressLock(){
  const held=(G.physicalMoveKeys?.size||0)>0||!!G.mobileMoveHeld;
  G.movementRepressRequired=held;
  for(const code of MOVEMENT_REPRESS_KEYS)G.keys?.delete?.(code);
  G.mobileX=0;G.mobileY=0;
  if(G.moveVel){G.moveVel.x=0;G.moveVel.z=0;}
  return held;
}
function resetMovementControls(stopVelocity=true,clearPhysical=false){
  if(clearPhysical){G.physicalMoveKeys?.clear?.();G.mobileMoveHeld=false;G.movementRepressRequired=false;}else armMovementRepressLock();
...
function openOwnerPetLab(){
  ...
  const cards=OWNER_PREVIEW_PETS.map((pet,i)=>{const egg=EGG_PET_DEFS.find(p=>p.id===pet.id);return `<article class="${G.ownerPetPreviewId===pet.id?'owned':''}"><small>OWNER TEST PET ${String(i+1).padStart(2,'0')} · WORLD ${egg?.eggWorld||'?'} · ${egg?.rarityName||''}</small><h3>${pet.name}</h3><p>Komplett prozedural gebaut · ${pet.float?'Flug-/Schwebeform':'Bodenform'} · später aus dem ${eggConfigForWorld(egg?.eggWorldId)?.name||'World-Ei'} · +${petPercent(egg?.speedBonus||0)} Speed/Wins.</p><button class="owner" data-mod-preview-pet="${pet.id}">${G.ownerPetPreviewId===pet.id?'AKTIV · erneut ansehen':'In Pet verwandeln'}</button></article>`;}).join('');
...
function showPause(){
  ...
  G.overlay.querySelector('[data-ekl-owner-pet-lab]')?.addEventListener('click',openOwnerPetLab);G.overlay.querySelector('[data-ekl-owner-mod]')?.addEventListener('click',openOwnerModMenu);
}
...
function bindInput(){
  G.keyDown=e=>{ensureAudio();if(['KeyW','KeyA','KeyS','KeyD','Space','ShiftLeft','ShiftRight','ControlLeft','ControlRight','KeyE','KeyR','KeyF','KeyG','Escape'].includes(e.code))e.preventDefault();if(MOVEMENT_REPRESS_KEY_SET.has(e.code)){G.physicalMoveKeys.add(e.code);if(G.movementRepressRequired){G.keys.delete(e.code);return;}}if(e.code==='Escape'){if(G.modalOpen){closeModal();G.paused=false}else showPause();return}if(isEscapeOwner()&&!G.modalOpen&&!e.repeat&&e.code==='KeyF'){setOwnerFly(!G.ownerFlyActive);return}if(isEscapeOwner()&&!G.modalOpen&&!e.repeat&&e.code==='KeyG'){setOwnerVanish(!G.ownerVanish);return}if(e.code==='Space'){if(G.ownerFlyActive){G.keys.add('Space');return}G.jumpHeld=true;requestJump();return}if(e.code==='KeyE'){interact();return}if(e.code==='KeyR'){if(!G.ownerFlyActive)respawn();return}G.keys.add(e.code)};
  G.keyUp=e=>{if(MOVEMENT_REPRESS_KEY_SET.has(e.code)){G.physicalMoveKeys.delete(e.code);G.keys.delete(e.code);releaseMovementRepressIfReady();return}if(e.code==='Space'){G.keys.delete('Space');G.jumpHeld=false;G.jumpQueuedUntil=0;return}G.keys.delete(e.code)};document.addEventListener('keydown',G.keyDown);document.addEventListener('keyup',G.keyUp);
  G.inputBlurHandler=()=>resetMovementControls(true,true);G.visibilityHandler=()=>{if(document.hidden)resetMovementControls(true,true)};
  ...
  if(stick&&knob){
    ...
    const reset=()=>{pointerId=null;touchId=null;G.mobileMoveHeld=false;G.mobileX=0;G.mobileY=0;releaseMovementRepressIfReady();knob.style.transform='translate(-50%,-50%)';stick.classList.remove('active')};
    const applyPoint=(clientX,clientY)=>{ensureAudio();const r=stick.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,max=Math.max(24,r.width*.34);let dx=clientX-cx,dy=clientY-cy;const len=Math.hypot(dx,dy)||1,scale=Math.min(1,max/len);dx*=scale;dy*=scale;if(G.movementRepressRequired){G.mobileX=0;G.mobileY=0;}else{G.mobileX=Math.max(-1,Math.min(1,dx/max));G.mobileY=Math.max(-1,Math.min(1,dy/max));}knob.style.transform=`translate(calc(-50% + ${dx}px),calc(-50% + ${dy}px))`};
    stick.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;e.preventDefault();e.stopPropagation();pointerId=e.pointerId;G.mobileMoveHeld=true;stick.classList.add('active');applyPoint(e.clientX,e.clientY);try{stick.setPointerCapture?.(e.pointerId)}catch{}},{passive:false});
    ...
    stick.addEventListener('touchstart',e=>{if(pointerId!==null)return;const t=e.changedTouches[0];if(!t)return;e.preventDefault();touchId=t.identifier;G.mobileMoveHeld=true;stick.classList.add('active');applyPoint(t.clientX,t.clientY)},{passive:false});
