...
import { ESCAPE_WORLD_DEFS as WORLD_DEFS } from './escape-kl-worlds.js?v=20260813-escape-v434';
import { buildKeyboardLabWorld } from './escape-kl-world-keyboard-lab.js?v=20260813-escape-v434';
import { createEscapeCharacter } from './escape-kl-character.js?v=20260813-escape-v434';

/* Escape.kl – JK.Games Top Game V2 · Character/animation/world polish */
const VERSION = '2026-08-13-v434';
...
const G = {
  ...
  keys:new Set(), inputX:0, inputY:0, mobileX:0, mobileY:0, sprint:false,mobileSprint:false,moveIntensity:0,
...
function defaultProgress(){
  return {
    version:2,speed:0,wins:0,rebirths:0,speedUpgrade:0,limiter:1,
    trail:'none',ownedTrails:['none'],worldsUnlocked:['keyboard-lab'],
    worldStars:{},bestTimes:{},completions:{},hiddenKeys:{},totalDistance:0,
    lastWheelAt:0,dailyQuest:null,lastPlayedAt:Date.now()
  };
}
function normalizeProgress(raw){
  const d={...defaultProgress(),...(raw&&typeof raw==='object'?raw:{})};
  d.speed=Math.max(0,Number(d.speed)||0);d.wins=Math.max(0,Math.floor(Number(d.wins)||0));d.rebirths=Math.max(0,Math.floor(Number(d.rebirths)||0));d.speedUpgrade=Math.max(0,Math.min(SPEED_UPGRADES.length,Math.floor(Number(d.speedUpgrade)||0)));d.limiter=LIMITERS.includes(Number(d.limiter))?Number(d.limiter):1;d.ownedTrails=Array.isArray(d.ownedTrails)?[...new Set(d.ownedTrails.filter(id=>TRAILS.some(t=>t.id===id)))]:['none'];if(!d.ownedTrails.includes('none'))d.ownedTrails.unshift('none');d.trail=d.ownedTrails.includes(d.trail)?d.trail:'none';d.worldsUnlocked=Array.isArray(d.worldsUnlocked)?[...new Set(d.worldsUnlocked)]:['keyboard-lab'];if(!d.worldsUnlocked.includes('keyboard-lab'))d.worldsUnlocked.unshift('keyboard-lab');d.worldStars=d.worldStars&&typeof d.worldStars==='object'?d.worldStars:{};d.bestTimes=d.bestTimes&&typeof d.bestTimes==='object'?d.bestTimes:{};d.completions=d.completions&&typeof d.completions==='object'?d.completions:{};d.hiddenKeys=d.hiddenKeys&&typeof d.hiddenKeys==='object'?d.hiddenKeys:{};d.totalDistance=Math.max(0,Number(d.totalDistance)||0);d.version=2;d.lastWheelAt=Math.max(0,Number(d.lastWheelAt)||0);d.dailyQuest=d.dailyQuest&&typeof d.dailyQuest==='object'?d.dailyQuest:null;return d;
}
...
function addPlatform({x=0,y=0,z=0,w=3,h=.45,d=3,color=0x223a58,label='',checkpoint=0,finish=false,motion=null,blink=false,kind='key',hub=false}){
  ...
  const p={mesh,w,h,d,label,base:new THREE.Vector3(x,y,z),motion,blink,active:true,checkpoint,finish,kind,hub,lastPos:new THREE.Vector3(x,y,z),delta:new THREE.Vector3(),press:0,lastContactAt:0};mesh.userData.escapePlatform=p;G.platforms.push(p);return p;
}
...
function addCylinderDeco(x,y,z,rTop,rBottom,h,color,emissive=0,segments=16){const m=mat(`cyl-${color}-${emissive}`,{color,emissive,emissiveIntensity:emissive?.45:0,roughness:.52,metalness:.18});const o=new THREE.Mesh(new THREE.CylinderGeometry(rTop,rBottom,h,segments),m);o.position.set(x,y,z);o.castShadow=true;o.receiveShadow=true;G.scene.add(o);G.decorative.push(o);return o;}
function addRingDeco(x,y,z,r,tube,color,rotX=Math.PI/2){const m=mat(`ring-${color}`,{color,emissive:color,emissiveIntensity:.72,roughness:.36,metalness:.28});const o=new THREE.Mesh(new THREE.TorusGeometry(r,tube,8,32),m);o.position.set(x,y,z);o.rotation.x=rotX;o.castShadow=true;G.scene.add(o);G.decorative.push(o);return o;}
function addGlowLight(x,y,z,color=0x67e8ff,intensity=1.4,distance=11){const l=new THREE.PointLight(color,intensity,distance,2);l.position.set(x,y,z);G.scene.add(l);G.decorative.push(l);return l;}
...
function setupScene(){
  ...
  buildHub();buildRaceCourse();buildKeyboardLabWorld({addPlatform,addSign:(text,pos,color,scale)=>addSign(text,new THREE.Vector3(pos.x,pos.y,pos.z),color,scale),boxDeco,addCylinderDeco,addRingDeco,addGlowLight,addInteractable,returnHub:()=>setWorld('hub')});createPlayer();resize();setWorld('hub',true);updateHud(true);
}
...
function buildHub(){
  ...
  for(const x of[-26,26])for(let z=-22;z<=22;z+=11){addCylinderDeco(x,1.25,z,.09,.14,2.5,0x2cbfd8,0x155b70,10);addGlowLight(x,2.2,z,0x5cecff,.8,7);}
  ...
  // Three physical treadmill lanes. Mechanics stay identical; visuals are richer.
  for(let lane=-1;lane<=1;lane++){const x=-12+lane*2.35,tr=addPlatform({x,y:.38,z:0,w:1.9,h:.28,d:11.5,color:lane===0?0x1b6a62:0x18524f,label:lane===0?'RUN':'',kind:'training',hub:true});tr.training=true;boxDeco(x-0.82,.72,0,.08,.56,11.6,0x45e4d2,0x155d58);boxDeco(x+.82,.72,0,.08,.56,11.6,0x45e4d2,0x155d58);}
  boxDeco(-15.9,1.75,0,.22,3.5,12.1,0x36cbbb,0x115d58);boxDeco(-8.1,1.75,0,.22,3.5,12.1,0x36cbbb,0x115d58);addSign('SPEED TRAINING',new THREE.Vector3(-12,3.75,5.95),0x62f2e1,.98);addGlowLight(-12,2.8,0,0x4ef1db,1.15,12);

  // Rebirth altar with energy rings.
  addPlatform({x:-11,y:.45,z:-15,w:7,h:.5,d:7,color:0x542c72,label:'REBIRTH',kind:'altar',hub:true});addCylinderDeco(-11,1.1,-15,1.15,1.45,1.35,0x6c3a91,0x32195e,18);addCylinderDeco(-11,2.5,-15,.42,.55,2.3,0x9f68ff,0x452073,16);addRingDeco(-11,3.05,-15,1.45,.07,0xbb78ff,Math.PI/2);addRingDeco(-11,3.05,-15,1.05,.045,0x69dcff,Math.PI/2);addGlowLight(-11,3,-15,0xb76cff,1.7,13);addInteractable('rebirth','Rebirth-Altar benutzen',-11,1,-15,4.7,()=>{soundKey('REBIRTH');openRebirth()});

  // World 1 portal: multi-layer frame, animated plane, real lighting.
  boxDeco(0,3.2,-18,10,.45,.8,0x1a6f84,0x124c63);boxDeco(-4.65,1.65,-18,.62,6.5,.8,0x1a6f84,0x124c63);boxDeco(4.65,1.65,-18,.62,6.5,.8,0x1a6f84,0x124c63);addRingDeco(0,2.75,-17.86,3.05,.08,0x5be9ff,0);addGlowLight(0,2.8,-17.4,0x45dfff,1.8,14);
  const portal=new THREE.Mesh(new THREE.PlaneGeometry(8.1,5.7),new THREE.MeshBasicMaterial({color:0x4ddffc,transparent:true,opacity:.18,side:THREE.DoubleSide,depthWrite:false}));portal.position.set(0,2.85,-18);portal.rotation.y=Math.PI;G.scene.add(portal);G.portalFx.push(portal);addSign('WORLD 1 · KEYBOARD LAB',new THREE.Vector3(0,5.85,-17.55),0x56ddff,.82);addInteractable('world1','Keyboard Lab betreten',0,1,-16.6,6.3,()=>{soundKey('ENTER');enterWorld('keyboard-lab')});

  // Prepared future portals – visually distinct but deliberately locked.
  const futures=[['CANDY KEYS',14,-16,0xde6ba8],['TOXIC',21,-8,0x70db65],['CYBER',21,8,0x6d74ff]];for(const [name,x,z,c] of futures){addPlatform({x,y:.3,z,w:5.6,h:.35,d:5.6,color:c,label:'LOCK',kind:'locked',hub:true});addCylinderDeco(x,1.55,z,.12,.18,2.7,c,c,10);addSign(name,new THREE.Vector3(x,2.75,z+2.92),c,.62);}

  // Speed Race gate – short replayable race with its own best time.
  addPlatform({x:22,y:.34,z:20,w:7,h:.3,d:6,color:0x7a3d2b,label:'RACE',kind:'race-gate',hub:true});boxDeco(22,1.7,22.55,6.6,3.0,.35,0x271711);addSign('SPEED RACE',new THREE.Vector3(22,3.2,22.75),0xff8b5d,.78);addGlowLight(22,2.3,21.8,0xff7655,1.05,9);addInteractable('speed-race','Speed Race starten',22,1.0,20.4,4.5,()=>{soundKey('RACE');setWorld('race')});

  // Daily wheel – a real physical machine in the hub.
  addPlatform({x:-21,y:.34,z:14,w:6,h:.3,d:6,color:0x4e356e,label:'DAILY',kind:'wheel-floor',hub:true});boxDeco(-21,1.25,16.3,5.5,2.0,.35,0x171d2b);addRingDeco(-21,2.25,16.05,1.55,.15,0xf0b85b,0);addRingDeco(-21,2.25,15.96,.92,.08,0x69e4ff,0);addCylinderDeco(-21,.95,15.85,.16,.28,1.4,0xd5a24d,0x6d4b10,12);addGlowLight(-21,2.25,15.2,0xffcb68,1.1,9);addSign('DAILY WHEEL',new THREE.Vector3(-21,4.0,16.18),0xffc65a,.72);addInteractable('daily-wheel','Daily Wheel drehen',-21,1.1,14.2,4.2,()=>{soundKey('DAILY');openDailyWheel()});

  // Daily quest terminal.
  addPlatform({x:12,y:.32,z:17,w:7,h:.28,d:6,color:0x254967,label:'QUEST',kind:'quest-floor',hub:true});boxDeco(12,1.85,19.55,6.6,3.2,.32,0x0b1826);boxDeco(12,1.9,19.35,5.5,2.1,.12,0x163953,0x0b2d45);addSign('DAILY QUESTS',new THREE.Vector3(12,3.35,19.72),0x66e3ff,.76);addGlowLight(12,2.5,18.8,0x58dfff,.9,8);addInteractable('daily-quests','Tagesquests ansehen',12,1.0,17.8,4.3,()=>{soundKey('QUEST');openDailyQuests()});

  // Records board / personal leaderboard. No continuous Firestore writer is needed.
  boxDeco(-1.0,1.8,22,15,3.4,.28,0x0b1725);addSign('ESCAPE.KL',new THREE.Vector3(-1,2.75,21.82),0xffcb5a,1.22);addSign('RECORDS · RUN · JUMP',new THREE.Vector3(-1,1.45,21.8),0x6be5ff,.65);addGlowLight(-1,2.6,20.8,0xffcb5a,.75,9);addInteractable('records','Escape-Statistiken ansehen',-1,1.0,20.0,5.7,()=>{soundKey('RECORD');openRecords()});
}
...
function setWorld(id,initial=false){
  ...
  if(id==='hub'){
    ...
    G.stage=1;G.deaths=0;G.runStartedAt=performance.now();G.checkpoint={x:72,y:1.25,z:20};teleport(72,1.25,20);toast('Speed Race gestartet · 5 Race-Checkpoints · Bestzeit zählt!','good',2400);
  }
  updateHud(true);
}
...
function teleport(x,y,z){G.pos.set(x,y,z);G.vel.set(0,0,0);G.moveVel.set(0,0,0);G.grounded=false;G.support=null;G.lastSupport=null;G.lastGroundPos.copy(G.pos);G.trailPoints=[];if(G.playerRoot)G.playerRoot.position.copy(G.pos);}
function respawn(){G.deaths++;soundFail();const c=G.checkpoint||{x:0,y:1.08,z:8};teleport(c.x,c.y,c.z);toast(`Respawn · Checkpoint ${Math.max(1,G.stage)}`,'bad',1500);}
...
function updatePlatforms(t,dt=.016){
  ...
  for(const fx of G.portalFx){fx.material.opacity=.16+Math.sin(t*2.2)*.07;fx.scale.setScalar(1+Math.sin(t*1.7)*.02);fx.rotation.z=Math.sin(t*.55)*.025;}
}
...
function processMovement(dt,t){
  ...
  G.pos.x+=G.moveVel.x*dt;G.pos.z+=G.moveVel.z*dt;
  ...
  if(!wasGrounded&&G.grounded)G.landingPulse=1;G.landingPulse=Math.max(0,G.landingPulse-dt*7);
  ...
  if(isOnTraining()){G.trainingCarry+=dt*5*gainMultiplier();if(G.trainingCarry>=1){const add=Math.floor(G.trainingCarry);G.trainingCarry-=add;addSpeed(add,true);queuePersist(1800);}}
...
function updateHud(force=false){
  if(!G.overlay||!G.state)return;G.hudClock+=force?999:0;const set=(q,v)=>{const e=G.overlay.querySelector(q);if(e&&e.textContent!==String(v))e.textContent=String(v)};set('[data-ekl-speed]',fmt(G.state.speed));set('[data-ekl-wins]',fmt(G.state.wins));set('[data-ekl-rebirths]',G.state.rebirths);set('[data-ekl-gain]',`×${gainMultiplier().toFixed(2).replace('.',',')}`);const race=G.world==='race',hub=G.world==='hub',total=race?5:10;set('[data-ekl-world]',hub?'ESCAPE HUB':race?'SPEED RACE':'KEYBOARD LAB');set('[data-ekl-stage]',hub?'FREIE WELT':race?`RACE ${Math.max(1,G.stage)}/5`:`STAGE ${Math.max(1,G.stage)}/10`);set('[data-ekl-stage-total]',race?'5 CHECKPOINTS':'10 STAGES');const planar=Math.hypot(G.moveVel.x,G.moveVel.z),motion=!G.grounded?(G.vel.y>0?'SPRUNG':'FALL'):planar>.5?(G.sprint||planar>7.5?'RENNEN':'LAUFEN'):'IDLE';set('[data-ekl-motion]',motion);const motionEl=G.overlay.querySelector('[data-ekl-motion]');if(motionEl)motionEl.dataset.motion=motion.toLowerCase();const progress=G.overlay.querySelector('[data-ekl-stage-bar]');if(progress)progress.style.width=`${hub?0:Math.min(100,G.stage/total*100)}%`;G.overlay.querySelectorAll('[data-ekl-limiter]').forEach(b=>b.classList.toggle('active',Number(b.dataset.eklLimiter)===G.state.limiter));}
...
function spinDailyWheel(wrap){
  ...
  const roll=Math.random();let text='';if(roll<.28){addSpeed(750,false);text='+750 Speed';}else if(roll<.50){G.state.wins+=1;text='+1 Win';}else if(roll<.68){addSpeed(2000,false);text='+2.000 Speed';}else if(roll<.84){G.state.wins+=3;text='+3 Wins';}else if(roll<.96){G.state.wins+=5;text='+5 Wins';}else if(!G.state.ownedTrails.includes('cyan')){G.state.ownedTrails.push('cyan');text='Cyan Key Trail';}else{G.state.wins+=8;text='+8 Wins · Jackpot';}
...
function dailyQuestRows(){
  ...
  return [
    {id:'distance',icon:'🏃',name:'Laufstrecke',desc:'Lege heute 300 Meter zu Fuß zurück.',progress:distance,target:300,reward:2},
    {id:'speed',icon:'⚡',name:'Speed Training',desc:'Verdiene heute 1.500 Speed durch Laufen oder Training.',progress:Number(q.speedEarned)||0,target:1500,reward:2},
    {id:'escape',icon:'🏁',name:'Daily Escape',desc:'Schließe Keyboard Lab heute einmal ab.',progress:completions,target:1,reward:3}
  ];
}
...
function claimDailyQuest(id){const q=ensureDailyQuest(),row=dailyQuestRows().find(r=>r.id===id);if(!row||row.progress<row.target||q.claimed.includes(id))return;q.claimed.push(id);G.state.wins+=row.reward;soundBuy();queuePersist(50);updateHud(true);toast(`${row.name}: +${row.reward} Wins`,'good',2000);openDailyQuests();}
function openRecords(){const w=WORLD_DEFS[0],best=Number(G.state.bestTimes?.[w.id]||0),raceBest=Number(G.state.bestTimes?.['speed-race']||0),stars=Number(G.state.worldStars?.[w.id]||0),completions=Number(G.state.completions?.[w.id]||0),races=Number(G.state.completions?.['speed-race']||0);openModal(`<div class="ekl-modal"><div class="ekl-modal-head"><div><small>PERSÖNLICHE ESCAPE-REKORDE</small><h2>🏆 Records Board</h2><p>Diese Werte kommen direkt aus deinem Escape.kl-Spielstand und erzeugen keinen dauernden Firestore-Echtzeitstream.</p></div><button data-ekl-modal-close>×</button></div><div class="ekl-record-grid"><article><small>BESTZEIT</small><b>${best?timeText(best):'–'}</b><span>Keyboard Lab</span></article><article><small>RACE BEST</small><b>${raceBest?timeText(raceBest):'–'}</b><span>Speed Race · ${races} Läufe</span></article><article><small>STERNE</small><b>${'★'.repeat(stars)||'–'}${'☆'.repeat(Math.max(0,3-stars))}</b><span>Bestes Ergebnis</span></article><article><small>ESCAPES</small><b>${completions.toLocaleString('de-DE')}</b><span>Keyboard-Lab-Abschlüsse</span></article><article><small>DISTANZ</small><b>${Math.floor(G.state.totalDistance).toLocaleString('de-DE')} m</b><span>Gesamtlaufstrecke</span></article><article><small>SPEED</small><b>${fmt(G.state.speed)}</b><span>Aktueller Speed</span></article><article><small>REBIRTHS</small><b>${G.state.rebirths}</b><span>Permanente Progression</span></article></div></div>`);}
function showHelp(){openModal(`<div class="ekl-modal"><div class="ekl-modal-head"><div><small>ESCAPE.KL · STEUERUNG</small><h2>Wie spiele ich?</h2><p>Escape.kl ist ein echtes 3D-Jump'n'Run mit langfristigem Speed-Fortschritt.</p></div><button data-ekl-modal-close>×</button></div><div class="ekl-help"><article><b>🎮 PC</b><p>WASD laufen · Space springen · E interagieren · Shift sprinten · Maus ziehen = Kamera · Mausrad = Zoom · R = Checkpoint-Respawn.</p></article><article><b>📱 Handy</b><p>Linker virtueller Stick zum Laufen. Rechts: Sprint halten, Springen und Aktion.</p></article><article><b>⚡ Speed sammeln</b><p>Nur echte zurückgelegte Strecke am Boden zählt. Ein Meter gibt Basis-Speed × deinen Multiplikator.</p></article><article><b>🏪 Shop</b><p>Laufe im Hub in das SPEED-SHOP-Gebäude. Upgrades werden mit Wins gekauft.</p></article><article><b>🏁 Keyboard Lab</b><p>10 Stages mit echten Keycap-Reaktionen, beweglichen Tasten und Checkpoints. Fällst du herunter, startest du am letzten Checkpoint.</p></article><article><b>🏎 Speed Race</b><p>Im Hub steht ein eigener Race-Bereich. Das kurze Rennen besitzt 5 Checkpoints, Bestzeit und zusätzliche Wins.</p></article><article><b>🧍 Charakter</b><p>Escape.kl nutzt deinen männlichen/weiblichen JK.Games-Charakter mit echter Idle-, Lauf- und Rennanimation. Sprung und Fall besitzen eigene Übergangsposen.</p></article><article><b>🎡 Daily & Quests</b><p>Im Hub stehen ein echtes Daily Wheel und ein Quest-Terminal. Jeden Tag gibt es drei neue Aufgaben und einen kostenlosen Dreh.</p></article><article><b>🎚 Speed-Limiter</b><p>25/50/75/100 % bestimmt deine echte Laufgeschwindigkeit. Dein gesammelter Speed bleibt dabei unverändert.</p></article></div></div>`);}
...
function bindInput(){
  ...
  G.overlay.querySelector('[data-ekl-jump]')?.addEventListener('pointerdown',e=>{e.preventDefault();ensureAudio();jump()});G.overlay.querySelector('[data-ekl-interact]')?.addEventListener('pointerdown',e=>{e.preventDefault();ensureAudio();interact()});
