(() => {
  ...
  const FC_VERSION = "2026-07-25-financial-housing-v4-overdraft-home";
  ...
  isHomeDashboardAvailable = function fcIsHomeDashboardAvailable() {
    // Zuhause zählt immer als verfügbar. Ein alter oder kurzzeitig falscher
    // homeDashboardActive-Wert darf Heute & Arbeit nicht mehr verstecken.
    return !!state
      && isAtOwnHome()
      && !state.localTravel
      && !state.worldTravel
  ...
  function fcEvict(reason, source = "tenant") {
    ...
    if (typeof lowerCredit === "function") lowerCredit(12, "Räumung und Mietrückstand");
    else state.credit = typeof clamp === "function" ? clamp(fcNum(state.credit, 15) - 12) : Math.max(0, fcNum(state.credit, 15) - 12);
  ...
  function fcStartRepaymentPlan() {
    ...
    if (typeof lowerCredit === "function") lowerCredit(8, "Umschuldung wegen überzogenem Konto");
    else state.credit = typeof clamp === "function" ? clamp(fcNum(state.credit) - 8) : Math.max(0, fcNum(state.credit) - 8);
  ...
  function fcCrisisPanelHtml(activeSection = "overview") {
    ...
    return `<div class="fc-center" data-fc-section="${activeSection}"><nav class="fc-tabs">${sections.map(([id, label]) => `<button class="${activeSection === id ? "active" : ""}" data-fc-section-open="${id}">${label}</button>`).join("")}</nav>${content}<p class="fc-note">Wichtig: Bargeld bleibt immer mindestens 0 €. Das Bankkonto stoppt bei -10.000 €. Der Spieler kann jederzeit über Soforthilfe, Arbeit und Unterkunft weiterspielen.</p><div class="fc-center-exit"><button class="mini-button gold" data-fc-exit-world>Ausgang · Stadtkarte</button><button class="mini-button" data-fc-close>Schließen</button></div></div>`;
  }
  ...
  function fcBindDialog() {
    ...
    root.querySelector("[data-fc-exit-world]")?.addEventListener("click", () => original.exitHomeToWorldMap());
    root.querySelector("[data-fc-close]")?.addEventListener("click", () => els.dialog.close());
  ...
  function fcOpenCrisisCenter(section = "overview") {
    ...
    fcEnsureHomelessState();
    if (!state.housingCrisis?.homeless) {
      addFeed("Finanz- und Wohnhilfe wird erst freigeschaltet, wenn du den Obdachlosenstatus hast.");
      return false;
    }
  ...
  function fcAppendBankPanel() {
    ...
    panel.classList.toggle("locked", !housing.homeless);
    panel.innerHTML = `<h3>Finanzen & Wohnhilfe</h3><p class="muted">${housing.homeless ? `Kontolimit ${euro.format(FC_OVERDRAFT_LIMIT)} · Obdachlosenstatus aktiv · Krisenhilfe verfügbar.` : "Nur mit aktivem Obdachlosenstatus verfügbar. Sobald du wieder eine feste Unterkunft hast, wird dieser Bereich automatisch gesperrt."}</p><div class="fc-bank-summary"><span><small>Konto</small><b>${euro.format(state.bank)}</b></span><span><small>Bargeld</small><b>${euro.format(state.cash)}</b></span><span><small>Mietrückstand</small><b>${euro.format(housing.rentArrears)}</b></span></div><button class="primary-button" data-fc-open-center ${housing.homeless ? "" : "disabled"}>${housing.homeless ? "Finanz- und Wohnhilfe öffnen" : "Gesperrt · kein Obdachlosenstatus"}</button>`;
  ...
  openPlayerBankDialog = function fcOpenPlayerBankDialog() {
    ...
    button.disabled = !state?.housingCrisis?.homeless;
    button.textContent = state?.housingCrisis?.homeless ? "Finanz- und Wohnhilfe" : "Wohnhilfe gesperrt";
    button.title = state?.housingCrisis?.homeless ? "Krisenhilfe öffnen" : "Nur mit aktivem Obdachlosenstatus verfügbar";
  ...
  renderDailyActions = function fcRenderDailyActions() {
    ...
    const stage = fcFinancialStage();
    const shouldShow = !!state.housingCrisis.homeless;
  ...
  function fcUpdateGlobalBanner() {
    ...
    const stage = fcFinancialStage();
    const visible = !!state.housingCrisis.homeless;
    if (!visible) {
      banner?.remove();
      return;
    }
  ...
  render = function fcRender() {
    ...
    if (!state?.housingCrisis?.homeless && els.dialog?.classList?.contains("financial-crisis-dialog") && els.dialog.open) {
      els.dialog.close();
    }
