/* Escape.kl World 1 – Keyboard Lab V437.
...
export function buildKeyboardLabWorld(api){
  ...
  const startZ=-70;
  const rewards=[1,2,3,4,5,6,8,10,12,15,20,25,30,40,50];
  ...
  const stageTitle=(n,title,rec='')=>{
    ...
    if(rec)addSign(`EMPFOHLEN: ${rec} SPEED`,{x:0,y:2.45,z:z+2.12},0xb6c7d8,.35);
  };
  ...
  z-=2.5;stageTitle(13,'MOVING SPACE','200–260');
  ...
  addPlatform({x:0,y:3.93,z,w:6.2,h:.20,d:2.8,color:0xe0ad2d,label:`WIN +${rewards[14]}`,kind:'win-pad',winReward:rewards[14],winStage:15,stage:15,finish:true});
  ...
  addSign(`STAGE 15 GESCHAFFT · +${rewards[14]} WINS`,{x:0,y:6.55,z:z+3.6},0xffd35b,.60);
