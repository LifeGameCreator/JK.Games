/* Escape.kl World 2 – Candy Keys V437. Medium difficulty. */
export function buildCandyKeysWorld(api){
  ...
  const rewards=[30,40,55,70,90,120,160,220,300,400];
  const startZ=-70;
  ...
  addSign('WORLD 2 · CANDY KEYS',{x:0,y:5.2,z:startZ+4.5},0xff80c7,1.03);addSign('MITTEL · 10 STAGES',{x:0,y:4.0,z:startZ+4.52},0xffd26b,.48);
  let z=startZ-10;
  ...
  const win=(stage,reward,y=.7,finish=false)=>{z-=4.2;addPlatform({x:0,y:y-.16,z,w:9.0,h:.4,d:5.5,color:0x522740,label:'',kind:'safe-zone'});addPlatform({x:0,y:y+.15,z,w:4.7,h:.2,d:2.35,color:0xe4ad31,label:`WIN +${reward}`,kind:'win-pad',winReward:reward,winStage:stage,stage:Math.min(10,stage+1),finish});addSign(`+${reward} WINS`,{x:0,y:y+2.0,z:z+.1},0xffd45f,.5);};
  ...
  z-=5.2;addPlatform({x:0,y:3.05,z,w:11,h:.6,d:7,color:0x8d3e6d,label:'FINISH',kind:'safe-zone'});addPlatform({x:0,y:3.43,z,w:5.8,h:.2,d:2.7,color:0xe6b133,label:`WIN +${rewards[9]}`,kind:'win-pad',winReward:rewards[9],winStage:10,stage:10,finish:true});
  addRingDeco(0,5.1,z-2.7,2.0,.08,0xff87cc,0);addRingDeco(0,5.1,z-2.6,1.45,.05,0xffdc72,0);addGlowLight(0,5.2,z-2.4,0xff7fc6,1.25,12);addSign('CANDY KEYS COMPLETE',{x:0,y:5.8,z:z+3.5},0xff8dcc,.78);addSign(`+${rewards[9]} WINS`,{x:0,y:4.75,z:z+3.52},0xffd45f,.55);addInteractable('candy-hub-return','Zum Hub zurück',0,3.9,z-2.0,5,returnHub);
}
