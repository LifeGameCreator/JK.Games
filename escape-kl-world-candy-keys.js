/* Escape.kl World 2 – Candy Keys V438. Medium difficulty · Level 140+. */
export function buildCandyKeysWorld(api){
  ...
  const win=(stage,reward,y=.7,finish=false)=>{z-=4.2;addPlatform({x:0,y:y-.16,z,w:9.0,h:.4,d:5.5,color:0x522740,label:'',kind:'safe-zone'});addPlatform({x:0,y:y+.15,z,w:4.7,h:.2,d:2.35,color:0xe4ad31,label:`WIN +${compactReward(reward)}`,kind:'win-pad',winReward:reward,winStage:stage,stage:Math.min(10,stage+1),finish});addSign(`+${compactReward(reward)} WINS`,{x:0,y:y+2.0,z:z+.1},0xffd45f,.5);};
  ...
  z-=5.2;addPlatform({x:0,y:3.05,z,w:11,h:.6,d:7,color:0x8d3e6d,label:'FINISH',kind:'safe-zone'});addPlatform({x:0,y:3.43,z,w:5.8,h:.2,d:2.7,color:0xe6b133,label:`WIN +${compactReward(rewards[9])}`,kind:'win-pad',winReward:rewards[9],winStage:10,stage:10,finish:true});
