...
async upsertRemotePlayer(uid,profile,live){
  ...
  async installRemoteModel(remote,skin){
    ...
    const task=(async()=>{let template=null,fallback=null;try{template=await this.remoteTemplate(skin);}catch{fallback=this.makeFallbackCharacter(skin);}if(token!==remote.modelInstallToken||!this.remotePlayers.has(remote.uid))return;
      remote.mixer?.stopAllAction?.();remote.walkMixer?.stopAllAction?.();remote.runMixer?.stopAllAction?.();remote.vehicleObject?.userData?.mixer?.stopAllAction?.();this.disposeGeneratedVisual(remote.cape);this.disposeGeneratedVisual(remote.hat);this.disposeGeneratedVisual(remote.aura);this.disposeGeneratedVisual(remote.formObject);this.disposeGeneratedVisual(remote.vehicleObject);this.disposeGeneratedVisual(remote.heldObject);this.disposeGeneratedVisual(remote.staffBeamVisual);remote.staffBeamVisual=null;remote.heldAnchor?.removeFromParent?.();this.disposeClonedModelMaterials(remote.model);remote.pivot.clear();remote.mixer=null;remote.walkMixer=null;remote.runMixer=null;remote.dualLocomotion=false;remote.walkVisual=null;remote.runVisual=null;remote.action=null;remote.walkAction=null;remote.runAction=null;remote.activeAction=null;remote.wasWalking=false;remote.model=null;remote.bones=null;remote.aura=null;remote.cape=null;remote.hat=null;remote.formObject=null;remote.vehicleObject=null;remote.heldObject=null;remote.heldAnchor=null;remote.renderedHeldItem='';remote.renderedAppearanceSkin='';remote.renderedVehicle='';remote.lastVisualKey='';remote.lastFormKey='';remote.lastCosmeticKey='';remote.lastHeldKey='';remote.lastMaterialThemeKey='';
...
applyRemoteHeldItem(remote){
  ...
  removeRemotePlayer(uid){const remote=this.remotePlayers.get(uid);if(!remote)return;remote.modelInstallToken=(remote.modelInstallToken||0)+1;remote.mixer?.stopAllAction?.();remote.walkMixer?.stopAllAction?.();remote.runMixer?.stopAllAction?.();remote.vehicleObject?.userData?.mixer?.stopAllAction?.();this.disposeGeneratedVisual(remote.cape);this.disposeGeneratedVisual(remote.hat);this.disposeGeneratedVisual(remote.aura);this.disposeGeneratedVisual(remote.formObject);this.disposeGeneratedVisual(remote.vehicleObject);this.disposeGeneratedVisual(remote.heldObject);remote.heldAnchor?.removeFromParent?.();this.disposeClonedModelMaterials(remote.model);remote.group.removeFromParent();remote.label?.material?.map?.dispose?.();remote.label?.material?.dispose?.();this.remotePlayers.delete(uid);}
