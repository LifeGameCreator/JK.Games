...
const CENTER_VERSION = '2026-08-09-jkgames-v293-house-upgrades-chests-storage';
...
function defaultSaveState() {
  return {
    ...
    equippedTool: '',
    ownerAppearance: { skin: 'normal', staffSkinTheme: 'none', aura: false, auraTheme: 'owner', size: 1, heldItem: 'none', heldItemTheme: 'owner', cape: false, capeTheme: 'owner', cloak: false, cloakTheme: 'owner', hat: false, hatTheme: 'owner', vehicle: 'none', scooterTuning: 0, quickForm: 'fox', treeVariant: 0, publicMode: 'staff', publicGender: 'male', publicAlias: 'Center Spieler', staffMaxHealth: 500, staffDamage: 180 },
...
buildChunkGrass(chunk) {
  ...
  updatePlayerAnimation(delta,moving,sprint) {
    ...
    }else if(this.playerMixer&&(this.playerWalkAction||this.playerAction)){
...
async ensureCorrectPlayerModel() {
    ...
    try{
      if((skin==='male'||skin==='female')&&RUN_MODEL_PATHS[skin]){
        ...
        const walkClip=this.selectLocomotionClip(gltf.animations||[]);if(walkClip){this.playerMixer=new THREE.AnimationMixer(root);this.playerWalkAction=this.playerMixer.clipAction(walkClip);this.playerWalkAction.play();this.playerWalkAction.paused=true;this.playerAction=this.playerWalkAction;this.resetLocomotionToIdle(true);}
      }
...
async upsertRemotePlayer(uid,profile,live){
  ...
  async remoteTemplate(skin){if(this.remoteModelCache.has(skin))return this.remoteModelCache.get(skin);const promise=(async()=>{if((skin==='male'||skin==='female')&&RUN_MODEL_PATHS[skin]){const [walk,run]=await Promise.all([this.gltfLoader.loadAsync(MODEL_PATHS[skin]),this.gltfLoader.loadAsync(RUN_MODEL_PATHS[skin])]);return{dual:true,walkRoot:this.normalizeEgoCenterVisual(walk.scene,1.76),runRoot:this.normalizeEgoCenterVisual(run.scene,1.76),walkAnimations:walk.animations||[],runAnimations:run.animations||[]};}const gltf=await this.gltfLoader.loadAsync(MODEL_PATHS[skin]||MODEL_PATHS.male);const root=gltf.scene;root.updateMatrixWorld(true);const box=new THREE.Box3().setFromObject(root),height=Math.max(.01,box.max.y-box.min.y);root.scale.multiplyScalar((skin==='owner'?1.86:1.76)/height);root.updateMatrixWorld(true);const scaled=new THREE.Box3().setFromObject(root),center=scaled.getCenter(new THREE.Vector3());root.position.x-=center.x;root.position.z-=center.z;root.position.y-=scaled.min.y-.035;root.rotation.y=CHARACTER_MODEL_YAW;return{root,animations:gltf.animations||[],runAnimations:[]};})();this.remoteModelCache.set(skin,promise);try{return await promise;}catch(e){this.remoteModelCache.delete(skin);throw e;}}
...
applyRemoteHeldItem(remote){
  ...
  updateRemotePlayers(delta){
    ...
    for(const [uid,remote] of this.remotePlayers){
      ...
      else if(remote.mixer&&(remote.walkAction||remote.action)){if(humanWalking){const desired=remote.walkAction||remote.action;if(remote.activeAction!==desired){if(remote.activeAction&&remote.activeAction!==desired){remote.activeAction.fadeOut?.(.10);remote.activeAction.paused=false;}desired.enabled=true;desired.paused=false;desired.reset?.();desired.fadeIn?.(.10);desired.play?.();remote.activeAction=desired;}else desired.paused=false;remote.mixer.timeScale=1;remote.mixer.update(delta);remote.wasWalking=true;}else{if(remote.wasWalking){for(const a of [remote.walkAction,remote.action])if(a){a.fadeOut?.(.08);a.paused=true;}remote.activeAction=null;remote.wasWalking=false;}this.applyRemoteIdlePose(remote,delta,false);}}
