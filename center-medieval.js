...
const CENTER_VERSION = '2026-08-09-jkgames-v287-smooth-npcs-animals-cloak-back-fix';
...
applyOwnerWearables(){
    ...
    if(wantCloak&&!this.ownerCloakObject){const cloak=this.createStaffCloak(cloakTheme);this.modelPivot.add(cloak);this.ownerCloakObject=cloak;cloak.userData.centerBackAttachment=true;const backBone=this.playerBones?.neck||this.resolveBackAttachmentBone(this.playerModel,this.playerBones)||this.playerBones?.chest||this.playerBones?.spine,backPos=new THREE.Vector3(0,1.62,-.14),backQ=new THREE.Quaternion();if(!this.configureWearableFollower(cloak,this.modelPivot,backBone,backPos,backQ)){cloak.position.copy(backPos);cloak.quaternion.copy(backQ);}this.updateWearableFollower(cloak);}
