...
applyOwnerWearables(){
    ...
    if(wantCloak&&!this.ownerCloakObject){const cloak=this.createStaffCloak(cloakTheme);this.modelPivot.add(cloak);this.ownerCloakObject=cloak;cloak.userData.centerBackAttachment=true;const backBone=this.resolveBackAttachmentBone(this.playerModel,this.playerBones),backPos=new THREE.Vector3(0,1.56,-.08),backQ=new THREE.Quaternion();if(!this.configureWearableFollower(cloak,this.modelPivot,backBone,backPos,backQ)){cloak.position.copy(backPos);cloak.quaternion.copy(backQ);}}
...
applyRemoteOwnerAura(remote) {
    ...
    if(visible&&remote.staffCloak&&tier!=='minimal'&&!remote.cloak){remote.cloak=this.createStaffCloak(cloakTheme,tier!=='high');remote.cloak.userData.qualityTier=tier;this.simplifyRemoteVisual(remote.cloak,tier);remote.pivot.add(remote.cloak);const backBone=this.resolveBackAttachmentBone(remote.model,remote.bones),neckPos=new THREE.Vector3(0,1.57,-.115),q=new THREE.Quaternion();if(!this.configureWearableFollower(remote.cloak,remote.pivot,backBone,neckPos,q)){remote.cloak.position.copy(neckPos);remote.cloak.quaternion.copy(q);}}if(remote.cloak)remote.cloak.visible=visible&&remote.staffCloak&&tier!=='minimal';
