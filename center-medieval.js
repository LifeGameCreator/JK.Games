...
const MODEL_PATHS = Object.freeze({
  ...
  maskedOwner: `./owner-masked-v294.glb?v=20260809-v301`
});
...
class CenterDynastyGame {
  ...
  effectiveMaxHealth(){const levelMax=this.playerLevelMaxHealth();if(this.isStaffActive&&['owner','admin'].includes(this.activeStaffRole))return Math.max(levelMax,Math.floor(clamp(Number(this.state.ownerAppearance?.staffMaxHealth)||500,100,10000)));return levelMax;}
...
async updateChunkStreaming(force = false) {
  ...
  buildCurvedVillagePath(chunk,points,width,kind='gravel',shoulderKind=null){
    ...
    if(shoulderMaterial){
      this.createContinuousVillageRibbon(chunk,curve,width+1.20,shoulderMaterial,'v308-village-road-shoulder',.032);
      this.createTerrainCircle(chunk,points[0].x,points[0].z,(width+1.20)*.5,shoulderMaterial,28,.033);
      this.createTerrainCircle(chunk,points[points.length-1].x,points[points.length-1].z,(width+1.20)*.5,shoulderMaterial,28,.033);
    }
    const main=this.createContinuousVillageRibbon(chunk,curve,width,mainMaterial,'v308-village-road-main',.064);
    this.createTerrainCircle(chunk,points[0].x,points[0].z,width*.5,mainMaterial,28,.065);
    this.createTerrainCircle(chunk,points[points.length-1].x,points[points.length-1].z,width*.5,mainMaterial,28,.065);
  ...
  buildWorldwidePathRibbon(chunk,points,width,kind,shoulderKind=null,name='world-path'){
    ...
    if(shoulderKind){
      this.createContinuousVillageRibbon(chunk,curve,width+1.05,this.groundPathMaterial(shoulderKind),`${name}-shoulder`,.030);
    }
    const main=this.createContinuousVillageRibbon(chunk,curve,width,this.groundPathMaterial(kind),`${name}-main`,.060);
    for(let i=1;i<main.points.length;i+=1){
      const a=main.points[i-1],b=main.points[i];
  ...
  connectVillageToWorldwidePath(chunk,plan){
    ...
    this.createTerrainCircle(chunk,best.target.x,best.target.z,2.7,this.groundPathMaterial(plan.secondaryKind),32,.079);
  }
...
buildChunkGrass(chunk) {
  ...
  updatePlayerAnimation(delta,moving,sprint) {
    ...
    if(this.playerDualLocomotion&&(this.playerWalkMixer||this.playerRunMixer)){
...
async ensureCorrectPlayerModel() {
    ...
    try{
      if((skin==='male'||skin==='female')&&RUN_MODEL_PATHS[skin]&&IDLE_MODEL_PATHS[skin]){
        ...
        if(skin==='maskedOwner'){
          const animations=gltf.animations||[],idleClip=animations.find((clip)=>/^stand$/i.test(String(clip?.name||'')))||animations.find((clip)=>/stand/i.test(String(clip?.name||'')))||animations.find((clip)=>/^idle$/i.test(String(clip?.name||'')))||animations.find((clip)=>/idle/i.test(String(clip?.name||'')))||null,walkClip=this.makeInPlaceLocomotionClip(animations.find((clip)=>/^walk$/i.test(String(clip?.name||'')))||animations.find((clip)=>/walk/i.test(String(clip?.name||'')))||null),runClip=this.makeInPlaceLocomotionClip(animations.find((clip)=>/^run$/i.test(String(clip?.name||'')))||animations.find((clip)=>/run|jog/i.test(String(clip?.name||'')))||null);
          this.playerMixer=new THREE.AnimationMixer(root);this.playerUsesNativeIdle=!!idleClip;
          if(idleClip){this.playerIdleAction=this.playerMixer.clipAction(idleClip);this.playerIdleAction.play();this.playerIdleAction.paused=false;}
          if(walkClip){this.playerWalkAction=this.playerMixer.clipAction(walkClip);this.playerWalkAction.play();this.playerWalkAction.paused=true;this.playerAction=this.playerWalkAction;}
          if(runClip){this.playerRunAction=this.playerMixer.clipAction(runClip);this.playerRunAction.play();this.playerRunAction.paused=true;}
          this.resetLocomotionToIdle(true);
...
async upsertRemotePlayer(uid,profile,live){
  ...
  async installRemoteModel(remote,skin){
    ...
    const task=(async()=>{let template=null,fallback=null;try{template=await this.remoteTemplate(skin);}catch{fallback=this.makeFallbackCharacter(skin);}if(token!==remote.modelInstallToken||!this.remotePlayers.has(remote.uid))return;
      remote.mixer?.stopAllAction?.();remote.walkMixer?.stopAllAction?.();remote.runMixer?.stopAllAction?.();remote.vehicleObject?.userData?.mixer?.stopAllAction?.();this.disposeGeneratedVisual(remote.cape);this.disposeGeneratedVisual(remote.cloak);this.disposeGeneratedVisual(remote.hat);this.disposeGeneratedVisual(remote.aura);this.disposeGeneratedVisual(remote.formObject);this.disposeGeneratedVisual(remote.vehicleObject);this.disposeGeneratedVisual(remote.heldObject);this.disposeGeneratedVisual(remote.staffBeamVisual);remote.staffBeamVisual=null;remote.heldAnchor?.removeFromParent?.();this.disposeClonedModelMaterials(remote.model);remote.pivot.clear();remote.mixer=null;remote.walkMixer=null;remote.runMixer=null;remote.dualLocomotion=false;remote.walkVisual=null;remote.runVisual=null;remote.action=null;remote.walkAction=null;remote.runAction=null;remote.idleAction=null;remote.nativeIdle=false;remote.activeAction=null;remote.wasWalking=false;remote.model=null;remote.bones=null;remote.aura=null;remote.cape=null;remote.cloak=null;remote.hat=null;remote.formObject=null;remote.vehicleObject=null;remote.heldObject=null;remote.heldAnchor=null;remote.renderedHeldItem='';remote.renderedAppearanceSkin='';remote.renderedVehicle='';remote.lastVisualKey='';remote.lastFormKey='';remote.lastCosmeticKey='';remote.lastHeldKey='';remote.lastMaterialThemeKey='';
...
handleOwnerInput(event){
  ...
  async handleOwnerAction(event){
    ...
    if(action==='toggle-owner-galaxy-skin'||action==='toggle-staff-skin'){
      const theme=requestedTheme||'owner';if(!this.allowedCosmeticThemes().includes(theme))return;const appearance=this.state.ownerAppearance;if(this.isOwnerActive&&appearance.ownerBaseModel==='masked'){this.toast('Der Masked Owner-Skin bleibt in seiner Originaloptik. Galaxy-Wings, Umhang, Krone, Aura, Stab und Schwert kannst du weiterhin separat verwenden.');return;}appearance.staffSkinTheme=appearance.staffSkinTheme===theme?'none':theme;this.spawnTransformationEffectAt?.(this.player.position);this.setShadowCharacterMode(appearance.staffSkinTheme!=='none',appearance.staffSkinTheme==='none'?'owner':appearance.staffSkinTheme);this.applyOwnerAppearance();this.saveState(true);this.renderOwnerMenu();this.publishPresence(true).catch(()=>{});return;
    }
    if(action==='owner-base-default'||action==='owner-base-masked'){
      if(!this.ownerOnly())return;const appearance=this.state.ownerAppearance,next=action==='owner-base-masked'?'masked':'default';if(appearance.ownerBaseModel===next)return;appearance.ownerBaseModel=next;appearance.skin='normal';appearance.vehicle='none';if(next==='masked')appearance.staffSkinTheme='none';this.spawnTransformationEffectAt?.(this.player.position);await this.ensureCorrectPlayerModel();this.applyOwnerAppearance();this.saveState(true);this.renderOwnerMenu();this.publishPresence(true).catch(()=>{});this.toast(next==='masked'?'Owner Masked Character aktiviert · Stand, Walk und Run geladen.':'Standard Owner-Skin aktiviert.');return;
    }
