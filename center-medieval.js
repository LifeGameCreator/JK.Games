...
buildChunkGrass(chunk) {
  ...
  updatePlayerAnimation(delta,moving,sprint) {
    ...
    if(this.playerDualLocomotion&&(this.playerWalkMixer||this.playerRunMixer)){
      ...
      else if(this.playerUsesNativeIdle&&this.playerIdleAction&&!flying&&!scooter&&!plane&&!mounted){this.switchPlayerLocomotion(this.playerIdleAction);this.playerMixer.timeScale=1;this.playerMixer.update(delta);this.playerActionWasMoving=false;this.idleBlend=1;}
...
async ensureCorrectPlayerModel() {
    ...
    try{
      if((skin==='male'||skin==='female')&&RUN_MODEL_PATHS[skin]){
        const [walkGltf,runGltf]=await Promise.all([this.gltfLoader.loadAsync(MODEL_PATHS[skin]),this.gltfLoader.loadAsync(RUN_MODEL_PATHS[skin])]);if(token!==this.playerModelInstallToken)return;
        const walkVisual=this.normalizeEgoCenterVisual(walkGltf.scene,1.76),runVisual=this.normalizeEgoCenterVisual(runGltf.scene,1.76),root=new THREE.Group();root.userData.centerDualLocomotion=true;root.add(walkVisual,runVisual);runVisual.visible=false;
        this.playerModel=root;this.playerWalkVisual=walkVisual;this.playerRunVisual=runVisual;this.playerDualLocomotion=true;installPivot.add(root);this.cachePlayerBones(walkVisual);
        const walkClip=this.sanitizeEgoLocomotionClip((walkGltf.animations||[])[0]||this.selectLocomotionClip(walkGltf.animations||[])),runClip=this.sanitizeEgoLocomotionClip((runGltf.animations||[])[0]||this.selectLocomotionClip(runGltf.animations||[]));
        if(walkClip){this.playerWalkMixer=new THREE.AnimationMixer(walkVisual);this.playerMixer=this.playerWalkMixer;this.playerWalkAction=this.playerWalkMixer.clipAction(walkClip);this.playerWalkAction.play();this.playerWalkAction.paused=true;this.playerAction=this.playerWalkAction;}
        if(runClip){this.playerRunMixer=new THREE.AnimationMixer(runVisual);this.playerRunAction=this.playerRunMixer.clipAction(runClip);this.playerRunAction.play();this.playerRunAction.paused=true;}this.resetLocomotionToIdle(true);
      }else{
...
applyRemoteHeldItem(remote){
  ...
  updateRemotePlayers(delta){
    ...
    for(const [uid,remote] of this.remotePlayers){
      ...
      else if(remote.mixer&&(remote.walkAction||remote.runAction||remote.action||remote.idleAction)){if(humanWalking){const desired=remote.sprinting&&remote.runAction?remote.runAction:(remote.walkAction||remote.action);if(remote.activeAction!==desired){if(remote.activeAction&&remote.activeAction!==desired){remote.activeAction.fadeOut?.(.10);remote.activeAction.paused=false;}desired.enabled=true;desired.paused=false;desired.reset?.();desired.fadeIn?.(.10);desired.play?.();remote.activeAction=desired;}else desired.paused=false;remote.mixer.timeScale=desired===remote.runAction?1.08:1;remote.mixer.update(delta);remote.wasWalking=true;}else if(remote.nativeIdle&&remote.idleAction&&!remote.flying&&!airborne&&!remote.vehicleObject){const desired=remote.idleAction;if(remote.activeAction!==desired){if(remote.activeAction&&remote.activeAction!==desired){remote.activeAction.fadeOut?.(.10);remote.activeAction.paused=false;}desired.enabled=true;desired.paused=false;desired.reset?.();desired.fadeIn?.(.10);desired.play?.();remote.activeAction=desired;}else desired.paused=false;remote.mixer.timeScale=1;remote.mixer.update(delta);remote.wasWalking=false;}else{if(remote.wasWalking||remote.activeAction){for(const a of [remote.walkAction,remote.runAction,remote.action,remote.idleAction])if(a){a.fadeOut?.(.08);a.paused=true;}remote.activeAction=null;remote.wasWalking=false;}if(!remote.nativeIdle)this.applyRemoteIdlePose(remote,delta,false);}}
