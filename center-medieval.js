...
const CENTER_VERSION = '2026-08-09-jkgames-v314-owner-great-sword-player-sword-progression';
...
function defaultSaveState() {
  return {
    ...
    swordSystem: { owned: {}, stages: {}, equippedTier: 0, lastKnownMainLevel: 0 },
    markers: [],
    deathLoot: null,
...
class CenterDynastyGame {
  ...
  effectiveMaxHealth(){return this.isStaffActive&&['owner','admin'].includes(this.activeStaffRole)?Math.floor(clamp(Number(this.state.ownerAppearance?.staffMaxHealth)||500,100,10000)):this.playerLevelMaxHealth();}
  ...
  syncCombatProgression(){const level=this.mainAccountLevel(),previous=Math.max(0,Math.floor(Number(this.state.swordSystem?.lastKnownMainLevel)||0));if(previous===level)return;const oldMax=previous>0?this.playerLevelMaxHealth(previous):100,newMax=this.playerLevelMaxHealth(level);if(!this.isStaffActive||!['owner','admin'].includes(this.activeStaffRole)){if(previous===0)this.state.needs.health=Math.max(Number(this.state.needs.health)||0,newMax);else if(level>previous)this.state.needs.health=Math.min(newMax,(Number(this.state.needs.health)||0)+Math.max(0,newMax-oldMax));else this.state.needs.health=Math.min(newMax,Number(this.state.needs.health)||newMax);}this.state.swordSystem.lastKnownMainLevel=level;}
