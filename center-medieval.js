...
const CENTER_VERSION = '2026-08-09-jkgames-v316-white-team-jump-right-leg-fix';
...
const MODEL_PATHS = Object.freeze({
  ...
  owner: `./white-team-character-v316.glb?v=20260809-v316`,
...
class CenterDynastyGame {
  ...
  currentCombatDamage(){const appearance=this.state.ownerAppearance||{},staffHeld=this.isStaffActive&&!this.isPublicPlayerMode()?appearance.heldItem:'none';if(staffHeld==='greatSword'&&this.isOwnerActive)return Math.floor(clamp(Number(this.state.ownerAppearance?.ownerGreatSwordDamage)||5000,1,100000));if(staffHeld==='sword'){const role=normalizeStaffTheme(this.appearanceTheme('heldItem'));return {owner:1800,admin:1050,moderator:650,supporter:420,member:260}[role]||420;}const swordTier=Math.floor(Number(this.state.swordSystem?.equippedTier)||0);if(swordTier>0)return this.swordDamage(swordTier);const level=this.mainAccountLevel(),hunt=this.state.skills.hunting||0;if(this.state.equippedTool==='bow')return Math.round(35+level*2+hunt*8);if(this.state.equippedTool==='spear')return Math.round(30+level*1.8+hunt*8);if(['axe','ironAxe','pickaxe','hammer'].includes(this.state.equippedTool))return Math.round(18+level*1.25+hunt*4);return Math.round(8+Math.min(32,(level-1)*.33));}
...
buildChunkGrass(chunk) {
  ...
  updatePlayerAnimation(delta,moving,sprint) {
    const flying=this.isStaffActive&&this.ownerFlags.fly,appearance=this.state.ownerAppearance||{},scooter=this.isStaffActive&&appearance.vehicle==='scooter'&&!!this.ownerScooterObject,plane=this.isStaffActive&&appearance.vehicle==='plane'&&!!this.ownerPlaneObject,mounted=!!this.mountedHorse,animalForm=this.isStaffActive&&ANIMAL_FORM_IDS.includes(appearance.skin),airborneNow=!this.onGround&&!flying,humanMoving=moving&&!flying&&!scooter&&!plane&&!mounted&&!animalForm&&!airborneNow;
    if(this.nativeJumpPlaying&&this.playerUsesNativeJump&&this.playerJumpAction&&this.playerMixer){
      if(airborneNow){this.playerJumpAction.enabled=true;this.playerJumpAction.paused=false;this.playerMixer.timeScale=1;this.playerMixer.update(delta);}
      else{this.playerJumpAction.fadeOut?.(.08);this.playerJumpAction.paused=true;this.nativeJumpPlaying=false;this.resetLocomotionToIdle(false);}
  ...
  applyActionAnimation(now=performance.now()) {
    ...
    if(action.kind==='jump'&&!this.playerUsesNativeJump){
...
updateOwnerAura(delta=.016,now=performance.now()) {
  ...
  createHeldItemMesh(kind='none',theme='owner') {
    ...
    if(kind==='greatSword'){const asset=this.createOwnerGreatSwordVisual();if(!asset)return null;group.add(asset);group.userData.assetBased=true;group.rotation.set(.04,0,-.10);}
...
applyHeldItemVisual() {
  ...
  syncOwnerHeldAnchorPose(){
    ...
    const transforms={
      ...
      greatSword:{p:[.022,-.095,.06],r:[.72,.02,-.10]},
...
applyOwnerVisualState() {
    ...
    applyItem(this.ownerHeldObject);applyItem(this.firstPersonHeldObject);applyItem(this.ownerCapeObject);applyItem(this.ownerCloakObject);applyItem(this.ownerHatObject);applyItem(this.ownerScooterObject);applyItem(this.ownerPlaneObject);
...
async ensureCorrectPlayerModel() {
    ...
    try{
      if((skin==='male'||skin==='female')&&RUN_MODEL_PATHS[skin]){
        ...
        if(skin==='maskedOwner'||skin==='owner'){
          const animations=gltf.animations||[],
            idleClip=animations.find((clip)=>/^stand$/i.test(String(clip?.name||'')))||animations.find((clip)=>/stand/i.test(String(clip?.name||'')))||animations.find((clip)=>/^idle$/i.test(String(clip?.name||'')))||animations.find((clip)=>/idle/i.test(String(clip?.name||'')))||null,
            walkClip=this.makeInPlaceLocomotionClip(animations.find((clip)=>/^walk$/i.test(String(clip?.name||'')))||animations.find((clip)=>/walk/i.test(String(clip?.name||'')))||null),
            runClip=this.makeInPlaceLocomotionClip(animations.find((clip)=>/^run$/i.test(String(clip?.name||'')))||animations.find((clip)=>/run|jog/i.test(String(clip?.name||'')))||null),
            jumpClip=animations.find((clip)=>/^jump$/i.test(String(clip?.name||'')))||animations.find((clip)=>/jump/i.test(String(clip?.name||'')))||null;
          this.playerMixer=new THREE.AnimationMixer(root);this.playerUsesNativeIdle=!!idleClip;this.playerUsesNativeJump=!!jumpClip;
          ...
          if(jumpClip){this.playerJumpAction=this.playerMixer.clipAction(jumpClip);this.playerJumpAction.setLoop(THREE.LoopOnce,1);this.playerJumpAction.clampWhenFinished=true;this.playerJumpAction.paused=true;}
...
applyPerspectiveVisibility(){this.overlay.classList.toggle('is-first-person',this.firstPerson);this.overlay.classList.toggle('is-third-person',!this.firstPerson);const disguised=this.isPublicPlayerMode(),appearance=this.state.ownerAppearance||{},skin=disguised?'normal':appearance.skin||'normal',vehicle=disguised?'none':appearance.vehicle||'none',cosmeticAllowed=this.isStaffActive&&this.staffCapabilities?.ownerCosmetics&&!disguised;if(this.playerModel)this.playerModel.visible=!this.firstPerson&&['normal','galaxy'].includes(skin)&&vehicle!=='plane';if(this.ownerFormObject)this.ownerFormObject.visible=!this.firstPerson&&!disguised;if(this.ownerHeldAnchor)this.ownerHeldAnchor.visible=!this.firstPerson;if(this.firstPersonHeldObject)this.firstPersonHeldObject.visible=this.firstPerson;if(this.ownerCapeObject)this.ownerCapeObject.visible=!this.firstPerson&&cosmeticAllowed&&!!appearance.cape&&vehicle!=='plane';if(this.ownerCloakObject)this.ownerCloakObject.visible=!this.firstPerson&&cosmeticAllowed&&!!appearance.cloak&&vehicle!=='plane';if(this.ownerHatObject)this.ownerHatObject.visible=!this.firstPerson&&cosmeticAllowed&&!!appearance.hat&&vehicle!=='plane';if(this.ownerScooterObject)this.ownerScooterObject.visible=!this.firstPerson&&this.isStaffActive&&vehicle==='scooter';if(this.ownerPlaneObject)this.ownerPlaneObject.visible=!this.firstPerson&&this.isStaffActive&&vehicle==='plane';if(this.firstPersonArmsObject)this.firstPersonArmsObject.visible=this.firstPerson&&['normal','galaxy'].includes(skin)&&!['scooter','plane'].includes(vehicle);if(this.firstPerson&&vehicle!=='plane')this.ensureFirstPersonArms().catch(()=>{});this.applyOwnerAppearance();}
  requestJump() {
    ...
    if(this.playerUsesNativeJump&&this.playerJumpAction&&this.playerMixer){
      for(const action of [this.playerIdleAction,this.playerWalkAction,this.playerRunAction,this.playerAction])if(action&&action!==this.playerJumpAction){action.fadeOut?.(.06);action.paused=true;}
      this.playerJumpAction.enabled=true;this.playerJumpAction.paused=false;this.playerJumpAction.reset();this.playerJumpAction.setEffectiveWeight?.(1);this.playerJumpAction.play();this.playerActiveAction=this.playerJumpAction;this.nativeJumpPlaying=true;this.actionAnimation=null;
    }else this.triggerActionAnimation('jump',780);
  ...
  ownerGreatSwordEquipped(){
    return !!(this.isOwnerActive&&!this.isPublicPlayerMode()&&this.state.ownerAppearance?.heldItem==='greatSword');
  }
...
renderOwnerMenu(){
    ...
    if(this.isOwnerActive){const greatOn=appearance.heldItem==='greatSword',greatDamage=this.ownerGreatSwordDamage(),specials=appearance.ownerGreatSwordSpecials||{};sections.push(`<h3 data-owner-nav="great-sword">OWNER GREAT SWORD</h3><div class="mdc-owner-character"><p class="mdc-note">Eigenes Owner-Schwert. Rechtsklick halten = Zielmodus für Werfen. Linksklick = Angriff bzw. Wurf.</p><div class="mdc-owner-actions"><button class="${greatOn?'active ':''}" data-owner-action="admin-item" data-owner-item="greatSword" data-owner-theme="owner">${greatOn?'✓ Ausgerüstet':'○ Owner Great Sword ausrüsten'}</button></div><label>Great-Sword-Schaden <input data-owner-great-sword-damage type="number" min="1" max="100000" step="1" value="${greatDamage}"><b>${greatDamage.toLocaleString('de-DE')}</b></label><div class="mdc-owner-actions three"><button class="${specials.lightning?'active ':''}" data-owner-action="great-sword-special" data-great-special="lightning">${specials.lightning?'✓':'○'} Blitzschlag</button><button class="${specials.teleport?'active ':''}" data-owner-action="great-sword-special" data-great-special="teleport">${specials.teleport?'✓':'○'} Teleport-Schlag</button><button class="${specials.throw?'active ':''}" data-owner-action="great-sword-special" data-great-special="throw">${specials.throw?'✓':'○'} Schwert werfen</button></div><p class="mdc-note">Blitzschlag erzeugt zusätzliche Blitze mit Flächenschaden. Teleport-Schlag versetzt dich an den Zielpunkt. Beim Werfen bleibt das Schwert kurz im Ziel stecken und kehrt danach zurück.</p></div>`);}
