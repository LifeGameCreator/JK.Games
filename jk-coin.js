(() => {
  ...
  const GAME_STORE = [
    ...
    { game:"escape", id:"escape-treadmill-gold", name:"Gold Speed-Treadmill ×2,8 · Dauerhaft", cost:250, text:"Schaltet im Escape.kl-Training-Hall das goldene Laufband dauerhaft frei. Es trainiert die aktive Welt kontrolliert mit ×2,8 Level-Power.", grant:{kind:"speedTreadmill:gold",amount:1} },
    { game:"escape", id:"escape-treadmill-diamond", name:"Diamond Speed-Treadmill ×4 · Dauerhaft", cost:850, text:"Schaltet das Diamond-Laufband dauerhaft frei. Es trainiert im Stand mit ×4 Level-Power und bleibt das stärkste reguläre Laufband.", grant:{kind:"speedTreadmill:diamond",amount:1} },
    { game:"escape", id:"escape-treadmill-galaxy", name:"Galaxy Laufband ×6 · Dauerhaft", cost:1200, text:"Schaltet das Galaxy-Laufband dauerhaft frei. Es steht nicht im normalen Training Hall, sondern kann im Escape.kl-Pausenmenü unter Laufband jederzeit selbst erzeugt werden. Training: ×6 Level-Power.", grant:{kind:"speedTreadmill:galaxy",amount:1} },
    { game:"escape", id:"escape-treadmill-admin", name:"Admin Laufband ×10 · Dauerhaft", cost:2000, text:"Schaltet das Admin-Laufband dauerhaft frei. Es ist ein erzeugbares Premium-Laufband im Escape.kl-Pausenmenü und trainiert mit ×10 Level-Power.", grant:{kind:"speedTreadmill:admin",amount:1} },
    { game:"escape", id:"escape-galaxy-trail", name:"Galaxy Keyboard Trail +30 % Power · Dauerhaft", cost:300, text:"Exklusive violett-kosmische Fuß- oder Rückenspur für Escape.kl. Gibt +30 % additive Level-Power, ohne Multiplikator-Explosion.", grant:{kind:"trail:galaxy",amount:1} },
    { game:"escape", id:"escape-speed-boost-2", name:"Escape Power +50 % · 15 Min.", cost:180, text:"Gibt 15 Minuten lang +50 % additive Level-Power aus echter Laufstrecke und freigeschalteten Laufbändern. Andere normale Power-Boni werden addiert statt multipliziert.", grant:{kind:"speedBoost:2",amount:1} },
  ...
  function bigCardsPermanentOwned(entry){const game=String(entry?.game||""),kind=String(entry?.grant?.kind||"");try{if(game==="escape"){if(kind==="speedBoost:2")return false;const bought=Number(coinState()?.gamePurchases?.[entry.id]||0)>0;if(typeof window.EscapeKL?.canApplyJkPurchase==="function"&&window.EscapeKL.canApplyJkPurchase(kind)===false)return true;return bought;}if(game!=="bigcards")return false;if(kind==="bulkLevelUnlock"||kind==="bulkRebirthUnlock")return false;if(kind==="vipUnlock")return !!window.BigCardsKL?.hasVip?.()||Number(coinState()?.gamePurchases?.[entry.id]||0)>0;const target=bigCardsStorageTarget(entry);return target?Math.floor(Number(window.BigCardsKL?.getFeaturedStorageTier?.())||0)>=target:false;}catch{return false;}}
  ...
  function gamesHtml(){
    ...
    const games=["all","runner","city","match","fight","dungeon","money","escape","bigcards","egoshoot","weed","casino"],labels={all:"Alle Spiele",runner:"Runner.KL",city:"City.KL",match:"Match.KL",fight:"Fight.KL",dungeon:"Dungeon.KL",money:"Money.KL",escape:"Escape.kl",bigcards:"BigCards.kl",egoshoot:"Egoshoot.KL",weed:"Weed Business",casino:"Casino"};
  ...
  const GAME_LABELS={runner:"Runner.KL",city:"City.KL",match:"Match.KL",fight:"Fight.KL",dungeon:"Dungeon.KL",money:"Money.KL",escape:"Escape.kl",bigcards:"BigCards.kl",egoshoot:"Egoshoot.KL",weed:"Weed Business",casino:"Casino"};
