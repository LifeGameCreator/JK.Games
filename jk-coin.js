(() => {
  ...
  const VERSION = "2026-08-10-jkcoin-v353-bigcards-storage";
  ...
  function bigCardsStorageOwned(entry){const target=bigCardsStorageTarget(entry);if(!target)return false;try{return Math.floor(Number(window.BigCardsKL?.getFeaturedStorageTier?.())||0)>=target;}catch{return false;}}
  function gamesHtml(){const games=["all","runner","city","match","fight","dungeon","money","speedcar","bigcards","egoshoot","weed","casino"],labels={all:"Alle",runner:"Runner.KL",city:"City.KL",match:"Match.KL",fight:"Fight.KL",dungeon:"Dungeon.KL",money:"Money.KL",speedcar:"Speed Car.KL",bigcards:"BigCards.kl",egoshoot:"Egoshoot.KL",weed:"Weed Business",casino:"Casino"};const list=GAME_STORE.filter(item=>item&&typeof item==="object"&&(ui.game==="all"||String(item.game||"")===ui.game)),filters=games.map(g=>`<button class="jkc-button ${ui.game===g?"gold":"secondary"}" data-jkc-game-filter="${g}">${esc(labels[g]||g)}</button>`).join("");return `<section class="jkc-section"><small>SPIELE-SHOP</small><h4>JK/Coin-Inhalte</h4>${scrollNavHtml("games",filters)}<div class="jkc-grid jkc-game-grid">${list.map(item=>{const gameKey=String(item?.game||""),gameLabel=safeText(labels[gameKey]||GAME_LABELS?.[gameKey],"Spiel"),name=safeText(item?.name,"Spiel-Inhalt"),description=safeText(item?.text,"JK/Coin-Inhalt"),storageOwned=bigCardsStorageOwned(item);return `<article class="jkc-card ${storageOwned?"owned":""}"><small>${esc(safeUpper(gameLabel))}</small><h5>${esc(name)}</h5><p>${esc(description)}</p>${gameKey==="fight"&&item?.requiredFightLevel?`<div class="jkc-fight-level-note">🔒 Kauf sofort möglich · Benutzung erst ab Fight-Level ${Number(item.requiredFightLevel)||1}</div>`:""}<div class="jkc-actions"><button class="jkc-button" data-jkc-buy-game="${esc(item?.id||"")}" ${storageOwned?"disabled":""}>${storageOwned?"Freigeschaltet ✓":item?.variable?"Betrag wählen":`${Math.max(0,Number(item?.cost)||0)} JK/Coin`}</button></div></article>`}).join("")}</div></section>`;}
