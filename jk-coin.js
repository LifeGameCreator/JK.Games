(() => {
  ...
  const VERSION = "2026-08-07-jkcoin-v225-collector-boxes-filter-discovery";
  const PURCHASE_COLLECTION = "jkCoinPurchaseRequests";
  const GRANT_COLLECTION = "jkCoinGrants";
  const TOTAL_COLLECTIBLES = 6470;
  const JK_COIN_MIN_RATE = 500;
  const JK_COIN_MAX_RATE = 1672;
  const PACKS = [
  ...
  const BOXES = [
    ...
    { id:"collect-100", name:"Sammler-Kiste 100", cost:100, type:"collect", tier:"basic", items:3, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-500", name:"Sammler-Kiste 500", cost:500, type:"collect", tier:"premium", items:8, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-1000", name:"Sammler-Kiste 1000", cost:1000, type:"collect", tier:"elite", items:12, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-2000", name:"Sammler-Kiste 2000", cost:2000, type:"collect", tier:"master", items:16, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-5000", name:"Sammler-Kiste 5000", cost:5000, type:"collect", tier:"galaxy", items:20, itemValueCap:NON_GALAXY_BOX_ITEM_CAP }
  ];
  ...
  const ui = { tab:"home", game:"all", collectionRarity:"all", tabScroll:Math.max(0,Number(navMemory.tabScroll)||0), gameScroll:Math.max(0,Number(navMemory.gameScroll)||0), grantsListening:false, toastTimer:0, lastGrantSync:0, grantCollectionDenied:false, gameOverlay:"" };
  ...
  function currentRate(now=Date.now()){
    ...
    // Jeder volle Euro zwischen 500 € und 1.672 € kann als Kurs auftreten.
  ...
  function boxCard(b){const probs=b.type==="collect"?COLLECT_CHANCES[b.tier]:null;const valueHint=b.type==="collect"?`Normale Items maximal ${euro(NON_GALAXY_BOX_ITEM_CAP)} · Galaxy ${euro(COLLECTOR_GALAXY_MIN_VALUE)} bis ${euro(COLLECTOR_GALAXY_MAX_VALUE)}.`:b.cost>=5000?"Nur diese 5.000er Lucky Box kann Sammlerstücke mit Milliardenwert enthalten.":`Itemwert maximal ${euro(Number(b.itemValueCap||NON_GALAXY_BOX_ITEM_CAP))}.`;return `<article class="jkc-card jkc-box" style="--box-color:${b.cost>=5000?"#ff72d7":b.cost>=1000?"#a46cff":"#64e6ff"}"><div class="jkc-box-icon">${b.type==="collect"?"◈":"🎁"}</div><small>${b.type==="collect"?"SAMMLER-KISTE":"LUCKY BOX"}</small><h5>${esc(b.name)}</h5><p>${b.type==="reward"?`${euro(b.money[0])} bis ${euro(b.money[1])}, ${b.xp[0]}–${b.xp[1]} Haupt-EP und exklusive Belohnungen. ${valueHint}`:`${Number(b.items)||1} Sammlerstücke pro Kiste. ${valueHint}`}</p>${probs?`<div class="jkc-prob-list">${["epic","legendary","special","universe","blackhole","galaxy"].map(id=>`<span>${RARITIES.find(r=>r.id===id).name}<br><b>${probs[id]}%</b></span>`).join("")}</div>`:""}<div class="jkc-actions"><button class="jkc-button gold" data-jkc-open-box="${b.id}">${b.cost.toLocaleString("de-DE")} JK/Coin</button></div></article>`;}
  ...
  function bind(shell,item){
    ...
    shell.querySelectorAll("[data-jkc-collection-filter]").forEach(b=>b.addEventListener("click",()=>{ui.collectionRarity=b.dataset.jkcCollectionFilter||"all";refreshPhone(item);}));
  ...
  function openRewardBox(box){const root=rootState();const money=randInt(box.money[0],box.money[1]);const xp=randInt(box.xp[0],box.xp[1]);root.bank=Number(root.bank||0)+money;try{if(typeof addXp==="function")addXp(xp,box.name);}catch{root.xp=Number(root.xp||0)+xp;}const rewards=[{kind:"xp",name:`+${xp} Haupt-EP`,text:"Charakterfortschritt"},{kind:"money",name:`+${euro(money)}`,text:"Bankguthaben"}];
    ...
    for(let i=0;i<box.items;i++){const collectible=createCollectibleWeighted(box.minRarity,box.maxRarity,box.itemValueCap,box),already=!!coinState().collectionUnique[collectible.catalogId];addCollectible(collectible);rewards.push({kind:"item",rarity:collectible.rarity,name:collectible.name,text:`${collectible.rarityName} · ${euro(collectible.value)}${already?" · Bereits entdeckt":""}`,alreadyDiscovered:already});}return rewards;}
