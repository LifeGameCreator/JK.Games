(() => {
  ...
  const GAME_STORE = [
    ...
    { game:"fight", id:"fight-godmode", name:"1-Minute-Godmode", cost:100, text:"Wird bei zehn Prozent Leben automatisch ausgelöst.", grant:{kind:"godmode",amount:1} },
    { game:"fight", id:"fight-godmode-5", name:"5× 1-Minute-Godmode", cost:420, text:"Fünf automatische Notfall-Godmode-Ladungen.", grant:{kind:"godmode",amount:5} },
    ...
    { game:"fight", id:"fight-legendary-crate", name:"Legendäre Arsenal-Kiste", cost:260, text:"Zufälliges legendäres Fight.KL-Item.", grant:{kind:"legendaryItem",amount:1} },
    { game:"fight", id:"fight-universe-crate", name:"Universe-Arsenal-Kiste", cost:520, text:"Zufälliges Universe-Item für Fight.KL.", grant:{kind:"universeItem",amount:1} },
    { game:"fight", id:"fight-blackhole-crate", name:"Black-Hole-Arsenal-Kiste", cost:820, text:"Zufälliges Black-Hole-Item für Fight.KL.", grant:{kind:"blackholeItem",amount:1} },
    { game:"fight", id:"fight-galaxy-crate", name:"Galaxy-Arsenal-Kiste", cost:1200, text:"Zufälliges Galaxy-Endgame-Item für Fight.KL.", grant:{kind:"galaxyItem",amount:1} },
    { game:"fight", id:"fight-galaxy-weapon", name:"Galaxy-Waffenkiste", cost:1450, text:"Garantiert eine Galaxy-Pistole, Langwaffe, Schrotflinte oder Nahkampfwaffe.", grant:{kind:"galaxyWeapon",amount:1} },
    { game:"fight", id:"fight-galaxy-armor", name:"Galaxy-Rüstungskiste", cost:1450, text:"Garantiert Galaxy-Rüstung, Helm, Anzug oder Schuhe.", grant:{kind:"galaxyArmor",amount:1} },
  ...
  function gamesHtml(){const games=["all","runner","city","match","fight","dungeon","weed","casino"];const labels={all:"Alle",runner:"Runner.KL",city:"City.KL",match:"Match.KL",fight:"Fight.KL",dungeon:"Dungeon.KL",weed:"Weed Business",casino:"Casino"};const list=GAME_STORE.filter(x=>ui.game==="all"||x.game===ui.game);const filters=games.map(g=>`<button class="jkc-button ${ui.game===g?"gold":"secondary"}" data-jkc-game-filter="${g}">${labels[g]}</button>`).join("");return `<section class="jkc-section"><small>SPIELE-SHOP</small><h4>JK/Coin-Inhalte</h4>${scrollNavHtml("games",filters)}<div class="jkc-grid jkc-game-grid">${list.map(item=>`<article class="jkc-card"><small>${labels[item.game].toUpperCase()}</small><h5>${esc(item.name)}</h5><p>${esc(item.text)}</p><div class="jkc-actions"><button class="jkc-button" data-jkc-buy-game="${item.id}">${item.variable?"Betrag wählen":`${item.cost} JK/Coin`}</button></div></article>`).join("")}</div></section>`;}
