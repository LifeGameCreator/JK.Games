(() => {
  ...
  const VERSION = "2026-08-07-jkcoin-v232-euro-box-prices";
  ...
  const BOXES = [
    ...
    { id:"reward-5000", name:"Galaxy Lucky Box", cost:5500, currency:"jk", type:"reward", tier:"galaxy", money:[50000000,1000000000], xp:[8000,40000], clothingGuaranteed:4, clothingChance:.80, skinChance:1, cashbackChance:.72, cashback:[120,400] },

    // Euro-Lucky-Boxes: gleiche Stufen, aber ungefähr 50 % schwächer als die JK/Coin-Varianten.
    { id:"reward-euro-100", name:"Starter Lucky Box · Euro", cost:100000, currency:"euro", type:"reward", tier:"starter", money:[5000,25000], xp:[30,125], clothingGuaranteed:0, clothingChance:.30, skinChance:.02, cashbackChance:.09, cashback:[5,15] },
    { id:"reward-euro-500", name:"Premium Lucky Box · Euro", cost:1000000, currency:"euro", type:"reward", tier:"premium", money:[37500,175000], xp:[175,650], clothingGuaranteed:.5, clothingChance:.225, skinChance:.09, cashbackChance:.14, cashback:[10,35] },
    { id:"reward-euro-1000", name:"Elite Lucky Box · Euro", cost:10000000, currency:"euro", type:"reward", tier:"elite", money:[150000,750000], xp:[500,2000], clothingGuaranteed:1, clothingChance:.275, skinChance:.17, cashbackChance:.19, cashback:[25,80] },
    { id:"reward-euro-2000", name:"Universe Lucky Box · Euro", cost:100000000, currency:"euro", type:"reward", tier:"universe", money:[500000,4000000], xp:[1250,5000], clothingGuaranteed:1.5, clothingChance:.325, skinChance:.29, cashbackChance:.25, cashback:[50,160] },
    { id:"reward-euro-5000", name:"Galaxy Lucky Box · Euro", cost:1000000000, currency:"euro", type:"reward", tier:"galaxy", money:[25000000,500000000], xp:[4000,20000], clothingGuaranteed:2, clothingChance:.40, skinChance:.50, cashbackChance:.36, cashback:[120,400] },

    // Sammler-Kisten mit JK/Coin: bisherige Item-Anzahl und Chancen bleiben bestehen.
    { id:"collect-100", name:"Sammler-Kiste 100", cost:100, currency:"jk", type:"collect", tier:"basic", items:5, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-500", name:"Sammler-Kiste 500", cost:500, currency:"jk", type:"collect", tier:"premium", items:10, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-1000", name:"Sammler-Kiste 1000", cost:1000, currency:"jk", type:"collect", tier:"elite", items:15, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-2000", name:"Sammler-Kiste 2000", cost:2000, currency:"jk", type:"collect", tier:"master", items:20, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-5000", name:"Sammler-Kiste 5000", cost:5000, currency:"jk", type:"collect", tier:"galaxy", items:25, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },

    // Euro-Sammler-Kisten: gleiche Item-Anzahl, aber jede Seltenheitschance ist halbiert.
    { id:"collect-euro-100", name:"Sammler-Kiste 100 · Euro", cost:100000, currency:"euro", type:"collect", tier:"basic", chanceScale:.5, items:5, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-euro-500", name:"Sammler-Kiste 500 · Euro", cost:1000000, currency:"euro", type:"collect", tier:"premium", chanceScale:.5, items:10, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-euro-1000", name:"Sammler-Kiste 1000 · Euro", cost:10000000, currency:"euro", type:"collect", tier:"elite", chanceScale:.5, items:15, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-euro-2000", name:"Sammler-Kiste 2000 · Euro", cost:100000000, currency:"euro", type:"collect", tier:"master", chanceScale:.5, items:20, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-euro-5000", name:"Sammler-Kiste 5000 · Euro", cost:1000000000, currency:"euro", type:"collect", tier:"galaxy", chanceScale:.5, items:25, itemValueCap:NON_GALAXY_BOX_ITEM_CAP }
  ];
  ...
  function html(){
    ...
    const tabs=[["home","Übersicht"],["boxes","Lucky Boxes"],["collectorBoxes","Sammlerkisten"],["games","Spiele"],["collection","Sammlung"],["ledger","Kontobewegungen"]].map(([id,label])=>`<button type="button" class="jkc-nav-pill ${ui.tab===id?"active":""}" data-jkc-tab="${id}">${label}</button>`).join("");
  ...
  function boxCard(b){const probs=b.type==="collect"?collectChancesForBox(b):null,payment=b.currency==="euro"?euro(b.cost):`${b.cost.toLocaleString("de-DE")} JK/Coin`,kindLabel=b.type==="collect"?"SAMMLER-KISTE":"LUCKY BOX",currencyLabel=b.currency==="euro"?"EURO":"JK/COIN";return `<article class="jkc-card jkc-box" style="--box-color:${b.tier==="galaxy"?"#ff72d7":["elite","master","universe"].includes(b.tier)?"#a46cff":"#64e6ff"}"><div class="jkc-box-icon">${b.type==="collect"?"◈":"🎁"}</div><small>${currencyLabel} · ${kindLabel}</small><h5>${esc(b.name)}</h5><p>${b.type==="reward"?`${euro(b.money[0])} bis ${euro(b.money[1])}, ${b.xp[0]}–${b.xp[1]} Haupt-EP, Kleidung, Hauptskins und weitere Belohnungen.`:`${Number(b.items)||1} Sammlerstücke pro Kiste.`}</p>${probs?`<div class="jkc-prob-list">${["epic","legendary","special","universe","blackhole","galaxy"].map(id=>`<span>${RARITIES.find(r=>r.id===id).name}<br><b>${formatChance(probs[id])}%</b></span>`).join("")}</div>`:""}<div class="jkc-actions"><button class="jkc-button gold" data-jkc-open-box="${b.id}">${payment}</button></div></article>`;}
  ...
  function bind(shell,item){
    bindScrollNav(shell);
    bindCollectionFilterScroll(shell);
  ...
  function rewardIcon(reward){if(reward?.kind==="xp")return "XP";if(reward?.kind==="money")return "€";if(reward?.kind==="coin")return "JK";if(reward?.kind==="clothing")return "👕";if(reward?.kind==="special")return "★";return reward?.rarity==="galaxy"?"🌌":reward?.rarity==="blackhole"?"🕳️":"◈";}
