(() => {
  ...
  function jkcConfirm({title="Bitte bestätigen",message="",confirmText="Bestätigen",cancelText="Abbrechen",icon="◆",tone="normal"}={}){
    return new Promise(resolve=>{
      ...
      const wrap=document.createElement("div");wrap.className=`jkc-confirm-overlay tone-${tone}`;wrap.dataset.jkcConfirm="1";
      ...
      (document.fullscreenElement||document.body||document.documentElement).append(wrap);setTimeout(()=>wrap.querySelector(".jkc-confirm-ok")?.focus(),0);
    });
  }
  function jkcNumberPrompt({title="Betrag eingeben",message="",value=1,min=1,max=Number.MAX_SAFE_INTEGER,confirmText="Übernehmen",icon="◆"}={}){
    return new Promise(resolve=>{
      document.querySelector("[data-jkc-confirm]")?.remove();
      const wrap=document.createElement("div");wrap.className="jkc-confirm-overlay";wrap.dataset.jkcConfirm="1";
      ...
      (document.fullscreenElement||document.body||document.documentElement).append(wrap);setTimeout(()=>{input?.focus();input?.select();},0);
    });
  }
  ...
  function gameShopCategory(entry){
    ...
    if(/auto|bulklevel|bulkrebirth|unlock|komfort|collector|opener/.test(hay))return"comfort";
