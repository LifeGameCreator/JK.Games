(() => {
  ...
  const VERSION = "2026-08-07-jkcoin-v229-lucky-no-collectibles-front-dialog";
  ...
  const BOXES = [
    { id:"reward-100", name:"Starter Lucky Box", cost:150, type:"reward", tier:"starter", money:[15000,75000], xp:[60,250], clothingGuaranteed:0, clothingChance:.60, skinChance:.04, cashbackChance:.18, cashback:[5,15] },
    { id:"reward-500", name:"Premium Lucky Box", cost:550, type:"reward", tier:"premium", money:[100000,600000], xp:[350,1300], clothingGuaranteed:1, clothingChance:.45, skinChance:.18, cashbackChance:.28, cashback:[10,35] },
    { id:"reward-1000", name:"Elite Lucky Box", cost:1500, type:"reward", tier:"elite", money:[400000,2500000], xp:[1000,4000], clothingGuaranteed:2, clothingChance:.55, skinChance:.34, cashbackChance:.38, cashback:[25,80] },
    { id:"reward-2000", name:"Universe Lucky Box", cost:2500, type:"reward", tier:"universe", money:[1500000,30000000], xp:[2500,10000], clothingGuaranteed:3, clothingChance:.65, skinChance:.58, cashbackChance:.50, cashback:[50,160] },
    { id:"reward-5000", name:"Galaxy Lucky Box", cost:5500, type:"reward", tier:"galaxy", money:[150000000,2000000000], xp:[8000,40000], clothingGuaranteed:4, clothingChance:.80, skinChance:1, cashbackChance:.72, cashback:[120,400] },
    { id:"collect-100", name:"Sammler-Kiste 100", cost:100, type:"collect", tier:"basic", items:5, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-500", name:"Sammler-Kiste 500", cost:500, type:"collect", tier:"premium", items:10, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-1000", name:"Sammler-Kiste 1000", cost:1000, type:"collect", tier:"elite", items:15, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-2000", name:"Sammler-Kiste 2000", cost:2000, type:"collect", tier:"master", items:20, itemValueCap:NON_GALAXY_BOX_ITEM_CAP },
    { id:"collect-5000", name:"Sammler-Kiste 5000", cost:5000, type:"collect", tier:"galaxy", items:25, itemValueCap:NON_GALAXY_BOX_ITEM_CAP }
  ];
  ...
  function boxCard(b){const probs=b.type==="collect"?COLLECT_CHANCES[b.tier]:null;return `<article class="jkc-card jkc-box" style="--box-color:${b.cost>=5000?"#ff72d7":b.cost>=1000?"#a46cff":"#64e6ff"}"><div class="jkc-box-icon">${b.type==="collect"?"◈":"🎁"}</div><small>${b.type==="collect"?"SAMMLER-KISTE":"LUCKY BOX"}</small><h5>${esc(b.name)}</h5><p>${b.type==="reward"?`${euro(b.money[0])} bis ${euro(b.money[1])}, ${b.xp[0]}–${b.xp[1]} Haupt-EP, Kleidung, Hauptskins und weitere Premium-Belohnungen.`:`${Number(b.items)||1} Sammlerstücke pro Kiste.`}</p>${probs?`<div class="jkc-prob-list">${["epic","legendary","special","universe","blackhole","galaxy"].map(id=>`<span>${RARITIES.find(r=>r.id===id).name}<br><b>${probs[id]}%</b></span>`).join("")}</div>`:""}<div class="jkc-actions"><button class="jkc-button gold" data-jkc-open-box="${b.id}">${b.cost.toLocaleString("de-DE")} JK/Coin</button></div></article>`;}
