(() => {
  ...
  const VERSION = "2026-08-08-jkcoin-v248-egoshoot-kl";
  ...
  const REAL_GALAXY_SKIN = "The Real Galaxy Skin";
  ...
  const BOXES = [
    ...
    { id:"reward-5000", name:"Galaxy Lucky Box", cost:5500, currency:"jk", type:"reward", tier:"galaxy", money:[50000000,1000000000], xp:[8000,40000], clothingGuaranteed:4, clothingChance:.80, skinChance:1, realGalaxySkinChance:.08, cashbackChance:.72, cashback:[120,400] },
    ...
    { id:"reward-euro-5000", name:"Galaxy Lucky Box · 1B", cost:1000000000, currency:"euro", type:"reward", tier:"galaxy", money:[180000000,800000000], xp:[4000,20000], clothingGuaranteed:2, clothingChance:.40, skinChance:.50, realGalaxySkinChance:.04, cashbackChance:0, cashback:[0,0] },
  ...
  function grantLuckySkin(box,rewards){if(Math.random()>=Math.max(0,Math.min(1,Number(box?.skinChance)||0)))return;const root=rootState(),tier=luckySkinTier(box);root.wardrobe=Array.isArray(root.wardrobe)?root.wardrobe:[];const realGalaxyAvailable=box?.tier==="galaxy"&&!root.wardrobe.includes(REAL_GALAXY_SKIN)&&Math.random()<Math.max(0,Math.min(1,Number(box?.realGalaxySkinChance)||0));let skin=realGalaxyAvailable?REAL_GALAXY_SKIN:"";for(let tries=0;tries<20&&!skin;tries++){const candidate=`JK Exclusive ${tier} Skin #${randInt(1,999)}`;if(!root.wardrobe.includes(candidate))skin=candidate;}if(!skin)return;root.wardrobe.push(skin);coinState().entitlements[`main-skin:${skin}`]=1;rewards.push({kind:"special",name:skin,text:skin===REAL_GALAXY_SKIN?"Einzigartiger hochauflösender Galaxy-Hauptskin · dauerhaft kontogebunden und nicht verkäuflich":"Exklusiver Hauptcharakter-Skin · im Kleiderschrank verfügbar",value:skin===REAL_GALAXY_SKIN?0:luckySkinValue(tier)});}
