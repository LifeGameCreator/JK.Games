(() => {
  ...
  function dynamicStoreSignature(){const boost=["money-production","bigcards-points","bigcards-xp","bigcards-damage"].map(f=>{const st=dynamicBoosterState(f);return `${f}:${st.active?1:0}:${Number(st.value)||0}`}).join("|");const bulk=bigCardsBulkAccessMeta({game:"bigcards",grant:{kind:"bulkLevelUnlock"}});return `${boost}|bigcards-bulk:${bulk?.active?1:0}`;}
