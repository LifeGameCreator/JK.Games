(() => {
  ...
  const VERSION = "2026-08-07-jkcoin-v235-hype-special-lucky-balance";
  ...
  const BOXES = [
    ...
    { id:"reward-euro-5000", name:"Galaxy Lucky Box · 1B", cost:1000000000, currency:"euro", type:"reward", tier:"galaxy", money:[180000000,800000000], xp:[4000,20000], clothingGuaranteed:2, clothingChance:.40, skinChance:.50, cashbackChance:0, cashback:[0,0] },
  ...
  function hypeForBox(box){if(box?.type!=="reward")return 0;if(box?.currency==="euro")return Math.max(1,Math.round(Number(box.cost||0)/100000));const premiumHype={150:100,550:400,1500:1200,2500:3000,5500:15000};return premiumHype[Math.floor(Number(box.cost)||0)]||Math.max(100,Math.round(Number(box.cost||0)*.8));}
  function addHypeForBox(box,rewards){const c=coinState(),amount=hypeForBox(box);if(!c||!amount)return 0;c.hype.points+=amount;c.hype.totalBoxes+=1;if(box?.currency==="euro")c.hype.euroBoxes+=1;else c.hype.jkBoxes+=1;c.hype.updatedAtMs=Date.now();rewards?.push?.({kind:"hype",name:`+${amount.toLocaleString("de-DE")} % Hype`,text:`Hype-Status · ${c.hype.points.toLocaleString("de-DE")} % gesamt`,value:0});return amount;}
  async function syncHypeProfile(){try{const c=coinState(),fb=await runtime(),user=await currentUser(fb);if(!c||!user)return false;await fb.setDoc(fb.doc(fb.db,HYPE_COLLECTION,user.uid),{uid:user.uid,displayName:userName().slice(0,80)||"Spieler",hype:Math.max(0,Math.floor(c.hype.points)),totalBoxes:Math.max(0,Math.floor(c.hype.totalBoxes)),euroBoxes:Math.max(0,Math.floor(c.hype.euroBoxes)),jkBoxes:Math.max(0,Math.floor(c.hype.jkBoxes)),updatedAtMs:Date.now()},{merge:true});return true;}catch(error){console.debug?.("Hype-Sync pausiert",error?.message||error);return false;}}
  ...
  function init(){coinState();installSettingsCard();installHypeShortcut();const observer=new MutationObserver(()=>{installSettingsCard();decorateActiveViews();installGameShortcuts();installHypeShortcut();});observer.observe(document.documentElement,{childList:true,subtree:true});decorateActiveViews();installGameShortcuts();window.addEventListener("lifebuilder-local-save-flushed",()=>updateSettingsBalance());window.addEventListener("online",()=>syncHypeProfile().catch(()=>{}));setInterval(()=>syncGrants().catch(()=>{}),15000);setInterval(()=>{applyPendingGameEntitlements();syncProfileBalance();},30000);setTimeout(()=>{applyPendingGameEntitlements();syncProfileBalance();installGameShortcuts();installHypeShortcut();},2500);}

  window.JKCoinApp=Object.freeze({version:VERSION,html,bind,bankPanelHtml,bindBank,requestPack,coinState,currentRate,renderOwnerPanel,credit,spend,syncGrants,syncProfileBalance,applyPendingGameEntitlements,openForGame,openHype,syncHypeProfile,gameStore:GAME_STORE,boxes:BOXES});
  init();
})();
