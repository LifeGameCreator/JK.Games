...
const S = {
  ...
  worldIndex:0,worldTheme:WORLD_THEMES[0],worldAnnounceClock:0,effectiveSpeed:12,runStartedAt:0,remoteRunId:'',remoteBusy:false,worldData:null,worldDataLoaded:false,worldDataError:'',rewardClaiming:false,nextTramDistance:120,nextJkDropDistance:250,
...
function runnerFunctionsBlockedByOrigin(){const host=String(location.hostname||'').toLowerCase();if(window.JK_RUNNER_FUNCTIONS_PROXY||window.JK_RUNNER_FUNCTIONS_ENABLED===true)return false;return host.endsWith('github.io')||host.endsWith('pages.dev')}
async function callRunnerFunction(name,data={},requireAuth=true){if(runnerFunctionsBlockedByOrigin())throw new Error('Runner Online-Funktionen sind auf dieser Hosting-Domain noch nicht freigegeben.');const {fb,user}=await firebaseRuntime(requireAuth);if(typeof fb.httpsCallable!=='function'||!fb.functions)throw new Error('Firebase Functions sind nicht verfügbar.');const fn=fb.httpsCallable(fb.functions,name);const result=await fn(data);return {data:result?.data||{},user}}
async function beginRemoteRun(){S.remoteRunId='';S.runStartedAt=Date.now();try{const r=await callRunnerFunction('runnerStartRun',{});S.remoteRunId=String(r.data.runId||'')}catch(e){/* lokaler Lauf ohne fehlerhafte CORS-Anfrage */}}
...
async function loadWorldData(force=false){if(runnerFunctionsBlockedByOrigin()){S.worldDataError='Online-Rangliste benötigt eine für Firebase Functions freigegebene Hosting-Domain.';return null;}if(S.worldDataLoaded&&!force)return S.worldData;try{const r=await callRunnerFunction('runnerGetWorldData',{limit:10},false);S.worldData=r.data;S.worldDataLoaded=true;S.worldDataError='';const top=Number(S.worldData?.global?.[0]?.distance||0);if(top>S.best)S.best=top;updateHud();return S.worldData}catch(e){S.worldDataError=e?.message||String(e);return null}}
...
function setupScene(){
  ...
  S.renderer.shadowMap.enabled=S.quality!=='low';S.renderer.shadowMap.type=THREE.PCFShadowMap;S.renderer.shadowMap.autoUpdate=true;
...
function update(dt){
  ...
  const boostFactor=S.activeBoosts.jetpack>0?2.08:(S.activeBoosts.sneakers>0?1.27:1)*(S.hoverboard>0?1.14:1);S.effectiveSpeed=S.speed*boostFactor;S.distance+=S.effectiveSpeed*dt*.52;S.score+=S.effectiveSpeed*dt*mult;while(S.distance>=S.nextJkDropDistance){const checkpoint=S.nextJkDropDistance;const won=window.JKCoinApp?.rollTopGameDrop?.("runner","distance250",`runner:${S.runStartedAt}:${checkpoint}`)||0;if(won)showBoostToast("GRATIS JK/COIN",`+${won} JK/Coin gefunden`);S.nextJkDropDistance+=250;}updateWorldZone();
...
function togglePause(){
  ...
  showCard(`<div class="rkl-kicker">RUNNER.KL</div><h1>Pause</h1><p class="rkl-lead">Der Lauf ist angehalten.</p><button data-rkl-resume>Weiter</button><button class="rkl-secondary" data-rkl-restart>Neu starten</button><button class="rkl-secondary" data-rkl-exit>Top Games</button><button class="rkl-secondary jkc-game-menu-button" data-rkl-jkcoin title="JK/Coin öffnen">JK</button>`);
  S.overlay.querySelector('[data-rkl-resume]').onclick=resumeWithCountdown;S.overlay.querySelector('[data-rkl-restart]').onclick=resetGame;S.overlay.querySelector('[data-rkl-exit]').onclick=returnToTopGames;S.overlay.querySelector('[data-rkl-jkcoin]').onclick=()=>window.JKCoinApp?.openForGame?.('runner')
}
