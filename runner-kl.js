...
async function callRunnerFunction(name,data={},requireAuth=true){if(location.hostname.endsWith('github.io')&&!window.JK_RUNNER_FUNCTIONS_PROXY)throw new Error('Runner Online-Funktionen sind auf GitHub Pages deaktiviert.');const {fb,user}=await firebaseRuntime(requireAuth);if(typeof fb.httpsCallable!=='function'||!fb.functions)throw new Error('Firebase Functions sind nicht verfügbar.');const fn=fb.httpsCallable(fb.functions,name);const result=await fn(data);return {data:result?.data||{},user}}
...
async function loadWorldData(force=false){if(location.hostname.endsWith('github.io')&&!window.JK_RUNNER_FUNCTIONS_PROXY){S.worldDataError='Online-Rangliste benötigt eine freigegebene Functions-Domain.';return null;}if(S.worldDataLoaded&&!force)return S.worldData;try{const r=await callRunnerFunction('runnerGetWorldData',{limit:10},false);S.worldData=r.data;S.worldDataLoaded=true;S.worldDataError='';const top=Number(S.worldData?.global?.[0]?.distance||0);if(top>S.best)S.best=top;updateHud();return S.worldData}catch(e){S.worldDataError=e?.message||String(e);return null}}
