(() => {
  ...
  function stopRoomListeners(){
    ...
    clearTimeout(UI.shotResolveTimer);UI.shotResolveTimer=0;
  ...
  function destroyScene(){try{window.ChamberKLScene?.destroy?.();}catch{}UI.sceneMounted=false;UI.gameDomReady=false;}
  ...
  function sceneOptions(room,mode="game"){
    const canAim=mode==="game"&&room?.status==="playing"&&room?.phase==="playing"&&room?.activeUid===ownUid();
    return {room,mode,ownUid:ownUid(),selectedTarget:canAim?UI.selectedTarget:"",cosmetics:sceneCosmetics(room),crateRevealStep:UI.crateRevealStep,onCrateTap:revealNextCrateItem,onSelectTarget:(uid)=>{if(UI.room?.status==="playing"&&UI.room.phase==="playing"&&UI.room.activeUid===ownUid()&&Number(UI.room.hp?.[uid]||0)>0)selectGameTarget(uid);}};
  }
  function mountScene(room,mode="game"){
    const host=UI.shell?.querySelector("[data-chkl-scene]");if(!host)return;const api=window.ChamberKLScene;if(!api?.mount)return;try{api.mount(host,sceneOptions(room,mode));UI.sceneMounted=true;}catch(error){console.warn("Chamber.KL 3D",error);}
  }
  function updateScene(room,mode="game"){
    const api=window.ChamberKLScene;if(!UI.sceneMounted||!api?.updateRoom){mountScene(room,mode);return;}try{api.updateRoom(room,sceneOptions(room,mode));}catch(error){console.warn("Chamber.KL 3D update",error);}
  }
  ...
  function targetStrip(room){
    const own=ownUid();return `<div class="chkl-target-strip chkl-target-strip-v4"><small>WOHIN SOLL DIE PUMPGUN ZIELEN?</small><div>${aliveUids(room).map(uid=>{const p=roomPlayer(room,uid),hp=Number(room.hp?.[uid]||0),sel=UI.selectedTarget===uid,isSelf=uid===own;return `<button class="${sel?"active":""} ${isSelf?"self":""}" data-chkl-target="${esc(uid)}"><span>${isSelf?"SELBSTSCHUSS":"GEGNER"}</span><b>${isSelf?"AUF MICH SELBST":`AUF ${esc(p.name)}`}</b>${hpHtml(hp)}</button>`;}).join("")}</div></div>`;
  }
  function matchHud(room){
    const active=roomPlayer(room,room.activeUid),own=ownUid(),mine=room.activeUid===own&&room.phase==="playing";
    const resolving=room.phase==="reloading"&&["shot-live","shot-blank"].includes(room.lastEvent?.type);
    const supply=room.phase==="reloading"&&!resolving;
    const calloutClass=mine?"mine":resolving?"resolving":supply?"supply":"other";
    const calloutTitle=mine?"DU BIST DRAN":resolving?"SCHUSS LÄUFT":supply?"NEUE RUNDE":`${esc(active.name)} IST DRAN`;
    const calloutText=mine?"Ziel auswählen, optional Items benutzen, dann abziehen.":resolving?"Treffer oder Leerschuss wird erst vollständig aufgelöst.":supply?"Kiste öffnen, Items nehmen und Kammer laden.":"Der Tisch ist gesperrt, bis der Gegner fertig ist.";
    return `<div class="chkl-turn-callout ${calloutClass}"><strong>${calloutTitle}</strong><span>${calloutText}</span></div><div class="chkl-game-hud"><div class="chkl-round"><small>RUNDE</small><b>${room.round}</b></div><div class="chkl-event"><small>LETZTE AKTION</small><b>${esc(lastEventText(room))}</b></div><div class="chkl-ammo"><span class="live"><i></i><b>${room.shellLive}</b><small>SCHARF</small></span><span class="blank"><i></i><b>${room.shellBlank}</b><small>LEER</small></span></div><div class="chkl-active ${mine?"mine":""}"><small>${mine?"DEIN ZUG":supply?"RUNDE":resolving?"AUFLÖSUNG":"AM ZUG"}</small><b>${supply?"Versorgung":resolving?"Schuss":esc(active.name)}</b><span data-chkl-timer>${mine||(!supply&&!resolving)?"--":"—"}</span></div></div>`;
  }
  function actionHtml(room){
    const uid=ownUid(),alive=Number(room.hp?.[uid]||0)>0,ownTurn=room.activeUid===uid&&alive,target=UI.selectedTarget&&Number(room.hp?.[UI.selectedTarget]||0)>0?UI.selectedTarget:"",items=room.items?.[uid]||[],jammed=!!room.effects?.[uid]?.jammed;
    const shotResolving=room.phase==="reloading"&&["shot-live","shot-blank"].includes(room.lastEvent?.type);
    if(shotResolving){const e=room.lastEvent;return `<section class="chkl-v2-controls spectate chkl-shot-resolving"><div><small>${e.type==="shot-live"?"SCHUSS":"KLICK"}</small><b>${esc(roomPlayer(room,e.actorUid).name)} → ${esc(roomPlayer(room,e.targetUid).name)}</b><span>Die Aktion wird vollständig abgespielt. Danach wird erst der nächste Zug freigegeben.</span></div></section>`;}
    if(room.phase==="reloading"){const drops=crateDrops(room),done=Math.min(UI.crateRevealStep,drops.length);return `<section class="chkl-v2-controls reload chkl-v3-supply"><div><small>VERSORGUNGSRUNDE</small><b>Versorgungskiste · danach wird die Pumpgun geladen</b><span>Acht Tischplätze: vier links, vier rechts. Tippe die Kiste für deine neuen Items an.</span>${drops.length?`<button class="primary" data-chkl-crate-tap ${done>=drops.length?"disabled":""}>${done>=drops.length?"ALLE ITEMS AUSGEGEBEN":done?`NÄCHSTES ITEM (${done}/${drops.length})`:"KISTE ÖFFNEN"}</button>`:`<em>Inventar voll – keine neuen Items.</em>`}</div></section>`;}
    if(!alive)return `<section class="chkl-v2-controls spectate"><div><small>AUSGESCHIEDEN</small><b>Du beobachtest den restlichen Tisch live.</b></div></section>`;
    if(!ownTurn)return `<section class="chkl-v2-controls spectate"><div><small>GEGNER AM ZUG</small><b>${esc(roomPlayer(room,room.activeUid).name)} entscheidet.</b><span>Deine Items und dein Abzug sind bis zu deinem nächsten Zug gesperrt.</span></div></section>`;
    return `<section class="chkl-v2-controls ready chkl-ready-v4">${targetStrip(room)}<div class="chkl-v2-item-drawer"><div class="chkl-panel-title"><div><small>DEINE TISCH-ITEMS</small><b>${jammed?"STÖRSENDER — ITEMS GESPERRT":"Nur jetzt vor dem Schuss einsetzbar"}</b></div><span>${items.length}/8</span></div><div class="chkl-v2-items">${items.length?items.map((id,i)=>itemCard(room,id,i)).join(""):`<div class="chkl-no-items">Keine Items mehr</div>`}</div></div><div class="chkl-v2-fire"><div><small>AUSGEWÄHLTES ZIEL</small><b>${target?esc(roomPlayer(room,target).name):"Ziel auswählen"}</b><span>${target===uid?"Leer = du bleibst dran · scharf = du triffst dich selbst":"Scharf = Ziel erhält Schaden · leer = Zug endet"}</span></div><button class="primary danger" data-chkl-shoot ${target?"":"disabled"}><i></i>ABZIEHEN</button></div></section>`;
  }
  function seatTagsHtml(room){
    const order=room.turnOrder||room.playerUids||[],n=Math.max(2,order.length||2),ownIndex=Math.max(0,order.indexOf(ownUid()));
    return order.map((uid,index)=>{const relative=(index-ownIndex+n)%n,p=roomPlayer(room,uid),hp=Number(room.hp?.[uid]||0),eff=room.effects?.[uid]||{};return `<button class="seat-${relative} seats-${n} ${room.activeUid===uid?"active":""} ${UI.selectedTarget===uid?"target":""} ${hp<=0?"out":""}" data-chkl-target="${esc(uid)}" ${hp<=0?"disabled":""}><small>${uid===ownUid()?"DU":isBotUid(uid)?`BOT · ${String(p.botDifficulty||"easy").toUpperCase()}`:uid===room.hostUid?"HOST":"SPIELER"}</small><b>${esc(p.name)}</b>${hpHtml(hp)}<span>${eff.plate?"⬡":""}${eff.doubleNext?"⌁":""}${eff.jammed?"⌁":""}</span></button>`;}).join("");
  }
  function selectGameTarget(uid){
    const room=UI.room;if(UI.actionBusy||!room||room.status!=="playing"||room.phase!=="playing"||room.activeUid!==ownUid()||Number(room.hp?.[uid]||0)<=0)return;UI.selectedTarget=String(uid);try{window.ChamberKLScene?.setAimTarget?.(UI.selectedTarget);}catch{}updateGameDom(room,false);
  }
  function bindGameControls(){
    UI.shell?.querySelector("[data-chkl-crate-tap]")?.addEventListener("click",revealNextCrateItem);
    UI.shell?.querySelectorAll("[data-chkl-target]").forEach(btn=>btn.addEventListener("click",()=>selectGameTarget(btn.dataset.chklTarget)));
    UI.shell?.querySelector("[data-chkl-shoot]")?.addEventListener("click",()=>sendAction("shoot",{targetUid:UI.selectedTarget}));
    UI.shell?.querySelectorAll("[data-chkl-item]").forEach(btn=>btn.addEventListener("click",()=>{const id=btn.dataset.chklItem,item=ITEM_DEFS[id];if(UI.room?.phase!=="playing"||UI.room?.activeUid!==ownUid())return toast("Items kannst du nur in deinem eigenen Zug vor dem Schuss benutzen.","error");if(UI.room?.effects?.[ownUid()]?.jammed)return toast("Der Störsender blockiert deine Items in diesem Zug.","error");if(item?.target&&(!UI.selectedTarget||UI.selectedTarget===ownUid()))return toast("Wähle zuerst einen anderen lebenden Spieler als Ziel.","error");sendAction("item",{itemId:id,targetUid:item?.target?UI.selectedTarget:""});}));
  }
  function updateGameDom(room,refreshScene=true){
    const hud=UI.shell?.querySelector("[data-chkl-hud]"),seats=UI.shell?.querySelector("[data-chkl-seats]"),actions=UI.shell?.querySelector("[data-chkl-actions]");if(!hud||!seats||!actions)return false;hud.innerHTML=matchHud(room);seats.innerHTML=seatTagsHtml(room);actions.innerHTML=actionHtml(room);bindGameControls();if(refreshScene)updateScene(room,"game");else try{window.ChamberKLScene?.setAimTarget?.(UI.selectedTarget);}catch{}startTicker();return true;
  }
  function renderGame(){
    const room=UI.room;if(!room||!UI.shell)return;const living=aliveUids(room);if(!UI.selectedTarget||!living.includes(UI.selectedTarget)){const own=ownUid();UI.selectedTarget=living.find(uid=>uid!==own)||living[0]||"";}if(room.status==="finished")return renderFinished();
    if(UI.view==="game"&&UI.gameDomReady&&UI.shell.querySelector(".chkl-game-v4")){updateGameDom(room,true);return;}
    destroyScene();UI.view="game";UI.shell.innerHTML=`${topbar(`RUNDE ${room.round}`)}<main class="chkl-game chkl-game-v2 chkl-game-v4"><section class="chkl-v2-stage chkl-v4-stage"><div class="chkl-scene-frame game" data-chkl-scene><div class="chkl-scene-fallback"><span>3D TABLE</span></div></div><div data-chkl-hud></div><div class="chkl-seat-tags" data-chkl-seats></div></section><div data-chkl-actions></div></main>`;bindTopbar();UI.gameDomReady=true;updateGameDom(room,false);mountScene(room,"game");startTicker();
  }
  function startTicker(){clearInterval(UI.ticker);const update=()=>{const node=UI.shell?.querySelector("[data-chkl-timer]");if(!node||!UI.room)return;if(UI.room.phase!=="playing"||!UI.room.turnStartedAtMs){node.textContent="—";node.classList.remove("urgent");return;}const left=Math.max(0,TURN_MS-(now()-Number(UI.room.turnStartedAtMs||now())));node.textContent=`${Math.ceil(left/1000)}s`;node.classList.toggle("urgent",left<6000);};update();UI.ticker=setInterval(update,500);}
  ...
  async function sendAction(action,payload={}){
    if(UI.actionBusy||!UI.room||UI.room.status!=="playing")return false;
    if(UI.room.phase!=="playing")return toast(["shot-live","shot-blank"].includes(UI.room.lastEvent?.type)?"Der Schuss wird gerade noch ausgeführt.":"Die Kammer wird gerade neu geladen.","error");
    const uid=ownUid();if(UI.room.activeUid!==uid&&action!=="leave")return toast("Du bist gerade nicht am Zug.","error");
    setBusy(true);let guestPending=false;
    try{
      if(isHost(UI.room)){
        ...
        const {fb}=await firebase();const id=`${uid.slice(0,8)}-${now()}-${randomInt(9999)}`;const data={uid,action,targetUid:String(payload.targetUid||""),itemId:String(payload.itemId||""),createdAtMs:now(),status:"pending"};const ref=commandRef(fb,UI.room.roomId,id);
        await fb.setDoc(ref,data);guestPending=true;listenOwnCommand(fb,ref,id,action,payload);
      }
      return true;
    }catch(error){toast(error.message||error,"error");return false;}
    finally{if(!guestPending)setTimeout(()=>setBusy(false),220);}
  }
  ...
  function listenOwnCommand(fb,ref,id,action,payload={}){
    let settled=false;
    const release=()=>{if(settled)return;settled=true;setBusy(false);};
    const unsub=fb.onSnapshot(ref,snap=>{if(!snap.exists())return;const d=snap.data();if(d.status==="done"||d.status==="denied"){if(d.status==="done")applyGuestCommandStats(action,payload,d.result);if(d.result)toast(d.result,d.status==="done"?"good":"error");try{unsub();}catch{}UI.ownCommandUnsubs.delete(id);release();setTimeout(()=>fb.deleteDoc(ref).catch(()=>{}),5000);}},()=>{release();});
    UI.ownCommandUnsubs.set(id,unsub);
    setTimeout(()=>{if(UI.ownCommandUnsubs.has(id)){release();toast("Die Aktion wartet noch auf den Host. Du kannst es erneut versuchen.","error");}},12000);
  }
  ...
  function ensureHostRuntime(){
    ...
    if(UI.room?.phase==="reloading"&&["shot-live","shot-blank"].includes(UI.room?.lastEvent?.type)&&!UI.shotResolveTimer){const elapsed=Math.max(0,now()-Number(UI.room.lastEvent?.atMs||now())),delay=Math.max(80,SHOT_RESOLVE_MS-elapsed);UI.shotResolveTimer=setTimeout(()=>{UI.shotResolveTimer=0;hostResolveShot(UI.roomId,UI.room?.eventSeq).catch(()=>{});},delay);}
  ...
  function scheduleShotResolution(roomId,eventSeq){
    clearTimeout(UI.shotResolveTimer);UI.shotResolveTimer=setTimeout(()=>{UI.shotResolveTimer=0;hostResolveShot(roomId,eventSeq).catch(error=>console.warn("Chamber.KL Shot resolve",error));},SHOT_RESOLVE_MS);
  }
  async function hostResolveShot(roomId,eventSeq){
    if(!roomId||UI.roomId!==roomId)return;const {fb}=await firebase();const ref=roomRef(fb,roomId);const snap=await fb.getDoc(ref);if(!snap.exists())return;const room={id:snap.id,...snap.data()},event=room.lastEvent||{};
    if(!isHost(room)||room.status!=="playing"||room.phase!=="reloading"||Number(room.eventSeq||0)!==Number(eventSeq||0)||!["shot-live","shot-blank"].includes(event.type))return;
    const remaining=aliveUids(room);if(remaining.length<=1){const winnerUid=remaining[0]||"",winnerName=winnerUid?roomPlayer(room,winnerUid).name:"";await fb.updateDoc(ref,{status:"finished",phase:"finished",winnerUid,winnerName,turnStartedAtMs:0,updatedAtMs:now()});return;}
    const actor=event.actorUid,keepTurn=event.type==="shot-blank"&&event.targetUid===actor&&Number(room.hp?.[actor]||0)>0,nextUid=keepTurn?actor:nextTurn(room,actor);
    if(Number(room.shellTotal||0)<=0){await fb.updateDoc(ref,{activeUid:nextUid,turnStartedAtMs:0,updatedAtMs:now()});await hostReloadEmptyRoom(roomId,eventSeq);return;}
    await fb.updateDoc(ref,{phase:"playing",activeUid:nextUid,turnStartedAtMs:now(),updatedAtMs:now()});
  }
  ...
  async function hostProcessCommand(cmd,local=false){
    ...
    if(room.phase!=="playing"&&cmd.action!=="leave"){await resolveCommandDoc(fb,cmd,"denied",["shot-live","shot-blank"].includes(room.lastEvent?.type)?"Der vorherige Schuss wird noch ausgeführt.":"Die Kammer wird gerade neu geladen.");return;}
    ...
    // V4: the shot animation is authoritative part of the turn. Keep the shooter as
    // active player and lock the table until the visible fire/blank sequence finished.
    state.phase="reloading";state.turnStartedAtMs=0;
    await fb.updateDoc(ref,{status:"playing",phase:state.phase,round:state.round,activeUid:cmd.uid,turnStartedAtMs:0,direction:state.direction,hp:state.hp,items:state.items,effects:state.effects,shellLive:state.shellLive,shellBlank:state.shellBlank,shellTotal:state.shellTotal,eventSeq:state.eventSeq,lastEvent:event,winnerUid:"",winnerName:"",dealerEpoch:state.dealerEpoch,updatedAtMs:now()});
    scheduleShotResolution(room.roomId,state.eventSeq);
