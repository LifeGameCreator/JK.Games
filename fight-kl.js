(() => {
  ...
  const VERSION = "20260802-fight-kl-v139-boss-wave-hud-fix";
  const MAX_LEVEL = 100;
  const MAX_STAR = 5;
  const INVENTORY_LIMIT = 900;
  const CANVAS_W = 1280;
  const CANVAS_H = 720;
  const WORLD_W = 2200;
  const WORLD_H = 1400;
  ...
  const COOP_SNAPSHOT_INTERVAL = 70;
  const COOP_INPUT_INTERVAL = 35;
  ...
  ITEMS.push(...V137_EXPANSION_ITEMS);
...
function detailHtml(item) {
  ...
  const canRepair = max && item.durability < max && ([repairFamily,"universal","field","precision","nano","phase","mythic","exotic","universe"].some(id=>id&&data.repairKits[id]>0));
...
function repairItem(id) {
  ...
  const repairPriority=["universe","exotic","mythic","phase","nano","precision","field",family,"universal"].filter(Boolean);
...
function dailyShopItems(category = UI.shopCategory) {
  ...
  function bulkDeleteCandidateItems(rarity){
    const data=ensureState();
    return data.inventory.filter(item=>rarity==="all"||rarityKey(item)===rarity);
  }
  ...
  function requestBulkDeleteFightItems(rarity){
    ...
    if(!deletable.length)return toast("Nichts löschbar",keep?`${itemDef(keep).name} bleibt als letztes Item erhalten.`:`Keine Items der Stufe ${label} vorhanden.`);
    const equippedCount=deletable.filter(item=>Object.values(data.equipped||{}).includes(item.uid)).length;
    const modal=showModal(`<div class="fkl-bulk-delete-confirm"><div class="fkl-bulk-delete-icon">🗑️</div><small class="fkl-kicker">MASSEN-LÖSCHEN</small><h3>${deletable.length} Items löschen?</h3><p>Auswahl: <b>${escapeHtml(label)}</b>.<br>Gelöschte Items bringen kein Geld und können nicht wiederhergestellt werden.${equippedCount?`<br><b>${equippedCount}</b> ausgerüstete Items werden automatisch abgelegt.`:""}${keep?`<br><br>Das stärkste beziehungsweise aktive letzte Item <b>${escapeHtml(itemDef(keep).name)}</b> bleibt erhalten.`:""}</p><div class="fkl-modal-actions"><button class="fkl-btn danger" type="button" data-fkl-confirm-bulk-delete>Jetzt löschen</button><button class="fkl-btn" type="button" data-fkl-cancel-bulk-delete>Abbrechen</button></div></div>`);
  ...
  function bulkDeleteFightItems(rarity){
    const data=ensureState(),candidates=bulkDeleteCandidateItems(rarity),keep=bulkDeleteKeepItem(candidates),removeIds=new Set(candidates.filter(item=>item.uid!==keep?.uid).map(item=>item.uid));
    ...
    if(!data.equipped[data.activeWeaponSlot])data.activeWeaponSlot=WEAPON_SLOT_KEYS.find(slot=>data.equipped[slot])||"melee";
    data.itemsBulkDeleted=Math.max(0,Math.floor(Number(data.itemsBulkDeleted)||0))+removeIds.size;
    safeSave();updateHead();drawInventory();toast("Items gelöscht",`${removeIds.size} Gegenstände wurden dauerhaft entfernt.`);
  }
  ...
  function renderShop() {
    ...
    UI.main.innerHTML = `<div class="fkl-page">${pageHeader("Arsenal-Shop", "324 neue Items erweitern alle Seltenheiten und Kategorien. Der V136-Waffen-Nerf, Preise, Gegner und Wellen bleiben unverändert.")}<nav class="fkl-shop-tabs">${tabs}</nav>${rarityTabs}${universeBanner}${content}</div>`;
    bindPageHome();
...
function startCombat(startAtWave = null, options = {}) {
  ...
  function startWave(number) {
    ...
    showCombatMessage(bossWave ? `⚠ HAUPTBOSS · WELLE ${number}` : number >= 20 ? `WELLE ${number} · ELITE-ZONE` : `WELLE ${number}`); updateHud();
  }
  ...
  function spawnEnemy(spec) {
    ...
    if (spec.boss) {
      ...
      s.enemies.push(enemy); s.boss = enemy; s.bossPending = false; s.expectedBoss = boss; showCombatMessage(`${boss.name} BETRITT DIE ARENA`); updateHud(); return;
    }
...
function beginReload() {
  ...
  function previewCoopSpecial(player) {
  ...
  function relayCoopSpecialVisual(event) {
  ...
  function triggerSpecial() {
    ...
    const coopRelay=s.coop?.role==="host"?{seq:++s.coop.specialVisualSeq,owner:p===s.coop.remote?"guest":"host",sourceSeq:p===s.coop.remote?Number(s.coop.processingGuestSpecialSeq||0):0,type,color:spec?.color||"#fff",x:p.x,y:p.y,angle:p.angle,points:[]}:null;
  ...
  function fireEnemyProjectile(enemy, angle, damage) {
    const s = UI.session; s.enemyProjectiles.push({ netId:uid(), x: enemy.x + Math.cos(angle) * enemy.radius, y: enemy.y + Math.sin(angle) * enemy.radius, vx: Math.cos(angle) * 370, vy: Math.sin(angle) * 370, radius: enemy.boss ? 7 : 5, damage, life: 3, color: enemy.color });
  }
...
function updateHud() {
  ...
  function updateCompanion(dt) {
    ...
    if(target){
      ...
      if(c.cooldown<=0&&d<=Math.max(c.range,desired+18)){
        ...
        else{const projectileSpeed=620;s.projectiles.push({netId:uid(),x:c.x,y:c.y,vx:Math.cos(c.angle)*projectileSpeed,vy:Math.sin(c.angle)*projectileSpeed,radius:4,damage:c.damage,life:2.2,pierce:c.pierce,splash:c.splash,crit:false,color:rarityDef(c.item).color,ownerKey:coopOwnerKey(p),hit:new Set()});}
  ...
  function updateHud(){
    ...
    set("[data-fkl-wave]",s.coop?`KOOP · WELLE ${s.wave}${bossWave?" · BOSS":""}`:p.godMode?`WELLE ${s.wave}${bossWave?" · BOSS":""} · GOD MODE`:`WELLE ${s.wave}${bossWave?" · BOSS":""}`);
  ...
  function updateCoopGuest(dt,now){
    ...
    for(const entity of [p,c.remote]){if(!entity)continue;entity.attackAnim=Math.max(0,Number(entity.attackAnim||0)-dt);entity.hitFlash=Math.max(0,Number(entity.hitFlash||0)-dt);if(entity.companion){entity.companion.runPhase=Number(entity.companion.runPhase||0)+dt*(entity.companion.moving?13:4);entity.companion.attackAnim=Math.max(0,Number(entity.companion.attackAnim||0)-dt);}}
  ...
  function applyCoopSnapshot(data){
    const s=UI.session,c=s?.coop,snap=data?.snapshot;if(!s||!c)return;if(data.status==="ended"){finishCoopGuest(data.finishReason||"Der KOOP-Lauf wurde beendet.");return;}const snapshotSeq=coopFinite(snap?.seq,-1);if(!snap||snapshotSeq<=Number(c.lastSnapshotSeq||-1))return;c.lastSnapshotSeq=snapshotSeq;c.lastSnapshotAt=performance.now();s.wave=coopFinite(snap.wave,s.wave);s.score=coopFinite(snap.score,s.score);s.kills=coopFinite(snap.kills,s.kills);s.themeIndex=coopFinite(snap.themeIndex,s.themeIndex);s.theme=ARENA_THEMES[s.themeIndex%ARENA_THEMES.length]||ARENA_THEMES[0];s.isBossWave=coopHas(snap,"bossWave")?!!snap.bossWave:(s.wave%10===0);s.bossPending=coopHas(snap,"bossPending")?!!snap.bossPending:s.bossPending;
