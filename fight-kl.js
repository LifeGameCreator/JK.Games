(() => {
  ...
  const VERSION = "20260802-fight-kl-v136-weapons-second-half-nerf";
  const MAX_LEVEL = 100;
  const MAX_STAR = 5;
  ...
  const COOP_SNAPSHOT_INTERVAL = 95;
  const COOP_INPUT_INTERVAL = 45;
  const COOP_MAX_PREDICTION = .22;
...
function dailyShopItems(category = UI.shopCategory) {
  ...
  function renderShop() {
    ...
    UI.main.innerHTML = `<div class="fkl-page">${pageHeader("Arsenal-Shop", "Alle Waffen verursachen gegenüber V135 nochmals nur 50 % Schaden und liegen damit bei 25 % ihres ursprünglichen Waffenschadens. Preise, Gegner und Wellen bleiben unverändert.")}<nav class="fkl-shop-tabs">${tabs}</nav>${universeBanner}${content}</div>`;
    bindPageHome();
...
function beginReload() {
  ...
  function previewCoopSpecial(player) {
    ...
    const show=(enemy,damage,color=spec?.color||"#fff")=>{const value=Math.max(1,Math.round(damage));total+=value;addDamageText(enemy.x,enemy.y-enemy.radius-8,`≈${value}`,color,18);};
  ...
  function triggerSpecial() {
    ...
    if(s?.coop?.role==="guest"){
      ...
      s.coop.specialSeq++;const shown=previewCoopSpecial(p);showCombatMessage(`${SPECIALS[p.specialType]?.name||"SPECIAL"} AKTIV${shown?` · ≈${NUMBER.format(shown)} SCHADEN`:""}`);sendCoopInput(performance.now(),true);updateHud();return;
    }
...
function updateHud() {
  ...
  async function joinWaveCoop(){
    ...
    try{
      ...
      UI.coopWaiting={fb,user,lobbyRef,inviteRef:ownInvite,processing:false,inviteUnsub:null};if(status)status.textContent="Warteschlange aktiv · Live-Verbindung wartet auf einen Mitspieler …";if(cancel)cancel.hidden=false;
  ...
  async function consumeCoopInvite(waiting,snap){
    ...
    try{
      ...
      const match=matchSnap.data();if(match.hostUid!==waiting.user.uid)throw new Error("Die Einladung gehört nicht zu dieser Host-Lobby.");
  ...
  async function stopCoopWaiting(removeLobby=true){
    clearTimeout(UI.coopPollTimer);UI.coopPollTimer=0;const waiting=UI.coopWaiting;UI.coopWaiting=null;if(!waiting)return;try{waiting.inviteUnsub?.();}catch{}
  ...
  function startCoopArena(match,fb,user){
    ...
    const stage=UI.main?.querySelector(".fkl-stage");stage?.insertAdjacentHTML("beforeend",`<div class="fkl-coop-team" data-fkl-coop-team><div class="local"><small>${escapeHtml(local.coopName)}</small><div><i data-fkl-coop-local-bar></i></div><b data-fkl-coop-local-hp></b></div><span>🛡️ KOOP · LIVE-SYNC</span><div class="remote"><small>${escapeHtml(remote.coopName)}</small><div><i data-fkl-coop-remote-bar></i></div><b data-fkl-coop-remote-hp></b></div></div><button class="fkl-coop-leave" type="button" data-fkl-coop-leave>KOOP verlassen</button>`);UI.main?.querySelector("[data-fkl-coop-leave]")?.addEventListener("click",leaveWaveCoop);
    setupCoopNetwork();showCombatMessage(role==="host"?"KOOP GESTARTET · LIVE-SYNC AKTIV":"KOOP VERBUNDEN · SPECIALS UND SOUND AKTIV");updateHud();
  }
  function setupCoopNetwork(){
    const s=UI.session,c=s?.coop;if(!c)return;const fb=c.fb,selfRef=fb.doc(fb.db,COOP_MATCH_COLLECTION,c.matchId,"players",c.user.uid);c.selfRef=selfRef;c.inputRefs=Array.from({length:24},(_,index)=>fb.doc(fb.db,COOP_MATCH_COLLECTION,c.matchId,"players",c.user.uid,"inputs",String(index)));
    ...
    if(c.role==="host"){
      ...
      c.frameRefs=Array.from({length:16},(_,index)=>fb.doc(fb.db,COOP_MATCH_COLLECTION,c.matchId,"frames",String(index)));
  ...
  function updateCoopRemotePlayer(dt,now){
    ...
    if(Number(input.specialSeq||0)>Number(c.lastRemoteSpecialSeq||0)&&p.specialReady){c.lastRemoteSpecialSeq=Number(input.specialSeq||0);withSessionPlayer(p,()=>triggerSpecial());}
  ...
  function applyCoopSnapshot(data){
    ...
    const oldEnemies=new Map(s.enemies.map(enemy=>[enemy.id,enemy]));s.enemies=(snap.enemies||[]).map(raw=>{const old=oldEnemies.get(raw.id),targetX=Number(raw.x),targetY=Number(raw.y);if(!old)return{...raw,targetX,targetY,targetAngle:Number(raw.angle||0),netVx:0,netVy:0,netSampleAt:arrival};const previousTargetX=Number.isFinite(old.targetX)?old.targetX:old.x,previousTargetY=Number.isFinite(old.targetY)?old.targetY:old.y,inferredVx=(targetX-previousTargetX)/sampleDt,inferredVy=(targetY-previousTargetY)/sampleDt;return{...raw,x:old.x,y:old.y,angle:old.angle,targetX,targetY,targetAngle:Number(raw.angle||0),netVx:Number(old.netVx||0)*.3+inferredVx*.7,netVy:Number(old.netVy||0)*.3+inferredVy*.7,netSampleAt:arrival};});
