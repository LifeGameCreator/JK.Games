(() => {
  ...
  const COOP_SNAPSHOT_INTERVAL = 500;
  const COOP_INPUT_INTERVAL = 500;
...
function updateHud() {
  ...
  async function verifyCoopQueue(fb,lobbyRef,queueKey,userUid){
    ...
    const matchRef=fb.doc(fb.db,COOP_MATCH_COLLECTION,candidate.matchId),a=await fb.getDoc(matchRef).catch(()=>null);if(!a?.exists()||a.data()?.status!=="waiting")return{stale:true,matchId:candidate.matchId};const seq=Number(a.data()?.heartbeatSeq||0);await new Promise(resolve=>setTimeout(resolve,4200));const b=await fb.getDoc(matchRef).catch(()=>null);if(!b?.exists())return{stale:true,matchId:candidate.matchId};const data=b.data(),uids=coopParticipantUids(data);if(data.status!=="waiting"||Number(data.heartbeatSeq||0)<=seq||uids.length>=Number(data.teamSize||2))return{stale:true,matchId:candidate.matchId};return{matchRef,match:{...data,id:matchRef.id}};
  }
  function watchCoopWaiting(waiting){
    ...
    if(waiting.isHost){clearInterval(UI.coopHeartbeatTimer);UI.coopHeartbeatTimer=setInterval(async()=>{if(UI.coopWaiting!==waiting)return;waiting.heartbeatSeq=Number(waiting.heartbeatSeq||1)+1;await waiting.fb.runTransaction(waiting.fb.db,async tx=>{const snap=await tx.get(waiting.lobbyRef),root=snap.exists()?snap.data():{},queues={...(root.queues||{})},q=queues[waiting.queueKey];if(q?.matchId!==waiting.matchId)return;queues[waiting.queueKey]={...q,heartbeatSeq:waiting.heartbeatSeq};tx.set(waiting.matchRef,{heartbeatSeq:waiting.heartbeatSeq,updatedAtMs:Date.now()},{merge:true});tx.set(waiting.lobbyRef,{queues},{merge:false});}).catch(()=>{});},3000);}
  }
  async function joinWaveCoop(){
